...
local Config = {
  ...
  FILES = {
    players = "/home/players.db",
    buyCatalog = "/home/buyCatalog.lua",
    sellCatalog = "/home/sellCatalog.lua",
    reports = "/home/reports.json",
    pendingChanges = "/home/pending_changes.lua",
    catalogVersion = "/home/catalogVersion.dat",
    setsCatalog = "/home/setsCatalog.lua",
    setsProgress = "/home/setsProgress.lua"
  }
}
local State = {
...
local pimIntrospect
local applySetsFilter, performSetPurchase, performSetDispense
-- ===== ВРЕМЯ / ЛОГ =====
...
function CatalogModule.loadVersion()
  ...
  if type(data) == "table" then
    Data.versions = { buy = tonumber(data.buy) or 0, sell = tonumber(data.sell) or 0, users = tonumber(data.users) or 0, sets = tonumber(data.sets) or 0 }
  else
    Data.versions = { buy = tonumber(data) or 0, sell = 0, users = 0, sets = 0 }
  end
...
-- ===== НАБОРЫ: ХЕЛПЕРЫ =====
local function setItemKey(internalName, damage) return tostring(internalName) .. ":" .. tostring(damage or 0) end
local function setProgressGet(setId)
  if not setId then return nil end
  return Data.setsProgress[setId]
end
local function setDispensedFor(set, item)
  local pr = setProgressGet(set and set.id)
  if not pr or not pr.dispensed then return 0 end
  return pr.dispensed[setItemKey(item.internalName, item.damage)] or 0
end
...
local function setPurchasedNotDone(set)
  local pr = setProgressGet(set and set.id)
...
function applySetsFilter()
  ...
  if State.setView == "list" then
    if #Data.setsDisplay > 0 then
      ...
      State.selectedIndex, State.selectedSet = 0, nil
...
local function loadSetsPages()
  ...
  while page <= totalPages do
    ...
    if not data then State.welcomeLog[logIndex] = { message = "Наборы: нет ответа", color = Colors.error_red }; drawWelcomeScreen(); return nil, nil end
    if data.status ~= "ok" then State.welcomeLog[logIndex] = { message = "Наборы: " .. tostring(data.message), color = Colors.error_red }; drawWelcomeScreen(); return nil, nil end
    for _, s in ipairs(data.sets or {}) do table.insert(allSets, s) end
    version = tonumber(data.version) or version
    totalPages = data.totalPages or 1
    page = page + 1
  end
  State.welcomeLog[logIndex] = { message = "НАБОРЫ: " .. #allSets .. " наборов", color = Colors.success_green }; drawWelcomeScreen()
  return allSets, version
end
-- ===== СИНХРОНИЗАЦИЯ =====
...
local function applySetsDelta(changes, newVersion)
  ...
  for _, ch in ipairs(changes or {}) do
    ...
    if sid then
      if ch.action == "REMOVE" or ch.action == "delete" then
        local idx = index[sid]
        if idx then table.remove(currentData, idx); delCnt = delCnt + 1
          index = {}; for i2, s2 in ipairs(currentData) do index[tonumber(s2.id)] = i2 end
        end
  ...
  if newVersion then Data.versions.sets = newVersion end
...
local function syncCatalogsIfNeeded(source)
  ...
  local serverSets = tonumber(v.setsVersion) or nil
  ...
  local needSets = (serverSets ~= nil) and (serverSets ~= (Data.versions.sets or 0)) or false
  ...
  if needSets then
    ...
    if sd and sd.status == "ok" then
      applySetsDelta(sd.changes or {}, tonumber(sd.version) or serverSets)
    else
      local sets, ver = loadSetsPages()
      if sets then
        Data.setsCatalog = sets
        Data.versions.sets = tonumber(ver) or serverSets or Data.versions.sets
        CatalogModule.saveSets(sets); CatalogModule.saveVersions()
        if State.currentShopMode == "sets" then applySetsFilter() end
      end
    end
  end
...
local function loadFullInit()
  ...
  usersData = nil
  PlayerModule.save()
  local sets, sver = loadSetsPages()
  if sets then
    Data.setsCatalog = sets
    Data.versions.sets = tonumber(sver) or vSets
    CatalogModule.saveSets(sets)
  else
    Data.versions.sets = vSets
  end
  Data.versions.buy = vBuy; Data.versions.sell = vSell; Data.versions.users = vUsers
  CatalogModule.saveVersions()
    ...
    if State.setView == "list" then
      ...
      for row = 1, State.visibleRows do
        ...
        if s then
          ...
          local symbolColor = isSelected and Colors.cyan or Colors.purple
          ...
          local st, stCol
          if setPurchasedNotDone(s) then st, stCol = "ЖДЁТ", Colors.warning
          elseif setIsAvailable(s) then st, stCol = "ГОТОВ", Colors.success_green
          else st, stCol = "НЕТ", Colors.text_dark end
      ...
      for row = 1, State.visibleRows do
        ...
        if it then
          ...
          writeText(listX + listW - 22, y, got .. "/" .. need, gotCol, bg)
        else Buffer.fill(listX + 1, y, listW - 2, 1, " ", Colors.text_bright, Colors.bg_main) end
      end
    end
    return
...
local function drawSetBox(panelX, panelY, panelW, topH, set)
  ...
  local st, stCol
  if setPurchasedNotDone(set) then st, stCol = "ЖДЁТ ВЫДАЧИ", Colors.warning
  elseif setIsAvailable(set) then st, stCol = "ГОТОВ К ВЫДАЧЕ", Colors.success_green
  else st, stCol = "НЕДОСТУПЕН", Colors.text_dark end
  writeText(panelX + 2, baseY + lineSpacing, "СТАТУС: " .. st, stCol, Colors.bg_main)
...
local function drawSetContentsBox(panelX, panelY, panelW, topH, set)
  ...
  if it then
    ...
    local need = tonumber(it.qty) or 0
    local gotCol = (got >= need and need > 0) and Colors.success_green or (got > 0 and Colors.warning or Colors.text_dark)
    writeText(panelX + 2, baseY + lineSpacing * 3, "ПОЛУЧЕНО: " .. got .. "/" .. need, gotCol, Colors.bg_main)
  else
  ...
  local dispCol = (dispTotal >= reqTotal and reqTotal > 0) and Colors.success_green or (dispTotal > 0 and Colors.warning or Colors.text_dark)
  writeText(panelX + 4 + inputW + 2, inputY, "ВЫДАНО: " .. dispTotal .. "/" .. reqTotal, dispCol, Colors.bg_main)
  local btnY = inputY + 2
  local purchased = setPurchasedNotDone(set)
  local bw1, bw2 = 16, 10
  local bx1 = panelX + 2
  local bx2 = bx1 + bw1 + 2
  if purchased then
...
local function performSetDispense()
  ...
  if not TransactionModule.lock(txid) then openErrorModal("Подожди", "Транзакция уже выполняется или слишком частые запросы."); return end
  clearLog()
  addLogMessage("📦 Выдача набора: " .. set.name, Colors.accent_main)
  ...
  for _, it in ipairs(set.items or {}) do
    ...
    if need > 0 then
      ...
      while need > 0 do
        ...
        if got > 0 then gotTotal = gotTotal + got; need = need - got else break end
      end
      if gotTotal > 0 then
        pr.dispensed[key] = (pr.dispensed[key] or 0) + gotTotal
        addLogMessage("✔ " .. (it.displayName or it.internalName) .. ": +" .. gotTotal .. " шт.", Colors.success_green)
      end
    end
  end
  ...
  if completed then
    ...
    addLogMessage("⚠ Выдано частично: " .. setDispensedTotal(set) .. "/" .. setTotalQty(set) .. ". Вернитесь позже.", Colors.warning)
  end
  State.needStockRefresh = true
  markDirty("full")
end
    ...
    if isButtonClicked(btn, x, y) then
      ...
      elseif btn.id == "set_open" then
        if State.selectedSet then
          ...
          clearLog()
          addLogMessage("Набор: " .. State.selectedSet.name, Colors.accent_cyan)
...
function switchToMode(mode)
  ...
  if mode == "sets" then
    ...
    if #Data.setsDisplay > 0 then
      State.selectedIndex = 1; State.selectedSet = Data.setsDisplay[1]
    else State.selectedIndex = 0; State.selectedSet = nil end
    updateSelectorDisplay(nil)
  else
    if mode == "buy" then
