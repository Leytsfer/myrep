-- ============================================================
-- TRADE MARKET CLIENT v6.10.5 (HEARTBEAT + PAUSE + BAN + SEARCH FIX)
-- ============================================================
...
local function setTotalQtyLocal(set)
local t = 0
...
local function setIsAvailableLocal(set)
if not set or not set.items then return false end
local idx = meSnapshot()
for _, it in ipairs(set.items) do
if (idx[it.internalName .. ":" .. (it.damage or 0)] or 0) < (tonumber(it.qty) or 0) then return false end
...
{ text = "Есть сортировка каталога по конкретному моду.", color = Colors.text_main, center = true },
{ text = "", color = Colors.text_main },
...
{ text = "Баланс COINS & EMA всегда доступен в разделе АККАУНТ.", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
...
{ text = "Если выдано меньше чем покупалось, магазин спишет оплату только за фактически выданные предметы.", color = Colors.success_green, center = true },
{ text = "", color = Colors.text_main },
...
{ text = "", color = Colors.text_main },
{ text = "Не сходите с PIM во время процесса крафта, а также заранее освободите место в инвентаре.", color = Colors.tomato, center = true }
}
...
{ text = "Купить набор можно только если в наличие есть все предметы набора. Купить отдельные предметы из набора не получится.", color = Colors.text_main, center = true },
...
-- Помощь: сдвинута вправо (x = 5 вместо 2)
local helpBtnW = 19
drawButton(5, y + 1, helpBtnW, btnH, "[ Нужна помощь ]", Colors.accent_cyan, Colors.white, "help_open")
...
-- "Нашёл ошибку": опущена на строку 49 (в один ряд с остальными)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.10.5", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
-- ===== HEARTBEAT + BAN RECHECK =====
local function asBoolFlag(v)
...
if not ok1 or type(list) == "table" and #list == 0 then
