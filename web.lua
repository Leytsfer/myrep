...
-- Лог с переменным отступом: gap=1 => пустая строка между, gap=0 => плотно
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.11", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.11", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
-- компактный лог автокрафта (gap=0)
...
-- временно блокируем полную сумму как гарантию; фактическое списание будет по факту выдачи
...
-- возвращаем разницу между заблокированным и фактически выданным
...
-- лог оплаты ТОЛЬКО по факту выданного (net), пишется ПОСЛЕ выдачи
...
acLogJob(job, { { "Списано только за выданное количество.", Colors.warning } })
job.phase = "partial"
else
acLogJob(job, { { "Результат крафта: ", Colors.text_bright }, { "Неудача (не хватает ресурсов в МЭ)", Colors.error_red } })
acLogJob(job, { { "Попробуйте заказать меньше предметов", Colors.warning } })
...
-- игрок просто ждёт крафт, ввод игнорируется
