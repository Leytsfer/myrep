-- ============================================================
-- TRADE MARKET CLIENT v6.9.13 (DIAG PRESENCE)
-- ============================================================
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.13", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.13", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
writeDebugLog("🔎 open: pre getPlayer=" .. tostring(PimModule.getPlayer()))
updateBuyCatalogFromME()
...
gone = true
elseif not (e2 == "player_on" or e2 == "pim" or e2 == "pim_player_enter") then
...
writeDebugLog("🔎 open: post gone=" .. tostring(gone) .. " getPlayer=" .. tostring(PimModule.getPlayer()))
if gone then
...
elseif e2 == "player_on" or e2 == "pim" or e2 == "pim_player_enter" then -- дубль
else table.insert(State.deferredEvents, pulled) end
end
writeDebugLog("🔎 enter: after sync+drain stillHere=" .. tostring(stillHere) .. " getPlayer=" .. tostring(PimModule.getPlayer()))
if stillHere then
local deadline = computer.uptime() + 0.4
while computer.uptime() < deadline and stillHere do
local pulled = { pcall(event.pull, 0.05) }
if pulled[1] and pulled[2] then
table.remove(pulled, 1)
local e2, a1 = pulled[1], pulled[2]
...
writeDebugLog("🔎 enter: after window stillHere=" .. tostring(stillHere) .. " getPlayer=" .. tostring(PimModule.getPlayer()))
end
...
local ok, actual = pcall(PimModule.getPlayer)
if not ok then actual = nil end
writeDebugLog("🔍 Watchdog: getPlayer=" .. tostring(actual) .. " expected=" .. tostring(State.currentPlayer))
if not actual or actual == "" then
writeDebugLog("⚠️ Watchdog: getPlayer вернул nil/пусто — закрываем магазин")
handlePlayerLeave()
elseif actual ~= State.currentPlayer then
writeDebugLog("⚠️ Watchdog: getPlayer вернул другого игрока — закрываем магазин")
handlePlayerLeave()
end
end
end
