-- ============================================================
-- TRADE MARKET CLIENT v6.10.2 (SETS OPTIMIZED)
-- ============================================================
...
local st, stCol
if setCompleted(s) then st, stCol = "ВЫДАН", Colors.text_dark
elseif setProgressGet(s.id) then st, stCol = "ЖДЁТ", Colors.warning
...
for _, m in ipairs(items) do if m.name == it.internalName and (m.damage or 0) == (it.damage or 0) then mc = mc + (m.size or 0) end end
end
end
...
if pr and not pr.completed then stText, stColor = "ОЖИДАЕТ ВЫДАЧИ", Colors.warning
...
-- УБРАНА строка "Выдано: 0/0" напротив поля ввода
local btnY = inputY + 2
local pr = setProgressOf(set)
local bought = pr and not pr.completed
local completed = pr and pr.completed
local bw1, bw2 = 16, 10
local bx1 = panelX + 2
local bx2 = bx1 + bw1 + 2
if bought then
...
local totalY = btnY + 2
-- Если набор полностью выдан, показываем сообщение об успешной выдаче
if completed then
writeText(panelX + 2, totalY, "Выдача набора успешно завершена", Colors.blue, Colors.bg_main)
elseif bought then
-- Если куплен, но не полностью выдан, показываем прогресс
...
-- Если не куплен, показываем цену
local coinPrice, emaPrice = set.price_coin or 0, set.price_ema or 0
writeText(panelX + 2, totalY, "ИТОГО: ", Colors.text_bright, Colors.bg_main)
local totalX = panelX + 2 + unicode.len("ИТОГО: ")
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.10.2", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.10.2", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
clearLog()
-- ИЗМЕНЕНО: новое сообщение в логировании
addLogMessage("Перед \"Покупкой набора\" не забудь освободить свой инвентарь", Colors.warning)
markDirty("full")
end
