-- ============================================================
-- TRADE MARKET CLIENT v6.10.3 (SETS RESET FIX)
-- ============================================================
...
-- Если есть прогресс (куплен, но не выдан полностью) -> ОЖИДАЕТ
-- Если прогресса нет (nil) -> проверяем наличие в МЭ
if pr then
...
-- Убрали строку "ВЫДАНО: ..." напротив поля ввода, как просили
local btnY = inputY + 2
local pr = setProgressOf(set)
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.10.3", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.10.3", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
if completed then
-- ПОЛНЫЙ СБРОС: удаляем запись о прогрессе, чтобы набор стал как новый
