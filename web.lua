...
-- Главный признак: имя игрока на PIM (надёжнее, чем инвентарь)
local nm = nil
...
if State.pimActive and pimPresenceState() ~= "present" then
writeDebugLog("⚠️ Игрок отсутствует сразу после открытия магазина — закрываем сессию")
handlePlayerLeave()
end
end
...
if pimPresenceState() ~= "present" then
writeDebugLog("⚠️ Действие получено, но игрока нет на PIM — закрываем магазин")
handlePlayerLeave()
return
end
...
if pimPresenceState() ~= "present" then handlePlayerLeave(); return end
if State.bannedNow then
...
if State.currentPlayer and (playerName == nil or playerName == State.currentPlayer or PimModule.namesMatch(playerName, State.currentPlayer)) then
State.absentStreak = 2 -- подтверждение придёт на следующем тике watchdog (~1 сек)
end
end
end
end
