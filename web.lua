...
-- Патч 9: бан-окно без кнопок (поверх каталога)
...
drawCenteredText(y + 4, "Ваш аккаунт заблокирован администрацией.", Colors.text_main, Colors.bg_modal)
local reason = (State.currentSession and State.currentSession.banReason) or ""
if reason ~= "" then drawCenteredText(y + 6, "Причина: " .. reason, Colors.warning, Colors.bg_modal) end
return
end
...
-- сервер сообщает состояние ТО
if r.maintenance ~= nil then State.serverState.maintenance = r.maintenance end
if r.paused ~= nil then
if r.paused and not State.serverState.terminalPaused then
State.serverState.terminalPaused = true
State.pauseGraceUntil = computer.uptime() + Config.PAUSE_GRACE_SECONDS
if State.pimActive and not State.bannedNow then
State.modalState.active = true; State.modalState.kind = "pause_countdown"; State.modalState.data = {}
State.currentScreen = "modal"
...
elseif not r.paused and State.serverState.terminalPaused then
State.serverState.terminalPaused = false
State.pauseGraceUntil = 0
if State.modalState.kind == "pause_countdown" then closeModal() end
markDirty("full")
end
...
-- Патч 7: всегда свежий баланс с сервера
...
-- бан: окно без кнопок поверх каталога
if State.currentSession.banned then
State.bannedNow = true
forceRender()
return
end
