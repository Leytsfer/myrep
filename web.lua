-- ============================================================
-- TRADE MARKET CLIENT v6.9.14 (PRESENCE CONFIRM WINDOW)
-- ============================================================
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.14", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.14", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
forceRender()
-- короткое пост-окно: ловим player_off, пришедший сразу после открытия
local gone = false
...
-- дренаж очереди после синхронизации
while true do
...
if stillHere then
-- ОКНО ПОДТВЕРЖДЕНИЯ: ждём PRESENCE_CONFIRM_DELAY сек; любой player_off отменяет открытие
...
writeDebugLog("🔎 enter: after confirm window stillHere=" .. tostring(stillHere))
end
