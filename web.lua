-- ============================================================
-- TRADE MARKET CLIENT v6.9.20 (CONFIRM WINDOW 4s)
-- ============================================================
...
if not okp or not pim then return "unknown" end
-- Основной сигнал: getInventoryName отвечает именем, когда игрок привязан;
-- когда никого нет — вызов падает с ошибкой или возвращает nil/"" => считаем "absent"
if pim.getInventoryName then
local ok, r = pcall(pim.getInventoryName, pim)
if not ok then return "absent" end
if type(r) == "string" and r ~= "" then return "present" end
return "absent"
end
-- Запасной: getInventorySize
if pim.getInventorySize then
local ok, r = pcall(pim.getInventorySize, pim)
if not ok then return "absent" end
if type(r) == "number" then return (r > 0) and "present" or "absent" end
return "absent"
end
return "unknown"
end
...
addWelcomeLog("🔍 Проверка присутствия...", Colors.warning)
