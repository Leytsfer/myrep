-- ============================================================
-- TRADE MARKET CLIENT v6.9.21 (SETS & QUESTS FIXED)
-- ============================================================
...
-- ===== НАБОРЫ: ХЕЛПЕРЫ =====
local function getSetKey(item) return tostring(item.internalName) .. ":" .. tostring(item.damage or 0) end
local function getSetProgress(setId)
if not setId then return nil end
return Data.setsProgress[setId]
end
local function isSetFullyDispensed(set)
local pr = getSetProgress(set.id)
if not pr or not pr.dispensed then return false end
for _, item in ipairs(set.items or {}) do
local needed = tonumber(item.qty) or 0
local got = math.floor(pr.dispensed[getSetKey(item)] or 0)
if got < needed then return false end
end
return true
end
local function isSetAvailableInME(set)
if not component.isAvailable("me_interface") then return false end
local me = component.me_interface
local ok, items = pcall(function() return me.getItemsInNetwork() end)
if not ok or type(items) ~= "table" then return false end
local meIndex = {}
for _, it in ipairs(items) do meIndex[it.name .. ":" .. (it.damage or 0)] = (meIndex[it.name .. ":" .. (it.damage or 0)] or 0) + (it.size or 0) end
for _, item in ipairs(set.items or {}) do
local needed = tonumber(item.qty) or 0
local have = meIndex[item.internalName .. ":" .. (item.damage or 0)] or 0
if have < needed then return false end
end
return true
end
local function getSetStatus(set)
local pr = getSetProgress(set.id)
if pr and not isSetFullyDispensed(set) then return "ОЖИДАЕТ", Colors.warning end
if isSetAvailableInME(set) then return "ГОТОВ", Colors.success_green end
return "НЕТ В МЭ", Colors.text_dark
end
local function applySetsFilter()
Data.setsDisplay = {}
local searchLower = lowerStr(State.searchInput or "")
for _, set in ipairs(Data.setsCatalog or {}) do
if set.enabled ~= 0 and set.enabled ~= false then
local nameLower = lowerStr(set.name or "")
if matchesSearch(nameLower, searchLower) then table.insert(Data.setsDisplay, set) end
end
end
...
else State.selectedIndex, State.selectedSet = 0, nil end
end
...
for _, set in ipairs(sets) do table.insert(allSets, set) end
totalPages = data.totalPages or 1
page = page + 1
end
...
local index = {}
for i, set in ipairs(currentData) do index[tonumber(set.id)] = i end
local addCnt, updCnt, delCnt = 0, 0, 0
for _, ch in ipairs(changes or {}) do
local set = ch.set or {}
local sid = tonumber(set.id)
if sid then
if ch.action == "delete" then
local idx = index[sid]
if idx then table.remove(currentData, idx); delCnt = delCnt + 1; index = {}; for i2, s2 in ipairs(currentData) do index[tonumber(s2.id)] = i2 end end
if State.selectedSet and tonumber(State.selectedSet.id) == sid then State.selectedSet = nil; State.selectedSetItem = nil end
else
local idx = index[sid]
if idx then currentData[idx] = set; updCnt = updCnt + 1; if State.selectedSet and tonumber(State.selectedSet.id) == sid then State.selectedSet = set end
else table.insert(currentData, set); index[sid] = #currentData; addCnt = addCnt + 1 end
end
end
end
...
local serverBuy, serverSell, serverUsers, serverSets = tonumber(v.buyVersion) or 0, tonumber(v.sellVersion) or 0, tonumber(v.usersVersion) or 0, tonumber(v.setsVersion) or 0
...
local sets = loadSetsPages()
if sets then Data.setsCatalog = sets; Data.versions.sets = serverSets; CatalogModule.saveSets(sets); CatalogModule.saveVersions(); if State.currentShopMode == "sets" then applySetsFilter() end end
end
end
if needUsers then
...
local vBuy, vSell, vUsers, vSets = tonumber(usersData.buyVersion) or 0, tonumber(usersData.sellVersion) or 0, tonumber(usersData.usersVersion) or 0, tonumber(usersData.setsVersion) or 0
...
PlayerModule.save()
local sets = loadSetsPages()
if sets then Data.setsCatalog = sets; vSets = tonumber(usersData.setsVersion) or 0; CatalogModule.saveSets(sets) end
...
local statusText, statusColor = getSetStatus(set)
writeText(listX + listW - 32, y, statusText, statusColor, bg)
writeText(listX + listW - 22, y, formatPrice(set.price_coin or 0), (set.price_coin or 0) > 0 and Colors.accent_cyan or Colors.text_dark, bg)
writeText(listX + listW - 12, y, formatPrice(set.price_ema or 0), (set.price_ema or 0) > 0 and Colors.tomato or Colors.text_dark, bg)
...
local y = listY + 3 + row
if item then
local isSelected = (State.selectedSetItem and State.selectedSetItem.internalName == item.internalName and (State.selectedSetItem.damage or 0) == (item.damage or 0))
local bg = isSelected and Colors.bg_selected or Colors.bg_main
Buffer.fill(listX + 1, y, listW - 2, 1, " ", Colors.text_bright, bg)
local symbol = isSelected and ">" or "*"
writeText(listX + 2, y, symbol, isSelected and Colors.cyan or Colors.purple, bg)
...
for _, it in ipairs(items) do if it.name == item.internalName and (it.damage or 0) == (item.damage or 0) then meQty = meQty + (it.size or 0) end end
end
end
writeText(listX + listW - 32, y, formatQtyCompact(meQty), meQty > 0 and Colors.success_green or Colors.text_dark, bg)
local pr = getSetProgress(set.id)
local got = 0
if pr and pr.dispensed then got = math.floor(pr.dispensed[getSetKey(item)] or 0) end
local needed = tonumber(item.qty) or 0
local gotColor = (got >= needed and needed > 0) and Colors.success_green or (got > 0 and Colors.warning or Colors.text_dark)
writeText(listX + listW - 16, y, got .. "/" .. needed, gotColor, bg)
else Buffer.fill(listX + 1, y, listW - 2, 1, " ", Colors.text_bright, Colors.bg_main) end
end
end
return
...
local statusText, statusColor = getSetStatus(set)
writeText(panelX + 2, baseY + lineSpacing, "СТАТУС: " .. statusText, statusColor, Colors.bg_main)
...
local baseY = panelY + 3
writeText(panelX + 2, baseY, "НАБОР: " .. (set.name or ""), Colors.text_bright, Colors.bg_main)
local item = State.selectedSetItem
if item then
writeText(panelX + 2, baseY + lineSpacing, "ПРЕДМЕТ: " .. (item.displayName or item.internalName or ""), Colors.text_bright, Colors.bg_main)
writeText(panelX + 2, baseY + lineSpacing * 2, "В НАБОРЕ: " .. (tonumber(item.qty) or 0), Colors.text_bright, Colors.bg_main)
local pr = getSetProgress(set.id)
local got = 0
if pr and pr.dispensed then got = math.floor(pr.dispensed[getSetKey(item)] or 0) end
local needed = tonumber(item.qty) or 0
local gotColor = (got >= needed and needed > 0) and Colors.success_green or (got > 0 and Colors.warning or Colors.text_dark)
writeText(panelX + 2, baseY + lineSpacing * 3, "ПОЛУЧЕНО: " .. got .. "/" .. needed, gotColor, Colors.bg_main)
else
...
local pr = getSetProgress(set.id)
local isPurchased = pr and not isSetFullyDispensed(set)
if isPurchased then
local totalGot = 0
local totalNeeded = 0
if pr and pr.dispensed then for _, v in pairs(pr.dispensed) do totalGot = totalGot + math.floor(v) end end
for _, it in ipairs(set.items or {}) do totalNeeded = totalNeeded + (tonumber(it.qty) or 0) end
writeText(panelX + 18, inputY, "ВЫДАНО: " .. totalGot .. "/" .. totalNeeded, Colors.warning, Colors.bg_main)
else
writeText(panelX + 18, inputY, "ВЫДАНО: 0/0", Colors.text_dark, Colors.bg_main)
end
local btnY = inputY + 2
...
if isPurchased then
drawButton(bx1, btnY, btnW1, 1, "[ Выдать набор ]", Colors.blue, Colors.white, "set_dispense")
else
drawButton(bx1, btnY, btnW1, 1, "[ Купить набор ]", Colors.success_green, Colors.white, "set_buy")
end
drawButton(bx2, btnY, btnW2, 1, "[ Назад ]", Colors.bg_button, Colors.white, "set_back")
local totalY = btnY + 2
if isPurchased then
local totalGot = 0
local totalNeeded = 0
if pr and pr.dispensed then for _, v in pairs(pr.dispensed) do totalGot = totalGot + math.floor(v) end end
for _, it in ipairs(set.items or {}) do totalNeeded = totalNeeded + (tonumber(it.qty) or 0) end
writeText(panelX + 2, totalY, "ВЫДАНО: " .. totalGot .. "/" .. totalNeeded, Colors.warning, Colors.bg_main)
else
local coinPrice, emaPrice = set.price_coin or 0, set.price_ema or 0
writeText(panelX + 2, totalY, "ИТОГО: ", Colors.text_bright, Colors.bg_main)
local totalX = panelX + 2 + unicode.len("ИТОГО: ")
...
writeText(totalX, totalY, " & ", Colors.text_bright, Colors.bg_main); totalX = totalX + 3
writeText(totalX, totalY, emaStr .. " EMA", Colors.tomato, Colors.bg_main)
elseif coinPrice > 0 then writeText(totalX, totalY, coinStr .. " COINS", Colors.accent_cyan, Colors.bg_main)
elseif emaPrice > 0 then writeText(totalX, totalY, emaStr .. " EMA", Colors.tomato, Colors.bg_main)
else writeText(totalX, totalY, "Бесплатно", Colors.text_bright, Colors.bg_main) end
end
end
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.21", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.21", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
writeDebugLog("Отмену крафта не удалось подтвердить")
return false
end
...
if job.phase == "crafting" or job.phase == "confirm" then
writeDebugLog("Игрок ушёл — крафт отменён в ME")
end
State.autocraftJob = nil
State.modalState.active = false; State.modalState.kind = nil; State.modalState.data = nil; State.modalData = nil
end
...
local pr = getSetProgress(set.id)
if pr and not isSetFullyDispensed(set) then openErrorModal("Ошибка", "Набор уже куплен, заберите содержимое"); return end
if not isSetAvailableInME(set) then openErrorModal("Недоступно", "В МЭ нет всех предметов набора"); return end
...
if not set then openErrorModal("Ошибка", "Набор не выбран"); return end
local pr = getSetProgress(set.id)
if not pr or isSetFullyDispensed(set) then openErrorModal("Ошибка", "Набор не куплен или уже выдан полностью"); return end
if not component.isAvailable("me_interface") then openErrorModal("Ошибка", "ME интерфейс недоступен"); return end
local me = component.me_interface
local txid = computer.address():sub(1, 8) .. "_" .. computer.uptime()
if not TransactionModule.lock(txid) then openErrorModal("Подожди", "Транзакция уже выполняется или слишком частые запросы."); return end
clearLog()
addLogMessage("Выдача предметов набора: " .. set.name, Colors.accent_cyan)
local inventoryFull = false
local meMissing = false
local totalGotThisRun = 0
local totalNeeded = 0
for _, item in ipairs(set.items or {}) do totalNeeded = totalNeeded + (tonumber(item.qty) or 0) end
for _, item in ipairs(set.items or {}) do
if inventoryFull or meMissing then break end
local key = getSetKey(item)
local needed = tonumber(item.qty) or 0
local got = math.floor(pr.dispensed[key] or 0)
local remaining = needed - got
if remaining > 0 then
local meQty = 0
local okItems, items = pcall(function() return me.getItemsInNetwork() end)
if okItems and type(items) == "table" then
for _, it in ipairs(items) do if it.name == item.internalName and (it.damage or 0) == (item.damage or 0) then meQty = meQty + (it.size or 0) end end
end
if meQty < remaining then meMissing = true; break end
local id = item.internalName
if not id:find(":", 1, true) then id = "minecraft:" .. id end
local fingerprint = { id = id, dmg = item.damage or 0 }
local toGive = remaining
while toGive > 0 do
...
local toTake = math.min(toGive, 64, freeSlots * 64)
local okExport, result = pcall(function() return me.exportItem(fingerprint, PULL_DIRECTION, toTake) end)
local got = 0
if okExport then
if type(result) == "number" then got = result
...
if got > 0 then
toGive = toGive - got
totalGotThisRun = totalGotThisRun + got
...
local isCompleted = isSetFullyDispensed(set)
pr.completed = isCompleted
CatalogModule.saveSetsProgress()
PendingModule.add("set_progress", { player = State.currentPlayer, setId = set.id, dispensed = pr.dispensed, completed = isCompleted })
TransactionModule.unlock()
if isCompleted then
...
addLogMessage("Инвентарь переполнен, Выдано " .. totalGotThisRun .. "/" .. totalNeeded, Colors.warning)
addLogMessage("Освободите место в инвентаре и нажмите повторно \"Выдать набор\"", Colors.warning)
elseif meMissing then
addLogMessage("⚠ В МЭ не хватает предметов. Выдано " .. totalGotThisRun .. "/" .. totalNeeded, Colors.warning)
end
State.needStockRefresh = true
markDirty("full")
end
...
writeDebugLog("Вход отменён: игрок " .. tostring(playerName) .. " уже не на PIM")
...
if State.setView == "contents" then State.selectedSetItem = (State.selectedSet.items or {})[1] else State.selectedSetItem = nil end
else State.selectedIndex = 0; State.selectedSet = nil; State.selectedSetItem = nil end
else
applyFilter()
if #Data.filteredItems > 0 then
...
writeDebugLog("Игрок ушёл во время выдержки — синхронизация не начиналась")
...
writeDebugLog("Игрок ушёл во время синхронизации/подтверждения — магазин не открывается")
State.welcomeLog = {}; drawWelcomeScreen()
return
end
...
if State.currentShopMode == "sets" then applySetsFilter()
else applyFilter(); if #Data.filteredItems > 0 then State.selectedIndex = 1; State.selectedItem = Data.filteredItems[1]; updateSelectorDisplay(State.selectedItem) else State.selectedIndex = 0; State.selectedItem = nil; updateSelectorDisplay(nil) end; if State.currentShopMode == "sell" then refreshSellPlayerQty() end end
markDirty("full")
...
clearLog()
addLogMessage("Для покупки набора нажмите \"Купить набор\" и освободите инвентарь для предметов.", Colors.text_gray)
markDirty("full")
end
...
local set = State.selectedSet
if set then
local item = (set.items or {})[idx]
if item then State.selectedIndex = idx; State.selectedSetItem = item; markDirty("full") end
end
end
return
end
...
applySetsFilter()
if #Data.setsDisplay > 0 then State.selectedIndex = 1; State.selectedSet = Data.setsDisplay[1] else State.selectedIndex = 0; State.selectedSet = nil end
updateSelectorDisplay(nil)
else
if mode == "buy" then updateBuyCatalogFromME(); State.lastStockRefresh = computer.uptime() end
applyFilter()
if #Data.filteredItems > 0 then
State.selectedIndex = 1; State.selectedItem = Data.filteredItems[1]
...
local pst = pimPresenceState()
if pst == "present" then State.presenceWorks = true
elseif pst == "absent" then State.presenceWorks = true; writeDebugLog("Watchdog: PIM пуст — закрываем магазин"); handlePlayerLeave() end
end
if e == "touch" then
local x, y = ev[3], ev[4]
...
if State.autocraftJob.phase == "confirm" then State.autocraftJob = nil; forceRender() else acCancel(true) end
...
if State.currentShopMode == "sets" then applySetsFilter()
else applyFilter(); State.selectedIndex = #Data.filteredItems > 0 and 1 or 0; State.selectedItem = Data.filteredItems[State.selectedIndex]; if State.currentShopMode == "sell" then refreshSellPlayerQty() end; updateSelectorDisplay(State.selectedItem) end
markDirty("full")
elseif ch == 8 then
State.searchInput = unicode.sub(State.searchInput or "", 1, -2)
if State.currentShopMode == "sets" then applySetsFilter()
else applyFilter(); State.selectedIndex = #Data.filteredItems > 0 and 1 or 0; State.selectedItem = Data.filteredItems[State.selectedIndex]; updateSelectorDisplay(State.selectedItem) end
markDirty("full")
elseif ch >= 32 then
...
local total
if State.currentShopMode == "sets" then
if State.setView == "list" then total = #Data.setsDisplay else total = #((State.selectedSet and State.selectedSet.items) or {}) end
else total = #Data.filteredItems end
...
if not State.catalogsLoaded then writeDebugLog("Игрок на PIM, но каталоги ещё не загружены. Игнорируем.")
elseif State.syncInProgress then writeDebugLog("Идёт обновление, событие входа пропущено")
else
local playerName = ev[2] or "Игрок"
playerName = playerName:match("^%s*(.-)%s*$") or playerName
if not playerName or playerName == "" then playerName = "Неизвестный" end
if State.pimActive and State.currentPlayer == playerName then writeDebugLog("Дубль события входа, игрок уже в магазине")
else
...
if State.currentPlayer then handlePlayerLeave() end
end
end
end
...
if string.find(lowerErr, "interrupted", 1, true) or string.find(lowerErr, "terminate", 1, true) then writeDebugLog("Завершение: " .. errText)
else
