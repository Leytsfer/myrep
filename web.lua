-- ============================================================
-- TRADE MARKET CLIENT v6.10.1 (SETS FIXES)
-- ============================================================
...
return pr.dispensed[tostring(item.internalName) .. ":" .. tostring(item.damage or 0)] or 0
end
...
setPurchasedNotDone = function(set)
local pr = set and Data.setsProgress[set.id] or nil
...
if not pr or not pr.dispensed then return 0 end
return pr.dispensed[setItemKey(it)] or 0
end
...
local st, stCol
-- Логика статусов: ГОТОВ, НЕТ В МЭ, ОЖИДАЕТ
-- Если есть прогресс и он не завершен (completed=false) -> ОЖИДАЕТ
-- Если прогресса нет (nil) или completed=true (сброшен) -> проверяем наличие в МЭ
local pr = setProgressGet(s.id)
local isWaiting = pr and (not pr.completed)
if isWaiting then
st, stCol = "ОЖИДАЕТ", Colors.warning
elseif setIsAvailable(s) then
st, stCol = "ГОТОВ", Colors.success_green
else
st, stCol = "НЕТ В МЭ", Colors.text_dark
end
...
-- Логика статусов для правой панели (покупка)
local isWaiting = pr and (not pr.completed)
if isWaiting then
stText, stColor = "ОЖИДАЕТ ВЫДАЧИ", Colors.warning
elseif setIsAvailable(set) then
stText, stColor = "ГОТОВ К ВЫДАЧЕ", Colors.success_green
else
stText, stColor = "НЕДОСТУПЕН", Colors.text_dark
end
...
dispTotal = math.floor(dispTotal) -- Округление
local dispColor = (dispTotal >= reqTotal and reqTotal > 0) and Colors.success_green or (dispTotal > 0 and Colors.warning or Colors.text_dark)
writeText(panelX + 18, inputY, "ВЫДАНО: " .. dispTotal .. "/" .. reqTotal, dispColor, Colors.bg_main)
local btnY = inputY + 2
local pr = setProgressOf(set)
...
local totalY = btnY + 2
-- Логика замены ИТОГО на ВЫДАНО
if bought then
-- Если куплен, показываем прогресс вместо цены
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.10.1", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.10.1", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
acLogJob(job, { { "✔ Выдано: ", Colors.success_green }, { math.floor(extracted) .. "шт из " .. job.ordered .. "шт", Colors.accent_cyan } })
if extracted >= job.ordered then
...
local freeSlots = 36 - PimModule.countOccupiedSlots()
if freeSlots <= 0 then
acLogJob(job, { { "⚠ Не хватило места в инвентаре: выдано только " .. math.floor(extracted) .. " шт.", Colors.warning } })
...
clearLog()
-- Лог 1: Выдача предметов набора
addLogMessage("Выдача предметов набора: " .. (set.name or ""), Colors.accent_main)
local inventoryFull = false
local meMissing = false
local totalGotThisRun = 0
local totalNeeded = setTotalQty(set)
for _, it in ipairs(set.items or {}) do
...
for _, it in ipairs(set.items or {}) do
local key = it.internalName .. ":" .. (it.damage or 0)
if (pr.dispensed[key] or 0) < (it.qty or 0) then isCompleted = false; break end
end
-- Если всё выдано, сбрасываем прогресс (Bug Fix #1)
if isCompleted then
pr.completed = true -- Помечаем как завершенный, чтобы триггернуть сброс в UI
-- Но для логики "бесконечной покупки" мы просто удаляем запись о прогрессе
-- Однако, нам нужно сначала отрисовать успех, а потом сбросить.
-- Или лучше: если completed, то pr.completed = true, а в UI мы проверяем pr.completed.
-- Если pr.completed == true, то в UI статус будет "ГОТОВ" (если есть в МЕ) или "НЕТ В МЕ".
-- А кнопка будет "Купить набор".
-- Значит, нам НЕ нужно удалять запись сразу, достаточно установить completed = true.
-- Но пользователь просил: "сбрасываться выдано: 0/0".
-- Значит, если completed == true, мы должны считать, что прогресса нет.
-- Давайте сделаем так: если completed, то в функциях setDispensedFor возвращаем 0.
-- Нет, лучше просто удалить запись из Data.setsProgress, тогда setProgressOf вернет nil.
-- Но нам нужно сохранить факт выдачи для лога? Нет, лог уже выведен.
-- Пользователь хочет: "Как только мы забрали все товары... статус должен обновиться".
-- Значит, если isCompleted, мы удаляем прогресс.
Data.setsProgress[set.id] = nil
CatalogModule.saveSetsProgress()
PendingModule.add("set_progress", { player = State.currentPlayer, setId = set.id, dispensed = {}, completed = true }) -- Отправляем сброс на сервер
addLogMessage("✅ Набор полностью выдан: " .. (set.name or ""), Colors.success_green)
else
-- Если не всё выдано, сохраняем прогресс
pr.completed = false
CatalogModule.saveSetsProgress()
PendingModule.add("set_progress", { player = State.currentPlayer, setId = set.id, dispensed = pr.dispensed, completed = false })
-- Лог 2 и 3: Инвентарь переполнен (Bug Fix #4)
if inventoryFull then
local currentTotal = setTotalDispensed(set) -- Это уже обновленный прогресс
addLogMessage("Инвентарь переполнен, Выдано " .. math.floor(currentTotal) .. "/" .. totalNeeded, Colors.warning)
addLogMessage("Освободите место в инвентаре и нажмите \"Выдать набор\"", Colors.warning)
elseif meMissing then
addLogMessage("⚠ В МЭ не хватает предметов.", Colors.warning)
end
end
TransactionModule.unlock()
State.needStockRefresh = true
