-- ============================================================
-- TRADE MARKET CLIENT v6.12.10 (AUTOCRAFT UPDATE)
-- ============================================================
...
local Config = {
WEB_URL = "https://trade-market.store/trade_market_shop/api.php",
WEB_TOKEN = "NiDNpFkWvEgcottpn71kNQntVgoWOgVdYwpHvQgVpWc4LTz1JqIir9JTF8V4XLJR8wTUIrwI70d2HAfjEVUhzQ==",
...
CATALOG_UPDATE_INTERVAL = 300, TIMEZONE_OFFSET = 3 * 3600,
PRESENCE_HOLD_DELAY = 1, PRESENCE_CONFIRM_DELAY = 1,
...
local CLIENT_VERSION = "6.12.10"   -- текущая версия ЭТОГО файла; при релизе меняй и её, и шапку
local SELF_PATH = "/home/web.lua"
local State = {
currentPlayer = nil, currentSession = nil, pimActive = false, pimOwner = nil,
pendingPlayer = nil, loadAborted = false,
...
lastPimCheck = 0, pimAbsentCount = 0, presenceWorks = false, pimUnknownCount = 0,
autocraftJob = nil, autocraftCraftablesDirty = false, acLastCmd = 0,
acCraftCache = {}, acProbeQueue = {},
acKnownRecipes = nil, acRecipesBuiltAt = 0, acDbRefreshAt = 0,
...
State.lastCatalogUpdate = State.lastCatalogUpdate or 0
State.acLastCmd = State.acLastCmd or 0
State.lastWelcomeAnim = State.lastWelcomeAnim or 0
State.bannedNow = State.bannedNow or false
State.helpPage = State.helpPage or 1
State.welcomeAnim = State.welcomeAnim or 0
...
State.hbCount = State.hbCount or 0
State.searchDirty = State.searchDirty or false
State.lastSearchApply = State.lastSearchApply or 0
State.clearLogTimer = State.clearLogTimer or nil
...
meSnap = { at = -1, index = {} }
-- ===== ЧИСТЫЙ LUA UTF-8 (без вызовов компонента unicode) =====
local function utf8Chars(s) return s:gmatch("[^\128-\191][\128-\191]*") end
local function utf8Len(s) local n = 0 for _ in s:gmatch("[^\128-\191][\128-\191]*") do n = n + 1 end return n end
local function utf8Sub(s, i, j)
local out, ci = {}, 0
if j == nil then j = -1 end
if j < 0 then j = utf8Len(s) + j + 1 end
if i < 1 then i = 1 end
for c in s:gmatch("[^\128-\191][\128-\191]*") do
ci = ci + 1
if ci > j then break end
if ci >= i then out[#out + 1] = c end
end
...
function cut(s, w)
s = tostring(s or "")
if utf8Len(s) > w then s = utf8Sub(s, 1, w - 1) .. "…" end
return s
end
...
local s = tostring(n)
local out = ""
while utf8Len(s) > 3 do
out = " " .. utf8Sub(s, -3) .. out
s = utf8Sub(s, 1, utf8Len(s) - 3)
end
return s .. out
end
...
for token in tostring(text or ""):gmatch("[^\128-\191]+") do
for word in token:gmatch("%S+") do
if cur == "" then cur = word
elseif utf8Len(cur) + 1 + utf8Len(word) <= width then cur = cur .. " " .. word
else pushCur(); cur = word end
while utf8Len(cur) > width do
lines[#lines + 1] = utf8Sub(cur, 1, width)
cur = utf8Sub(cur, width + 1)
end
end
pushCur()
end
...
function getRealIsoString() return os.date("%Y-%m-%d %H:%M:%S", getRealTimestamp()) end
local FileModule = {}
...
local selfUpdateBusy = false
local function performSelfUpdate()
if selfUpdateBusy then return false end
selfUpdateBusy = true
local url = Config.WEB_URL .. "?action=oc_get_update"
local headers = { ["X-OC-Token"] = Config.WEB_TOKEN }
local okReq, code = pcall(function()
local req = internet.request(url, nil, headers, "GET")
local chunks = {}
for chunk in req do chunks[#chunks + 1] = chunk end
return table.concat(chunks)
end)
if not okReq or not code or type(code) ~= "string" or #code < 500 then
writeDebugLog("⛔ Самообновление: сервер не отдал код (" .. tostring(code and #code or 0) .. " байт)")
selfUpdateBusy = false
return false
end
if code:sub(1, 5) == "ERROR" then
writeDebugLog("⛔ Самообновление: " .. code)
selfUpdateBusy = false
return false
end
local okCompile = pcall(function()
assert(load(code, "=selfupdate_check", "t", {}))
end)
if not okCompile then
writeDebugLog("⛔ Самообновление: новый код не компилируется, пропуск")
selfUpdateBusy = false
return false
end
local newVer = code:match("CLIENT v([%d%.]+)") or "?"
local tmp, bak = SELF_PATH .. ".upd", SELF_PATH .. ".bak"
local f = io.open(tmp, "w")
if not f then
writeDebugLog("⛔ Самообновление: не удалось создать временный файл")
selfUpdateBusy = false
return false
end
f:write(code); f:close()
pcall(function() if fs.exists(bak) then fs.remove(bak) end end)
pcall(function() if fs.exists(SELF_PATH) then fs.rename(SELF_PATH, bak) end end)
local okRen = fs.rename(tmp, SELF_PATH)
if not okRen then
pcall(function() if fs.exists(bak) then fs.rename(bak, SELF_PATH) end end)
writeDebugLog("⛔ Самообновление: не удалось заменить файл, откат")
selfUpdateBusy = false
return false
end
writeDebugLog("✅ Самообновление установлено: v" .. newVer .. ", перезагрузка")
os.sleep(0.5)
...
local Buffer = { currChars = {}, currFg = {}, currBg = {}, sentChars = {}, sentFg = {}, sentBg = {}, dirtyRows = {}, initialized = false, lastFg = nil, lastBg = nil, stats = { spans = 0, fg = 0, bg = 0 } }
...
Buffer.sentChars[y] = makeSpaceRow(); Buffer.sentFg[y], Buffer.sentBg[y] = {}, {}
for x = 1, Config.SCREEN_W do
Buffer.currFg[y][x], Buffer.currBg[y][x] = Colors.text_bright, Colors.bg_main
Buffer.sentFg[y][x], Buffer.sentBg[y][x] = Colors.text_bright, Colors.bg_main
end
end
Buffer.initialized = true
end
-- Дешёвый сброс кадра в пустой экран (без GPU-вызовов): diff-flush сам отправит дельту
function Buffer.clear()
Buffer.fill(1, 1, Config.SCREEN_W, Config.SCREEN_H, " ", Colors.text_bright, Colors.bg_main)
end
function Buffer.set(x, y, char, fg, bg)
if x < 1 or x > Config.SCREEN_W or y < 1 or y > Config.SCREEN_H then return end
fg = fg or Colors.text_bright; bg = bg or Colors.bg_main
if Buffer.currChars[y][x] == char and Buffer.currFg[y][x] == fg and Buffer.currBg[y][x] == bg then return end
Buffer.currChars[y][x] = char
Buffer.currFg[y][x], Buffer.currBg[y][x] = fg, bg
...
if x < 1 then text = utf8Sub(text, 2 - x); x = 1 end
local maxLen = Config.SCREEN_W - x + 1
if utf8Len(text) > maxLen then text = utf8Sub(text, 1, maxLen - 1) .. "…" end
local xx = x
for c in utf8Chars(text) do Buffer.set(xx, y, c, fg, bg); xx = xx + 1 end
end
-- DIFF-FLUSH: сравниваем ВЕСЬ кадр с тем, что реально на экране, шлём только изменившиеся span'ы
function Buffer.flush()
local lastFg, lastBg = Buffer.lastFg, Buffer.lastBg
for y = 1, Config.SCREEN_H do
...
local sRow, sFg, sBg = Buffer.sentChars[y], Buffer.sentFg[y], Buffer.sentBg[y]
local x = 1
while x <= Config.SCREEN_W do
if sRow[x] == row[x] and sFg[x] == cFg[x] and sBg[x] == cBg[x] then
x = x + 1
else
local fg, bg = cFg[x], cBg[x]
local startX = x
local seg = { row[x] }
sRow[x], sFg[x], sBg[x] = row[x], fg, bg
x = x + 1
while x <= Config.SCREEN_W do
if sRow[x] == row[x] and sFg[x] == cFg[x] and sBg[x] == cBg[x] then break end
if cFg[x] == fg and cBg[x] == bg then
seg[#seg + 1] = row[x]
sRow[x], sFg[x], sBg[x] = row[x], fg, bg
x = x + 1
else break end
end
if lastFg ~= fg then gpu.setForeground(fg); lastFg = fg; Buffer.stats.fg = Buffer.stats.fg + 1 end
if lastBg ~= bg then gpu.setBackground(bg); lastBg = bg; Buffer.stats.bg = Buffer.stats.bg + 1 end
gpu.set(startX, y, table.concat(seg))
Buffer.stats.spans = Buffer.stats.spans + 1
end
end
end
Buffer.lastFg, Buffer.lastBg = lastFg, lastBg
end
function shiftRowsCurr(y, h, shift)
local function shiftArr(arr)
local saved = {}
for i = 0, h - 1 do saved[i] = arr[y + i] end
for i = 0, h - 1 do
local src = i + shift
if src >= 0 and src < h then arr[y + i] = saved[src] end
end
end
shiftArr(Buffer.currChars); shiftArr(Buffer.currFg); shiftArr(Buffer.currBg)
for i = 0, h - 1 do Buffer.dirtyRows[y + i] = nil end
end
local PimModule = {}
...
-- «Сырое» имя инвентаря PIM: равно имени игрока, пока он привязан,
-- и idle-имени компонента (например "pim"), когда игрока нет.
function PimModule.getRawInventoryName()
local pimAddr = PimModule.getAddr()
if not pimAddr then return nil end
local okp, pim = pcall(component.proxy, pimAddr)
if not okp or not pim or not pim.getInventoryName then return nil end
local ok, r = pcall(function() return pim.getInventoryName() end)
if ok and type(r) == "string" and r ~= "" then return r end
return nil
end
-- Это idle-имя PIM (игрока нет): nil/пусто/"pim"/захваченное idle-имя
function PimModule.isIdleName(name)
if not name or name == "" then return true end
if name == "pim" then return true end
if State.pimIdleName and name == State.pimIdleName then return true end
return false
end
-- Игрок на PIM, но инвентарь «скрыт» открытой переносной ячейкой /
-- беспроводным терминалом: PIM отдаёт size=0 и пустые слоты, НО держит имя.
function pimInventoryHidden()
local addr = PimModule.getAddr()
if not addr then return false end
local okp, pim = pcall(component.proxy, addr)
if not okp or not pim or not pim.getInventorySize then return false end
local ok, r = pcall(function() return pim.getInventorySize() end)
if not (ok and type(r) == "number" and r == 0) then return false end
local inv = PimModule.getRawInventoryName()
return (inv ~= nil and not PimModule.isIdleName(inv))
end
function PimModule.getPlayer()
local pimAddr = PimModule.getAddr()
if not pimAddr then return nil end
local okp, pim = pcall(component.proxy, pimAddr)
if not okp or not pim then return nil end
local player = nil
-- Методы OC-компонентов вызываются БЕЗ self (точка, без аргументов)
if pim.getPlayer then local ok, res = pcall(function() return pim.getPlayer() end); if ok and type(res) == "string" and res ~= "" then player = res end end
if not player and pim.getPlayerName then local ok, res = pcall(function() return pim.getPlayerName() end); if ok and type(res) == "string" and res ~= "" then player = res end end
if not player and pim.getUsername then local ok, res = pcall(function() return pim.getUsername() end); if ok and type(res) == "string" and res ~= "" then player = res end end
-- Основной источник для этого PIM: имя инвентаря = имя игрока (минус idle-имя)
if not player then
local inv = PimModule.getRawInventoryName()
if inv and not PimModule.isIdleName(inv) then player = inv end
end
if not player then local ok, res = pcall(function() return pim.player end); if ok and type(res) == "string" and res ~= "" then player = res end end
return player
end
...
local nm = PimModule.getPlayer()
if nm and nm ~= "" then return "present" end
local okp, pim = pcall(component.proxy, addr)
if not okp or not pim or not pim.getInventorySize then return "unknown" end
local ok, r = pcall(function() return pim.getInventorySize() end)
if ok and type(r) == "number" then
if r > 0 then return "present" end
-- Инвентарь пуст: если PIM всё ещё держит не-idle имя —
-- это открытая переноска/терминал, а НЕ уход игрока
local inv = PimModule.getRawInventoryName()
if inv and not PimModule.isIdleName(inv) then return "present" end
return "absent"
end
return "unknown"
end
local function confirmedAbsent()
if pimPresenceState() ~= "absent" then return false end
os.sleep(0.25)
if pimPresenceState() ~= "absent" then return false end
os.sleep(0.25)
return pimPresenceState() == "absent"
end
local function loadStagePlayerGone(playerName)
if not playerName then return false end
local pst = pimPresenceState()
if pst == "absent" then return true end
if pst == "present" then
local nm = PimModule.getPlayer()
if nm and not PimModule.namesMatch(nm, playerName) then return true end
end
return false
end
...
local scanCache = {}
local occupiedCache = { at = -1, val = 0 }
function PimModule.invalidateInvCaches()
occupiedCache.at = -1
scanCache = {}
end
...
local key = targetName .. ":" .. targetDamage
local c = scanCache[key]
if c and computer.uptime() - c.at < 1.5 then return c.val end
local total = 0
for slot = 1, 36 do
...
scanCache[key] = { at = computer.uptime(), val = total }
return total
end
function PimModule.countOccupiedSlots()
local pimAddr = PimModule.getAddr()
if not pimAddr then return 0 end
if computer.uptime() - occupiedCache.at < 1.5 then return occupiedCache.val end
local occupied = 0
for slot = 1, 36 do local st = readStack(pimAddr, slot); if st and st.size > 0 then occupied = occupied + 1 end end
occupiedCache.at, occupiedCache.val = computer.uptime(), occupied
return occupied
end
...
if pimInventoryHidden() then return end   -- инвентарь скрыт GUI: количество не сбрасываем
...
if pimInventoryHidden() then return false, "Закройте переносную ячейку или беспроводной терминал" end
...
total = (State.setView == "list") and #Data.setsDisplay or #((State.selectedSet and State.selectedSet.items) or {})
else
total = #Data.filteredItems
end
local maxScroll = math.max(0, total - State.visibleRows)
State.listScroll = math.min(math.max(State.listScroll, 1), maxScroll + 1)
end
...
local nameLower = item.nameLower or lowerStr(item.displayName or item.internalName or "")
...
if not keepScroll then State.listScroll = 1 end
...
if keepScroll then clampScroll() end
end
...
table.insert(Data.buyCatalogDisplay, { internalName = item.internalName, displayName = item.displayName or item.internalName, nameLower = lowerStr(item.displayName or item.internalName or ""), priceCoin = tonumber(item.priceCoin) or 0, priceEma = tonumber(item.priceEma) or 0, damage = tonumber(item.damage) or 0, article = item.article or "", qty = 0 })
end
end
writeDebugLog("Каталог покупок: " .. #Data.buyCatalogDisplay .. " предметов")
local keepView = (State.pimActive and State.currentScreen == "shop")
applyFilter(keepView); restoreSelectedItemByKey(selectedKey)
...
local keepView = (State.pimActive and State.currentScreen == "shop")
applyFilter(keepView); restoreSelectedItemByKey(selectedKey)
markDirty("full"); writeDebugLog("ME обновлено")
end
end
...
local nl = s.nameLower or lowerStr(s.name or ""); s.nameLower = nl
if matchesSearch(nl, searchLower) then table.insert(Data.setsDisplay, s) end
end
end
...
if State.pendingPlayer and loadStagePlayerGone(State.pendingPlayer) then State.loadAborted = true; return nil end
...
while page <= totalPages do
if State.pendingPlayer and loadStagePlayerGone(State.pendingPlayer) then State.loadAborted = true; return nil, 0 end
...
if #State.welcomeLog > 6 then table.remove(State.welcomeLog, 1) end
drawWelcomeScreen()
end
...
local function stageGone()
if source == "player" and State.pendingPlayer and loadStagePlayerGone(State.pendingPlayer) then
State.loadAborted = true
return true
end
return false
end
...
if stageGone() then return syncEnd("aborted", source) end
...
if needUsers then
if stageGone() then return syncEnd("aborted", source) end
addWelcomeLog("Обновляю данные игроков (балансы)...", Colors.accent_cyan)
...
-- Если репортов нет — выходим, не делая лишних запросов
if not result or result.status == "ok" or not result.reports or #result.reports == 0 then
return
end
...
function scheduleClearLog(delay)
if State.clearLogTimer then
pcall(event.cancel, State.clearLogTimer)
State.clearLogTimer = nil
end
State.clearLogTimer = event.timer(delay or 5, function()
State.clearLogTimer = nil
clearLog()
markDirty("right")
return false
...
function drawCenteredText(y, text, fg, bg, offsetX) text = tostring(text or ""); Buffer.write(math.floor((Config.SCREEN_W - utf8Len(text)) / 2) + 1 + (offsetX or 0), y, text, fg or Colors.text_bright, bg or Colors.bg_main) end
...
writeText(x + math.floor((w - utf8Len(text)) / 2), y + math.floor((h - 1) / 2), text, fg or Colors.text_bright, bg or Colors.bg_button)
...
local function segLine(y, segs)
local total = 0
for _, s in ipairs(segs) do
if type(s) == "table" and type(s[1]) == "string" then total = total + utf8Len(s[1]) end
end
local xx = modalX + math.floor((modalW - total) / 2) + 1
for _, s in ipairs(segs) do
if type(s) == "table" and type(s[1]) == "string" then
writeText(xx, y, s[1], s[2], Colors.bg_modal)
xx = xx + utf8Len(s[1])
end
end
end
...
local totalW = utf8Len(icon) + utf8Len(head)
local sx = math.floor((Config.SCREEN_W - totalW) / 2) + 1
writeText(sx, modalY + 1, icon, Colors.success_green, Colors.bg_modal)
writeText(sx + utf8Len(icon), modalY + 1, head, Colors.accent_secondary, Colors.bg_modal)
...
local rw = 0
for _, s in ipairs(segs) do rw = rw + utf8Len(s.text) end
local cx = math.floor((Config.SCREEN_W - rw) / 2) + 1
for _, s in ipairs(segs) do writeText(cx, modalY + 4, s.text, s.color, Colors.bg_modal); cx = cx + utf8Len(s.text) end
...
local function cLine(y, txt, color)
local xx = modalX + math.floor((modalW - utf8Len(txt)) / 2) + 1
...
if coin > 0 then local s1 = formatPrice(coin) .. " COINS"; writeText(rx, ry, s1, Colors.accent_cyan, Colors.bg_modal); rx = rx + utf8Len(s1)
...
function categoryBtnX()
local clearBtnX = UI.header.searchX + UI.header.searchW + 1
return clearBtnX + UI.header.clearBtnW + 1
end
function drawHeader()
...
writeText(allBtnX + 2 + utf8Len(allBtnText), 2, "]", Colors.white, State.categoryDropdownOpen and Colors.dropdown_active or Colors.bg_button)
...
local tradeMarketX = panelXh + math.floor((panelWh - utf8Len(caption)) / 2)
...
function drawListRow(idx, y)
local listX = UI.list.x
local listW = math.floor(Config.SCREEN_W * UI.list.wRatio)
...
if utf8Len(name) > 38 then name = utf8Sub(name, 1, 37) .. "…" end
...
Buffer.fill(listX + 1, y, listW - 2, 1, " ", Colors.text_bright, Colors.bg_main)
end
end
...
local dashStart = listX + 2 + utf8Len(title) + 1
...
local dashEnd = listX + listW - 2
if dashEnd > dashStart then for x = dashStart, dashEnd do Buffer.set(x, listY + 1, "-", Colors.line, Colors.bg_main) end end
if s then writeText(listX + listW - 2 - utf8Len("[" .. (s.name or "") .. "]"), listY + 1, "[" .. (s.name or "") .. "]", Colors.accent_cyan, Colors.bg_main) end
local colY = listY + 2
Buffer.fill(listX + 1, colY, listW - 2, 1, " ", Colors.text_bright, Colors.bg_panel)
...
if utf8Len(name) > 34 then name = utf8Sub(name, 1, 33) .. "…" end
writeText(listX + 4, y, name, isSelected and Colors.white or Colors.text_bright, bg)
...
if dashEnd > dashStart then for x = dashStart, dashEnd do Buffer.set(x, listY + 1, "-", Colors.line, Colors.bg_main) end end
if State.currentShopMode == "buy" then
if State.currentCategory ~= "ВСЕ" then writeText(listX + listW - 2 - utf8Len("[" .. State.currentCategory .. "]"), listY + 1, "[" .. State.currentCategory .. "]", Colors.accent_cyan, Colors.bg_main)
else writeText(listX + listW - 2 - utf8Len("ВСЕ МОДЫ"), listY + 1, "ВСЕ МОДЫ", Colors.text_dark, Colors.bg_main) end
end
local colY = listY + 2
Buffer.fill(listX + 1, colY, listW - 2, 1, " ", Colors.text_bright, Colors.bg_panel)
writeText(listX + 2, colY, "НАЗВАНИЕ", Colors.text_bright, Colors.bg_panel)
...
State.listScroll = math.max(1, math.min(State.listScroll, maxScroll + 1))
for row = 1, State.visibleRows do
drawListRow(State.listScroll + row - 1, listY + 3 + row)
end
end
...
local ds = panelX + 2 + utf8Len("ЛОГИРОВАНИЕ") + 1
...
if e.segs then
local w = 0
for _, s in ipairs(e.segs) do w = w + utf8Len(s[1]) end
local xx = panelX + math.floor((panelW - w) / 2)
for _, s in ipairs(e.segs) do writeText(xx, y, s[1], s[2], Colors.bg_main); xx = xx + utf8Len(s[1]) end
else
local msg = tostring(e.text or "")
writeText(panelX + math.floor((panelW - utf8Len(msg)) / 2), y, msg, e.color or Colors.text_bright, Colors.bg_main)
end
...
local ds = panelX + 2 + utf8Len(title) + 1
local de = panelX + panelW - 2
...
local maxW = 0
for _, s in ipairs(segsList) do local w = 0; for _, seg in ipairs(s) do w = w + utf8Len(seg[1]) end; if w > maxW then maxW = w end end
local startX = panelX + math.floor((panelW - maxW) / 2)
local y = panelY + 4
for _, s in ipairs(segsList) do
local xx = startX
for _, seg in ipairs(s) do writeText(xx, y, seg[1], seg[2], Colors.bg_main); xx = xx + utf8Len(seg[1]) end
y = y + 2
end
...
writeText(panelX + math.floor((panelW - utf8Len(warn)) / 2), panelY + topH - 2, warn, Colors.error_red, Colors.bg_main)
end
end
...
local priceX = panelX + 2 + utf8Len("ЦЕНА: ")
local coinStr, emaStr = formatPrice(coinPrice), formatPrice(emaPrice)
if coinPrice > 0 and emaPrice > 0 then
writeText(priceX, priceY, coinStr .. " COINS", Colors.accent_cyan, Colors.bg_main); priceX = priceX + utf8Len(coinStr .. " COINS")
...
local qds = panelX + 2 + utf8Len("КОЛИЧЕСТВО") + 1
...
writeText(buyBtnX, inputY, useCraft and " [ ЗАКАЗАТЬ КРАФТ ]" or " [КУПИТЬ]", Colors.white, btnBg)
...
local totalX = panelX + 2 + utf8Len("Итого: ")
local totalCoinStr, totalEmaStr = formatPrice(totalCoin), formatPrice(totalEma)
if totalCoin > 0 and totalEma > 0 then
writeText(totalX, totalY, totalCoinStr .. " COINS", Colors.accent_cyan, Colors.bg_main); totalX = totalX + utf8Len(totalCoinStr .. " COINS")
...
local coinStr, emaStr = formatPrice(coinPrice), formatPrice(emaPrice)
if coinPrice > 0 and emaPrice > 0 then
writeText(priceX, baseY + lineSpacing * 2, coinStr .. " COINS", Colors.accent_cyan, Colors.bg_main); priceX = priceX + utf8Len(coinStr .. " COINS")
...
writeText(panelX + 2 + utf8Len("Имя: "), accY + 3, (State.currentPlayer or "Неизвестно"), Colors.accent_cyan, Colors.bg_panel)
writeText(panelX + 2, accY + 5, "Баланс: ", Colors.text_main, Colors.bg_panel)
local balX = panelX + 2 + utf8Len("Баланс: ")
writeText(balX, accY + 5, string.format("%.3f", State.currentSession.balance or 0) .. " COINS", Colors.accent_cyan, Colors.bg_panel)
balX = balX + utf8Len(string.format("%.3f", State.currentSession.balance or 0) .. " COINS")
...
local qBtnX = panelX + 2 + utf8Len(discTxt) + 1
...
writeText(panelX + 2 + utf8Len("Транзакции: "), accY + 9, tostring(State.currentSession.transactions or 0), Colors.accent_cyan, Colors.bg_panel)
...
writeText(Config.SCREEN_W - 30, Config.SCREEN_H, "Coded By Leytsfer v 6.12.10", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
local out = ""
for i = 1, utf8Len(t) do
local c = utf8Sub(t, i, i)
out = out .. c .. c
end
return out
end
...
"111111111111111111111111111111111121405888855777777800066666044111111111111111111111111111111111111",
...
Buffer.fill(1, 1, Config.SCREEN_W, Config.SCREEN_H, " ", Colors.text_bright, BG)
...
drawCenteredText(AUTHOR_Y, "Trade Shop by Leytsfer — v 6.12.10", Colors.text_dark, BG)
Buffer.flush()
State.screenInitialized = true
end
local RIGHT_BTN_IDS = { max_qty=true, buy_item=true, order_craft=true, autocraft_start=true, autocraft_close=true, sell_item=true, set_open=true, set_back=true, set_buy=true, set_dispense=true, discount_info=true }
local HEADER_BTN_IDS = { clear_search=true, toggle_category=true }
local BOTTOM_BTN_IDS = { help_open=true, page_buy=true, page_sell=true, page_quests=true, report_bug=true }
local function pruneButtons(ids)
local out = {}
for _, b in ipairs(State.buttons) do if not ids[b.id] then out[#out+1] = b end end
State.buttons = out
end
function renderScope(scope)
if State.currentScreen == "welcome" then drawWelcomeScreen(); return end
if scope == "right" then
pruneButtons(RIGHT_BTN_IDS); drawRightPanel()
elseif scope == "header" then
pruneButtons(HEADER_BTN_IDS); drawHeader()
elseif scope == "list" then
drawItemList()
elseif scope == "bottom" then
pruneButtons(BOTTOM_BTN_IDS); drawBottomBar()
else
State.buttons = {}
Buffer.clear()
drawHeader(); drawItemList(); drawRightPanel(); drawBottomBar()
...
State.guiDirty = true
if part ~= "header" and part ~= "list" and part ~= "right" and part ~= "bottom" then part = "full" end
State.pendingParts = State.pendingParts or {}
if part == "full" then State.pendingParts = { full = true } else State.pendingParts[part] = true end
if not State.renderTimer then
State.renderTimer = event.timer(0.05, function()
State.renderTimer = nil
if State.guiDirty then
local parts = State.pendingParts or { full = true }
State.guiDirty = false; State.pendingParts = { full = true }
if parts.full then renderScope("full")
else
if parts.header then renderScope("header") end
if parts.list then renderScope("list") end
if parts.right then renderScope("right") end
if parts.bottom then renderScope("bottom") end
end
end
return false
end)
...
if State.currentScreen == "welcome" then drawWelcomeScreen() else renderScope("full") end
end
...
if r.runBench then
writeDebugLog("🧪 Получена команда бенчмарка с сервера — запускаю")
runSelfBench()
return
end
if r.clientVersion and r.clientVersion ~= CLIENT_VERSION then
if not State.pimActive and not State.syncInProgress and not State.TRANSACTION_LOCK then
writeDebugLog("🔄 Доступна новая версия: " .. tostring(r.clientVersion) .. " (текущая " .. CLIENT_VERSION .. ")")
performSelfUpdate()
end
return
end
...
if State.pimActive and State.currentPlayer and State.hbCount % 3 == 0 then
...
-- ===== ВСТРОЕННЫЙ БЕНЧМАРК (запуск: touch /home/bench.flag в шелле) =====
function runSelfBench()
local lines = {}
local function add(s) lines[#lines + 1] = s; writeDebugLog("BENCH: " .. s) end
local function timeIt(label, fn, reps)
pcall(fn, 0)
local t0 = computer.uptime()
for i = 1, reps do pcall(fn, i) end
local dt = computer.uptime() - t0
add(string.format("%-30s %3d reps = %7.3fs (%6.2f ms/call)", label, reps, dt, dt / reps * 1000))
end
add("=== SHOP SELF-BENCH " .. CLIENT_VERSION .. " uptime=" .. string.format("%.0f", computer.uptime()) .. " ===")
timeIt("gpu.set", function(i) gpu.set(1 + (i % 100), 1, "x") end, 50)
timeIt("gpu.setForeground", function(i) gpu.setForeground(i % 2 == 0 and 0xFFFFFF or 0x00FF00) end, 50)
timeIt("gpu.setBackground", function(i) gpu.setBackground(i % 2 == 0 and 0x000000 or 0x222222) end, 50)
gpu.setForeground(0xFFFFFF); gpu.setBackground(0x000000)
if type(utf8Len) == "function" then
timeIt("utf8Len(40 символов)", function() utf8Len("Тестовая строка для замера длины юникод текста") end, 100)
end
if type(utf8Sub) == "function" then
timeIt("utf8Sub(1,20)", function() utf8Sub("Тестовая строка для замера зареза юникод текста", 1, 20) end, 100)
end
timeIt("unicode.len(40)", function() unicode.len("Тестовая строка для замера длины юникод текста") end, 50)
timeIt("unicode.sub(1,20)", function() unicode.sub("Тестовая строка для замера зареза юникод текста", 1, 20) end, 50)
timeIt("applyFilter", function() applyFilter(true) end, 3)
timeIt("forceRender (full)", function() forceRender() end, 3)
if type(renderScope) == "function" then
timeIt("renderScope(right)", function() renderScope("right") end, 3)
timeIt("renderScope(list)", function() renderScope("list") end, 3)
end
if PimModule and PimModule.invalidateInvCaches then
timeIt("countOccupiedSlots (cold)", function() PimModule.invalidateInvCaches(); PimModule.countOccupiedSlots() end, 1)
end
timeIt("meSnapshot (fresh ME scan)", function() meSnap.at = -1; meSnapshot() end, 1)
add("=== END ===")
Buffer.stats.spans, Buffer.stats.fg, Buffer.stats.bg = 0, 0, 0
renderScope("right")
add(string.format("NO-OP redraw right: spans=%d fg=%d bg=%d", Buffer.stats.spans, Buffer.stats.fg, Buffer.stats.bg))
Buffer.stats.spans, Buffer.stats.fg, Buffer.stats.bg = 0, 0, 0
forceRender()
add(string.format("FULL frame: spans=%d fg=%d bg=%d", Buffer.stats.spans, Buffer.stats.fg, Buffer.stats.bg))
timeIt("renderScope(right) повтор (no-op)", function() renderScope("right") end, 2)
local content = table.concat(lines, "\n")
pcall(function()
local f = io.open("/home/bench_shop.log", "w")
if f then f:write(content .. "\n"); f:close() end
end)
HttpModule.request("oc_debug_upload", { name = "bench_shop_" .. tostring(Config.TERMINAL_ID or computer.address():sub(1, 8)), content = content })
forceRender()
end
...
ok, r = pcall(function() return m() end)
if ok then return r end
return nil
end
...
-- ===== БАЗА РЕЦЕПТОВ: ЛОКАЛЬНЫЙ КЭШ ВСЕХ РЕЦЕПТОВ МЭ =====
local function acRecipesCount()
local n = 0
if State.acKnownRecipes then for _ in pairs(State.acKnownRecipes) do n = n + 1 end end
return n
end
local function acRecipesLoad()
local data = FileModule.loadLuaFile(Config.FILES.acRecipes)
if type(data) == "table" and type(data.recipes) == "table" then
State.acKnownRecipes = data.recipes
State.acRecipesBuiltAt = tonumber(data.builtAt) or 0
State.acDbRefreshAt = computer.uptime()
writeDebugLog("📦 База рецептов загружена из файла: " .. tostring(acRecipesCount()) .. " рецептов")
return true
end
return false
end
local function acRecipesSave()
if not State.acKnownRecipes then return false end
local ok = FileModule.saveLuaFile(Config.FILES.acRecipes, { builtAt = os.time(), recipes = State.acKnownRecipes })
if ok then writeDebugLog("💾 База рецептов сохранена: " .. tostring(acRecipesCount()) .. " рецептов") end
return ok
end
local function acRecipesBuild()
...
if st then
local name = st.name or st.label or st.id
if name then
local dmg = tonumber(st.damage) or 0
local key = acKey(name, dmg)
local size = tonumber(st.size) or 1
if not recipes[key] or (recipes[key].size or 0) < size then
recipes[key] = { name = name, damage = dmg, size = size }
end
end
end
end
State.acKnownRecipes = recipes
State.acRecipesBuiltAt = os.time()
State.acDbRefreshAt = computer.uptime()
acRecipesSave()
writeDebugLog("📦 База рецептов построена: " .. tostring(acRecipesCount()) .. " рецептов")
return true
end
-- ===== СНИМОК КРАФТ-CPU (GTNH: используем ТОЛЬКО storage, он стабилен по типу) =====
function acCpusSnapshot()
local snap = { occupied = 0, totalStorage = 0, ok = false }
if not component.isAvailable("me_interface") then return snap end
local me = component.me_interface
local okList, list = pcall(function() return me.getCraftingCPUs() end)
if not okList or type(list) == "table" then snap.ok = true end
if okList and type(list) == "table" then
for _, cpu in ipairs(list) do
if type(cpu) == "table" then
local st = tonumber(cpu.storage) or 0
snap.totalStorage = snap.totalStorage + st
if st > 256 then snap.occupied = snap.occupied + 1 end
end
end
end
return snap
end
function acCpusGrew(base, now)
if not base or not base.ok or not now or not now.ok then return false end
if now.occupied > base.occupied then return true end
if now.totalStorage > base.totalStorage + 4096 then return true end
return false
end
-- Жив ли request-прокси: отвечает ли хоть один его статус-метод
local function reqProxyAlive(req)
if type(req) ~= "table" then return false end
if callMethod(req, "isDone") ~= nil then return true end
if callMethod(req, "isCanceled") ~= nil then return true end
if callMethod(req, "getStatus") ~= nil then return true end
if callMethod(req, "hasFinished") ~= nil then return true end
return false
end
-- Формы вызова запроса крафта: colon → dot → me.requestCrafting (last resort)
local AC_REQ_FORMS = {
function(entry, me, amount) return entry.raw:request(amount) end,
function(entry, me, amount) return entry.raw.request(amount) end,
function(entry, me, amount) return me.requestCrafting(entry.raw, amount) end,
}
function acRequestCraft(entry, amount, startForm)
if not entry or not entry.raw then return nil, nil end
local me = component.me_interface
if not me then return nil, nil end
for i = (startForm or 1), #AC_REQ_FORMS do
local ok, rr = pcall(AC_REQ_FORMS[i], entry, me, amount)
if ok and rr then return rr, i end
end
return nil, nil
end
-- Достаём «сырой» паттерн крафта только когда игрок реально заказывает крафт
local function acEnsureRaw(item)
if not item then return nil end
local k = acKey(item.internalName, item.damage or 0)
local entry = State.acCraftCache[k]
if type(entry) == "table" and entry.raw then return entry end
if not component.isAvailable("me_interface") then return nil end
local me = component.me_interface
local full = acFullName(item.internalName)
local ok, list = pcall(function() return me.getCraftables({ name = full }) end)
...
local st = acStackOf(pattern)
local e2 = { name = (st and (st.name or st.label or st.id)) or full, damage = item.damage or 0, size = (st and tonumber(st.size)) or 1, raw = pattern, canCraft = true }
State.acCraftCache[k] = e2
State.autocraftCraftablesDirty = true
return e2
end
-- Свежий (не кэшированный) паттерн крафта — для повторной попытки
function acEnsureRawFresh(item)
if not item then return nil end
if not component.isAvailable("me_interface") then return nil end
local me = component.me_interface
local full = acFullName(item.internalName)
...
local pattern = list[1]
local st = acStackOf(pattern)
local e = { name = (st and (st.name or st.label or st.id)) or full, damage = item.damage or 0, size = (st and tonumber(st.size)) or 1, raw = pattern, canCraft = true }
State.acCraftCache[acKey(item.internalName, item.damage or 0)] = e
State.autocraftCraftablesDirty = true
return e
end
-- Реально ли МЭ крафтит этот предмет прямо сейчас
function acCraftActuallyRunning(job)
if not component.isAvailable("me_interface") then return nil end
local me = component.me_interface
local ok, r = pcall(function() return me.isCrafting({ name = acFullName(job.internalName), dmg = job.damage or 0 }) end)
if not ok or r == nil then
ok, r = pcall(function() return me.isCrafting() end)
end
...
acDoProbe = function(item) end
acProbeItem = function(item) end
acAvailable = function(item)
if not item then return false end
local k = acKey(item.internalName, item.damage or 0)
if type(State.acCraftCache[k]) == "table" then return true end
if State.acKnownRecipes then return State.acKnownRecipes[k] ~= nil end
return false
end
...
local k = acKey(item.internalName, item.damage or 0)
local v = State.acCraftCache[k]
if type(v) == "table" then return "yes"
elseif v == false then return "no" end
if State.acKnownRecipes then return (State.acKnownRecipes[k] ~= nil) and "yes" or "no" end
return "unknown"
end
...
local idn = callMethod(req, "isDone")
if idn == true then return "done" end
local ic = callMethod(req, "isCanceled")
...
local me = component.me_interface
if not me then
acLogJob(job, { { "Ошибка выдачи: ME интерфейс недоступен", Colors.error_red } })
job.phase = "partial"
acFinishAndClose(job, false)
return
end
...
local fingerprint = { id = id, dmg = job.damage or 0 }
local remaining, extracted = job.ordered, 0
local failStreak = 0
while remaining > 0 and failStreak < 5 do
...
elseif type(result) == "table" then got = result.count or result.amount or result.size or 0 end
else
writeDebugLog("⚠️ exportItem ошибка: " .. tostring(result))
end
if got > 0 then
...
writeDebugLog("⚠️ Выдача: попытка " .. failStreak .. " без результата (осталось " .. remaining .. ")")
if failStreak < 5 then os.sleep(0.5); meSnap.at = -1 end
end
end
job.dispensed = extracted
acDecrementSnapshot(job.internalName, job.damage, extracted)
meSnap.at = -1
-- Оплата ТОЛЬКО за фактически выданное (без предоплаты и возвратов)
local netCoin = (job.unitCoin or 0) * extracted
local netEma = (job.unitEma or 0) * extracted
if netCoin > 0 or netEma > 0 then
PlayerModule.updateBalance(State.currentPlayer, -netCoin, -netEma)
...
refreshSessionDiscount()
PendingModule.add("transaction", { player = State.currentPlayer, type = "buy", item = job.internalName, displayName = job.displayName, qty = extracted, totalCoin = netCoin, totalEma = netEma, timestamp = math.floor(getRealTimestamp()), dateIso = getRealIsoString() })
local p = PlayerModule.get(State.currentPlayer)
...
job.phase = "done"
elseif extracted == 0 and stockBefore < job.ordered then
acLogJob(job, { { "Результат крафта: ", Colors.text_bright }, { "Неудача (не хватает ресурсов в МЭ)", Colors.error_red } })
acLogJob(job, { { "Попробуйте заказать меньше предметов", Colors.warning } })
job.phase = "partial"
else
local freeSlots = 36 - PimModule.countOccupiedSlots()
if extracted == 0 and freeSlots > 0 then
acLogJob(job, { { "⚠ МЭ не отдала предмет после крафта. Попробуйте заказать ещё раз.", Colors.error_red } })
...
acLogJob(job, { { "⚠ Выдано частично: " .. extracted .. " из " .. job.ordered .. ". Списано только за выданное.", Colors.warning } })
end
job.phase = "partial"
end
acFinishAndClose(job, extracted > 0)
end
...
State.logMessages = {}
-- Товар УЖЕ в МЭ: крафт не заказываем, сразу выдача
if job.toCraft <= 0 then
acLogJob(job, { { "Товар уже в наличии в МЭ: ", Colors.text_bright }, { inStock .. "шт", Colors.accent_cyan } })
acLogJob(job, { { "Крафт не требуется — начинаю выдачу", Colors.success_green } })
job.req = nil
job.phase = "crafting"
...
local entry = acEnsureRaw({ internalName = job.internalName, damage = job.damage })
if not entry then
acLogJob(job, { { "Результат крафта: ", Colors.text_bright }, { "Неудача (нет шаблона)", Colors.error_red } })
acFinishAndClose(job, false)
return
end
-- Базовый снимок CPU ДО запроса (косвенная верификация старта)
job.cpuBase = acCpusSnapshot()
job.cpuGrew = false
job.proxyAlive = false
local req, form = acRequestCraft(entry, job.toCraft, 1)
if not req then
...
job.startUptime = computer.uptime(); job.elapsed = 0; job.lastStockTick = 0; job.lastReportU = 0
job.verified = false
job.verifyUntil = computer.uptime() + 12
job.retried = false
job.baseStock = inStock
...
-- ===== Верификация старта: фейлим ТОЛЬКО по позитивным признакам мёртвого запроса =====
if not job.verified then
if now - (job.lastVerifyAt or 0) >= 1.5 then
job.lastVerifyAt = now
meSnap.at = -1
local stockNow = meCountOf(job.internalName, job.damage)
if stockNow > (job.baseStock or 0) or stockNow >= job.ordered then
job.verified = true
acLogJob(job, { { "Старт крафта подтверждён (предмет появился в МЭ)", Colors.success_green } })
else
local snapNow = acCpusSnapshot()
if acCpusGrew(job.cpuBase, snapNow) then
job.cpuGrew = true
job.verified = true
acLogJob(job, { { "Старт крафта подтверждён МЭ (CPU занят)", Colors.success_green } })
writeDebugLog("✅ CPU-верификация: occupied " .. tostring(job.cpuBase and job.cpuBase.occupied) .. " → " .. tostring(snapNow.occupied) .. ", storage " .. tostring(job.cpuBase and job.cpuBase.totalStorage) .. " → " .. tostring(snapNow.totalStorage))
else
job.proxyAlive = reqProxyAlive(job.req)
local st = craftStatus(job.req)
local deadEvidence = (st == "canceled" or st == "missing" or st == "error")
or (st == "done" and stockNow < job.ordered)
if deadEvidence then
writeDebugLog("⛔ Верификация: позитивный признак мёртвого запроса, status=" .. tostring(st))
end
if deadEvidence or now >= (job.verifyUntil or 0) then
if not deadEvidence and (job.proxyAlive or job.cpuGrew) then
job.verified = true
writeDebugLog("ℹ️ Верификация: нет прямых признаков, но прокси жив — доверяем запросу")
elseif not job.retried then
job.retried = true
cancelCraftReq(job.req)
local entry2 = acEnsureRawFresh({ internalName = job.internalName, damage = job.damage })
local req2, form2 = acRequestCraft(entry2, job.toCraft, math.min((job.reqForm or 1) + 1, #AC_REQ_FORMS))
if req2 then
job.req = req2
job.reqForm = form2
job.verifyUntil = now + 10
job.cpuBase = snapNow
job.proxyAlive = false
acLogJob(job, { { "МЭ не подтвердила старт крафта — повторный запрос", Colors.warning } })
else
acLogJob(job, { { "Результат крафта: ", Colors.text_bright }, { "Неудача (ME не стартовала крафт)", Colors.error_red } })
acFinishAndClose(job, false)
return
end
else
...
-- ===== Проверка наличия: свежий снимок + повторное чтение =====
...
markDirty("right")
if stock >= job.ordered then
os.sleep(0.3)
meSnap.at = -1
local stock2 = meCountOf(job.internalName, job.damage)
if stock2 >= job.ordered then
...
if now - (job.lastReportU or 0) >= 10 then job.lastReportU = now; acReport() end
end
...
local function finishBuy(success, extracted, wasPartial)
TransactionModule.unlock()
if success then
local uc, ue = buyPrice(item)
...
PendingModule.add("transaction", { player = State.currentPlayer, type = "buy", item = item.internalName, displayName = item.displayName, qty = extracted, totalCoin = totalCoin, totalEma = totalEma, timestamp = math.floor(getRealTimestamp()), dateIso = getRealIsoString() })
...
meSnap.at = -1
applyFilter(true); restoreSelectedItemByKey(getSelectedItemKey(item))
clearLog()
...
local function finishSell(success, realExtracted)
TransactionModule.unlock()
if success then
...
PendingModule.add("transaction", { player = State.currentPlayer, type = "sell", item = item.internalName, displayName = item.displayName, qty = realExtracted, totalCoin = totalCoin, totalEma = totalEma, timestamp = math.floor(getRealTimestamp()), dateIso = getRealIsoString() })
State.currentSession.balance = player.balance; State.currentSession.emaBalance = player.emaBalance; State.currentSession.transactions = player.transactions
meSnap.at = -1
clearLog()
...
PendingModule.add("transaction", { player = State.currentPlayer, type = "buy", item = "SET:" .. tostring(set.id), displayName = set.name, qty = 1, totalCoin = coin, totalEma = ema, timestamp = math.floor(getRealTimestamp()), dateIso = getRealIsoString() })
State.currentSession.balance = player.balance; State.currentSession.emaBalance = player.emaBalance; State.currentSession.transactions = player.transactions
...
State.pendingPlayer = nil; State.loadAborted = false
...
if loadStagePlayerGone(playerName) then abortOpen(playerName) return end
...
forceRender()
State.needStockRefresh = true
if loadStagePlayerGone(playerName) then handlePlayerLeave() return end
...
if pst == "present" then
local nm = PimModule.getPlayer()
if nm and not PimModule.namesMatch(nm, playerName) then stillHere = false; break end
end
...
State.pendingPlayer = nil; State.welcomeLog = {}; drawWelcomeScreen()
return
end
if loadStagePlayerGone(playerName) then State.pendingPlayer = nil; State.welcomeLog = {}; drawWelcomeScreen(); return end
syncCatalogsIfNeeded("player")
if State.loadAborted then
State.loadAborted = false; State.pendingPlayer = nil
...
if pst == "present" then
local nm = PimModule.getPlayer()
if nm and not PimModule.namesMatch(nm, playerName) then stillHere2 = false; break end
end
local pulled = { pcall(event.pull, 0.05) }
if pulled[1] and pulled[2] then
table.remove(pulled, 1)
...
if stillHere2 then
local nm = PimModule.getPlayer()
if nm and not PimModule.namesMatch(nm, playerName) then stillHere2 = false end
end
...
if State.pimActive and confirmedAbsent() then
writeDebugLog("⚠️ Игрок отсутствует сразу после открытия магазина (подтверждено) — закрываем сессию")
handlePlayerLeave()
end
end
...
local pim = component.proxy(addr)
local ok, sz = pcall(function() return pim.getInventorySize() end)
if ok and (tonumber(sz) or 0) == 0 then
local nm = PimModule.getRawInventoryName()
if nm then State.pimIdleName = nm end
end
end)
State.currentScreen = "welcome"; State.screenInitialized = false
...
if confirmedAbsent() then
writeDebugLog("⚠️ Действие получено, но игрока нет на PIM (подтверждено) — закрываем магазин")
handlePlayerLeave()
return
end
...
updateSelectorDisplay(State.selectedItem); markDirty("list"); markDirty("right")
end
return
end
...
local modeChanged = (State.currentShopMode ~= mode)
...
if modeChanged then State.searchInput = "" end
State.searchInputActive = true
State.autocraftJob = nil
if mode == "sets" then
State.setView = "list"
...
local pim = component.proxy(addr)
local ok, sz = pcall(function() return pim.getInventorySize() end)
if ok and (tonumber(sz) or 0) == 0 then State.pimIdleName = nm end
end
end)
...
if State.acKnownRecipes == nil then acRecipesLoad() end
drawWelcomeScreen()
...
if fs.exists("/home/bench.flag") then
pcall(fs.remove, "/home/bench.flag")
runSelfBench()
end
if State.searchDirty and computer.uptime() - (State.lastSearchApply or 0) > 0.2 then
State.searchDirty = false
State.lastSearchApply = computer.uptime()
if State.currentShopMode == "sets" then
applySetsFilter()
else
applyFilter()
...
if State.pimActive and State.currentScreen == "shop" and computer.uptime() - (State.lastStockRefresh or 0) > 30 then
State.lastStockRefresh = computer.uptime()
updateBuyCatalogFromME()
if State.currentShopMode == "sets" then markDirty("full") end
end
...
local nm = PimModule.getPlayer()
if nm and State.currentPlayer and not PimModule.namesMatch(nm, State.currentPlayer) then
State.otherNameStreak = (State.otherNameStreak or 0) + 1
if State.otherNameStreak >= 3 and not State.TRANSACTION_LOCK then
writeDebugLog("⚠️ Watchdog: на PIM другой игрок (" .. tostring(nm) .. ") — закрываем сессию")
handlePlayerLeave()
end
else
State.otherNameStreak = 0
end
elseif pst == "absent" then
State.absentStreak = (State.absentStreak or 0) + 1
if State.absentStreak >= 3 and not State.TRANSACTION_LOCK then
State.presenceWorks = true
writeDebugLog("⚠️ Watchdog: PIM пуст 3 проверки подряд — закрываем магазин")
handlePlayerLeave()
else
writeDebugLog("⚠️ Watchdog: PIM absent (" .. tostring(State.absentStreak) .. "/3), жду подтверждения")
end
end
end
if e == "touch" then
...
elseif ch == 205 and State.helpPage < 5 then State.helpPage = State.helpPage + 1; forceRender() end
...
elseif ch == 8 then
if data.type == "general" then data.text = utf8Sub(data.text or "", 1, -2)
elseif data.type == "price" or data.type == "missing_item" then
if data._active_field == "comment" then data.comment = utf8Sub(data.comment or "", 1, -2) else data.item_id = utf8Sub(data.item_id or "", 1, -2) end
end
forceRender()
elseif ch >= 32 then
...
markDirty("full")
elseif ch == 8 then
State.searchInput = utf8Sub(State.searchInput or "", 1, -2)
State.searchDirty = true
markDirty("header")
elseif ch >= 32 then
...
if ch == 13 then
State.purchaseInputActive = false; State.sellInputActive = false; State.searchInputActive = true; markDirty("right")
elseif ch == 8 then
local qty = State.currentShopMode == "buy" and State.purchaseQuantity or State.sellQuantity
...
local maxScroll = math.max(0, total - State.visibleRows)
local oldScroll = State.listScroll
local newScroll = oldScroll
if direction == -1 then newScroll = math.max(1, oldScroll - 3)
elseif direction == 1 then newScroll = math.min(maxScroll + 1, oldScroll + 3) end
if newScroll == oldScroll then
-- ничего
else
State.listScroll = newScroll
if State.currentShopMode == "sets" then
markDirty("list")
else
local shifted = newScroll - oldScroll
local rx, ry, rw, rh = listX + 1, listY + 3, listW - 2, State.visibleRows
pcall(function() gpu.copy(rx, ry, rw, rh, 0, -shifted) end)
shiftRowsCurr(ry, rh, shifted)
if shifted > 0 then
for row = rh - shifted + 1, rh do drawListRow(newScroll + row - 1, ry + row - 1) end
else
for row = 1, -shifted do drawListRow(newScroll + row - 1, ry + row - 1) end
end
Buffer.flush()
end
end
...
if not State.catalogsLoaded then
writeDebugLog("⚠️ Игрок на PIM, но каталоги ещё не загружены. Игнорируем.")
elseif State.syncInProgress then
writeDebugLog("⚠️ Идёт обновление, событие входа пропущено")
else
...
if PimModule.namesMatch(playerName, State.currentPlayer) then
writeDebugLog("ℹ️ Дубль события входа, игрок уже в магазине")
else
writeDebugLog("ℹ️ player_on другого игрока при активной сессии — игнорирую (разберётся Watchdog)")
end
else
...
if State.currentPlayer and (playerName == nil or playerName == "" or PimModule.namesMatch(playerName, State.currentPlayer)) then
State.absentStreak = (State.absentStreak or 0) + 2
writeDebugLog("⚠️ player_off нашего игрока — жду подтверждение Watchdog (~1 сек)")
else
writeDebugLog("ℹ️ player_off проигнорирован (не наш игрок): " .. tostring(playerName))
end
end
if State.guiDirty and (e == "touch" or e == "key_down" or e == "scroll") then
if State.renderTimer then pcall(event.cancel, State.renderTimer); State.renderTimer = nil end
State.guiDirty = false
local parts = State.pendingParts or { full = true }
State.pendingParts = { full = true }
if State.currentScreen == "welcome" then
drawWelcomeScreen()
elseif parts.full then
renderScope("full")
else
if parts.header then renderScope("header") end
if parts.list then renderScope("list") end
...
if string.find(lowerErr, "interrupted", 1, true) or string.find(lowerErr, "terminate", 1, true) then
writeDebugLog("⚠️ Завершение: " .. errText)
else
