-- ============================================================
-- TRADE MARKET CLIENT v6.9.17 (HARDWARE PRESENCE VIA getInventoryName)
-- ============================================================
...
-- Аппаратный датчик присутствия: имя того, чей инвентарь открыт через PIM
function PimModule.getInventoryName()
local pimAddr = PimModule.getAddr()
if not pimAddr then return nil end
local okp, pim = pcall(component.proxy, pimAddr)
if not okp or not pim or not pim.getInventoryName then return nil end
local ok, r = pcall(pim.getInventoryName, pim)
if ok and type(r) == "string" and r ~= "" then return r end
return nil
end
local function pimPresenceName()
return PimModule.getInventoryName()
end
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.17", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.17", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
local pres0 = pimPresenceName()
if pres0 ~= nil and PimModule.namesMatch(pres0, playerName) then State.presenceWorks = true end
...
writeDebugLog("🔎 enter: after sync+drain stillHere=" .. tostring(stillHere))
if stillHere then
local deadline = computer.uptime() + Config.PRESENCE_CONFIRM_DELAY
while computer.uptime() < deadline and stillHere do
local pres = pimPresenceName()
if pres ~= nil then
State.presenceWorks = true
if PimModule.namesMatch(pres, playerName) then
stillHere = true
break
else
stillHere = false
...
if State.presenceWorks then
stillHere = false
end
end
if not stillHere then break end
...
writeDebugLog("🔎 enter: after confirm stillHere=" .. tostring(stillHere) .. " presenceWorks=" .. tostring(State.presenceWorks))
end
...
-- Аппаратный watchdog присутствия через getInventoryName
...
if pres ~= nil then
State.presenceWorks = true
if not PimModule.namesMatch(pres, State.currentPlayer) then
writeDebugLog("⚠️ Watchdog: на PIM другой игрок (" .. tostring(pres) .. ") — сброс сессии")
handlePlayerLeave()
end
else
if State.presenceWorks then
