-- ============================================================
-- TRADE MARKET CLIENT v6.10.5 (HEARTBEAT + PAUSE + BAN + HELP)
-- ============================================================
...
helpPage = 1,
-- Патч 1: бан / пауза
bannedNow = false,        -- забанен прямо сейчас -> окно без кнопок
pauseGraceUntil = 0,      -- uptime, до которого действует grace после постановки на ТО
lastHeartbeat = 0         -- uptime последнего heartbeat
}
...
-- ============================================================
-- Патч 2: КЭШ СНИМКА ME (лечит лаги наборов). Один снимок на рендер, TTL 2 сек.
-- ============================================================
local meSnap = { at = -1, index = {} }
local function meSnapshot()
-- при отрицательном at снимок принудительно устарел
...
if State.currentSession and (State.currentSession.banned == true or State.currentSession.banned == 1) then return false, "Аккаунт заблокирован" end
if State.pauseGraceUntil > 0 and computer.uptime() < State.pauseGraceUntil then
-- grace: разрешаем докупить
else
...
-- Патч 2: meCountOf / setIsAvailable через снимок ME
...
if State.modalState.kind == "ban" then return end
...
if not State.modalState.active and not State.bannedNow then return end
local kind = State.modalState.kind
local data = State.modalState.data or {}

-- Бан-окно: неудаляемое, поверх магазина
if kind == "ban" then
local modalW, modalH = 60, 10
...
drawBox(modalX, modalY, modalW, modalH, Colors.error_red, Colors.bg_modal)
drawCenteredText(modalY + 1, "ДОСТУП ЗАБЛОКИРОВАН", Colors.error_red, Colors.bg_modal)
drawCenteredText(modalY + 3, "Ваш аккаунт заблокирован администрацией.", Colors.text_bright, Colors.bg_modal)
drawCenteredText(modalY + 4, "Причина: " .. tostring(data.reason or "Не указана"), Colors.tomato, Colors.bg_modal)
drawCenteredText(modalY + 6, "Покиньте PIM. Доступ к магазину закрыт.", Colors.text_gray, Colors.bg_modal)
return
end

-- Патч 9: окно ТО с обратным отсчётом
...
drawCenteredText(y + 2, "ТЕРМИНАЛ УХОДИТ НА ТЕХНИЧЕСКОЕ ОБСЛУЖИВАНИЕ", Colors.warning, Colors.bg_modal)
drawCenteredText(y + 4, "У вас " .. left .. " сек, чтобы завершить покупки", Colors.warning, Colors.bg_modal)
drawCenteredText(y + 6, "После этого операции будут заблокированы", Colors.text_gray, Colors.bg_modal)
drawButton(x + math.floor((w - 16) / 2), y + h - 2, 16, 1, "[ ПОНЯТНО ]", Colors.warning, Colors.bg_modal, "pause_close")
return
end
...
elseif kind == "insufficient" then modalW, modalH = 60, 12
elseif kind == "help" then modalW, modalH = UI.modal.helpW, UI.modal.helpH end
local modalX = math.floor((Config.SCREEN_W - modalW) / 2)
local modalY = math.floor((Config.SCREEN_H - modalH) / 2)
Buffer.fill(modalX, modalY, modalW, modalH, " ", Colors.text_bright, Colors.bg_modal)
...
elseif kind == "my_reports" then borderColor, title = Colors.accent_cyan, "МОИ РЕПОРТЫ"
elseif kind == "help" then borderColor = Colors.accent_cyan; title = nil end
...
local ptitle = titles[State.helpPage] or "HELP INFO:"
drawCenteredText(modalY + 1, ptitle, Colors.accent_cyan, Colors.bg_modal)
local contentY = modalY + 4
local lines = {}
if State.helpPage == 1 then
lines = {
{ text = "--- НАЧАТО ТОРГОВОЙ СЕССИИ ---", color = Colors.accent_cyan, center = true },
...
{ text = "Поиск: сразу печатайте название, фокус уже в строке поиска.", color = Colors.text_main, center = true },
...
{ text = "", color = Colors.text_main },
{ text = "Экран и клавиатура принимают управление только от владельца текущей сессии.", color = Colors.tomato, center = true },
{ text = "Если сойти с PIM, сессия закрывается, магазин ждёт следующего игрока.", color = Colors.text_main, center = true },
...
{ text = "Баланс COINS & EMA всегда доступен в разделе АККАУНТ справа.", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- ЧТО НУЖНО ДЛЯ ПОКУПКИ ПРЕДМЕТА ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "На балансе должно быть достаточно COINS и/или EMA.", color = Colors.text_main, center = true },
{ text = "Итог: цена за единицу * количество.", color = Colors.text_main, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- КАК ПОПОЛНЯЕТСЯ БАЛАНС ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "Баланс пополняется в разделе [ Пополнение ] продажей предметов магазину.", color = Colors.text_main, center = true },
{ text = "У каждого предмета есть цена скупки — за неё магазин начисляет баланс.", color = Colors.text_main, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- АККАУНТ ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "Справа: Имя, COINS, EMA и число транзакций.", color = Colors.text_main, center = true },
{ text = "После покупки/продажи баланс обновляется в реальном времени.", color = Colors.success_green, center = true }
}
elseif State.helpPage == 3 then
lines = {
{ text = "--- ПОКУПКА ТОВАРА ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "На [ Покупки ] видно наличие товара в МЭ.", color = Colors.text_main, center = true },
{ text = "Введите количество — справа появится итог в COINS и EMA.", color = Colors.text_main, center = true },
{ text = "[ Купить ] проверяет баланс, сессию и место в инвентаре.", color = Colors.text_main, center = true },
{ text = "После проверок магазин списывает стоимость и выдаёт товар через PIM.", color = Colors.text_main, center = true },
{ text = "Если выдано меньше — спишется только за фактически выданное.", color = Colors.success_green, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- ПОПОЛНЕНИЕ / ПРОДАЖА ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "На [ Пополнение ] продаваемый предмет должен быть в инвентаре.", color = Colors.text_main, center = true },
{ text = "Выберите предмет — магазин увидит его количество у вас.", color = Colors.text_main, center = true },
{ text = "Введите количество или нажмите [МАКС] и нажмите [ ПРОДАТЬ ].", color = Colors.text_main, center = true },
{ text = "В ЛОГИРОВАНИИ увидите проданное количество и сумму.", color = Colors.text_main, center = true },
{ text = "", color = Colors.text_main },
{ text = "Перед подтверждением проверяйте название, количество и итог.", color = Colors.tomato, center = true }
}
elseif State.helpPage == 4 then
lines = {
{ text = "--- АВТОКРАФТ ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "Автокрафт позволяет покупать отсутствующие в наличии товары.", color = Colors.text_main, center = true },
{ text = "Условие — наличие рецепта крафта в МЭ магазина.", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- КАК ПОНЯТЬ, ЧТО АВТОКРАФТ ДОСТУПЕН ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "В правом блоке: АВТОКРАФТ: ДОСТУПЕН.", color = Colors.accent_cyan, center = true },
{ text = "Если введено больше, чем есть в МЭ — появится [ЗАКАЗАТЬ КРАФТ].", color = Colors.text_main, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- ЧТО ПРОИСХОДИТ ПОСЛЕ НАЖАТИЯ ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "Открывается окно с деталями операции.", color = Colors.text_main, center = true },
{ text = "[НАЧАТЬ КРАФТ] отправляет запрос в МЭ сеть.", color = Colors.text_main, center = true },
{ text = "Как только предметы созданы — начинается выдача.", color = Colors.success_green, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- ЛОГИРОВАНИЕ ПРОЦЕССА ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "Весь процесс виден в блоке ЛОГИРОВАНИЕ.", color = Colors.text_main, center = true },
{ text = "Оплата — только по факту выдачи товара.", color = Colors.success_green, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- ЕСЛИ УЙТИ С PIM ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "Сессия закрывается, крафт отменяется, предметы остаются в МЭ.", color = Colors.text_main, center = true },
{ text = "Не сходите с PIM во время крафта и освободите место заранее.", color = Colors.tomato, center = true }
}
elseif State.helpPage == 5 then
lines = {
{ text = "--- СИСТЕМА СКИДОК ---", color = Colors.accent_cyan, center = true },
{ text = "Раздел в разработке.", color = Colors.text_main, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- КВЕСТЫ И НАБОРЫ ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "В [ Наборы | Квесты ] можно купить готовые наборы или квесты.", color = Colors.text_main, center = true },
{ text = "На странице набора виден состав и количество предметов.", color = Colors.text_main, center = true },
{ text = "Купить можно только если в МЭ есть все предметы набора.", color = Colors.text_main, center = true },
{ text = "Сначала покупка, затем выдача.", color = Colors.text_main, center = true },
{ text = "Вы сами решаете когда забрать набор — выдача по кнопке.", color = Colors.success_green, center = true },
{ text = "Если место кончилось — вернитесь и продолжите выдачу.", color = Colors.text_main, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- ОШИБКИ И БАГИ ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "Магазин в открытом [BETA].", color = Colors.tomato, center = true },
{ text = "Нашли баг — оставьте репорт через [ Нашёл ошибку ].", color = Colors.text_main, center = true },
{ text = "За подтверждённые репорты начисляется награда.", color = Colors.text_main, center = true },
{ text = "", color = Colors.text_main },
{ text = "--- СИСТЕМА БЛОКИРОВОК ---", color = Colors.accent_cyan, center = true },
{ text = "", color = Colors.text_main },
{ text = "При попытке навредить магазину аккаунт блокируется.", color = Colors.text_main, center = true },
{ text = "Заблокированный аккаунт не может пользоваться магазином.", color = Colors.tomato, center = true },
}
end
for i, line in ipairs(lines) do
if i <= modalH - 6 then
if line.center then drawCenteredText(contentY + i - 1, line.text, line.color, Colors.bg_modal)
else writeText(modalX + 2, contentY + i - 1, line.text, line.color, Colors.bg_modal) end
end
end
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.10.5", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Market by Leytsfer — v 6.10.5", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
-- Патч 2: снимок устаревает после транзакции
meSnap.at = -1
...
-- ============================================================
-- Патч 3: HEARTBEAT + БАН-РЕЧЕК
-- ============================================================
-- Нормализация флагов сервера: в Lua 0 и "0" — это НЕ false!
local function asBool(v) return v == true or v == 1 or v == "1" end

local function heartbeatTick(force)
if not force and computer.uptime() - State.lastHeartbeat < Config.HEARTBEAT_INTERVAL then return end
State.lastHeartbeat = computer.uptime()
local ok, r = pcall(function()
return HttpModule.request("oc_heartbeat", {
...
if not ok or not r or r.status ~= "ok" then
writeDebugLog("⚠️ Heartbeat: сервер не ответил")
return
end
-- Источник истины — сервер; клиент своё состояние maintenance НЕ отправляет
local srvMaint = asBool(r.maintenance)
local srvPaused = asBool(r.paused)
if srvMaint ~= State.serverState.maintenance then
State.serverState.maintenance = srvMaint
if not State.pimActive then markDirty("full") end
end
if srvPaused ~= State.serverState.terminalPaused then
State.serverState.terminalPaused = srvPaused
if srvPaused then
State.pauseGraceUntil = computer.uptime() + Config.PAUSE_GRACE_SECONDS
if State.pimActive and not State.bannedNow then
State.modalState.active = true; State.modalState.kind = "pause_countdown"; State.modalState.data = {}
State.currentScreen = "modal"
forceRender()
end
else
State.pauseGraceUntil = 0
if State.modalState.kind == "pause_countdown" then closeModal() end
if not State.pimActive then markDirty("full") end
end
end
-- бан-речек текущего игрока
...
if nowBanned and not State.bannedNow then
State.bannedNow = true
if State.currentSession then
State.currentSession.banned = true
State.currentSession.banReason = bu.user.banReason or State.currentSession.banReason
end
forceRender()
elseif not nowBanned and State.bannedNow then
State.bannedNow = false
if State.currentSession then State.currentSession.banned = false end
markDirty("full")
end
end
end
...
-- Всегда свежие данные с сервера (баланс И флаг бана)
...
player.balance = tonumber(userData.user.balanceCoin) or player.balance or 0
player.emaBalance = tonumber(userData.user.balanceEma) or player.emaBalance or 0
player.transactions = tonumber(userData.user.transactions) or player.transactions or 0
...
player.banReason = userData.user.banReason or player.banReason or ""
PlayerModule.save()
end
...
State.logMessages = {}
-- СНАЧАЛА полностью открываем магазин
...
forceRender()
State.needStockRefresh = true
-- Бан: игрок УЖЕ в магазине, поверх показываем неудаляемое окно с причиной
if State.currentSession.banned == true or State.currentSession.banned == 1 then
State.modalState.active = true
State.modalState.kind = "ban"
State.modalState.data = { reason = tostring(State.currentSession.banReason or "Причина не указана") }
...
-- Патч 6: блокировка входа при ТО
local function handlePlayerEnter(playerName)
writeDebugLog("👤 Игрок вошёл: " .. tostring(playerName))
if State.serverState.terminalPaused or State.serverState.maintenance then
...
if State.modalState.kind == "ban" then return end
-- Патч 10: при бане блокируем весь ввод
...
-- Патч 10: закрытие окна ТО
if btn.id == "pause_close" then closeModal(); return end
if btn.id == "help_open" then State.helpPage = 1; safeOpenModal("help", {}); return end
if btn.id == "help_prev" and State.helpPage > 1 then State.helpPage = State.helpPage - 1; forceRender(); return end
if btn.id == "help_next" and State.helpPage < 5 then State.helpPage = State.helpPage + 1; forceRender(); return end
if btn.id == "help_close" then closeModal(); return end
...
elseif btn.id == "help_open" then State.helpPage = 1; safeOpenModal("help", {})
end
return
end
end
...
-- Патч 10: фокус полю количества, поиск временно теряет фокус
...
-- Патч 12: автофокус поиска при переключении режима
...
heartbeatTick(true) -- сразу узнать у сервера maintenance/paused, не ждать 30 сек
...
TransactionModule.checkTimeout()
-- Патч 4: heartbeat + контроль истечения grace
heartbeatTick()
if State.serverState.terminalPaused and State.pauseGraceUntil > 0 and computer.uptime() >= State.pauseGraceUntil then
State.pauseGraceUntil = 0
if State.modalState.kind == "pause_countdown" then closeModal() end
if State.pimActive then markDirty("full") end
end
...
-- заблокирован: игнорируем весь ввод
...
-- Патч 11: Esc только чистит текст, фокус остаётся; Enter подтверждает
if ch == 27 then
State.searchInput = ""
markDirty("full")
elseif ch == 13 then
...
local x, y = ev[3], ev[4]
local playerName = ev[6] or "Неизвестный"
if State.pimActive and State.currentPlayer and PimModule.isOwner(playerName) and PimModule.ensureValid(State.currentPlayer) and not State.categoryDropdownOpen and not isCraftingBusy() and not (State.modalState and State.modalState.kind == "ban") then
local listX, listY = UI.list.x, UI.list.y
local listW = math.floor(Config.SCREEN_W * UI.list.wRatio)
if x >= listX and x < listX + listW and y >= listY + 4 and y < listY + 4 + State.visibleRows then
local total
