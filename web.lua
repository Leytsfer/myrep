-- ============================================================
-- TRADE MARKET CLIENT v6.10.7 (OPTIMIZED)
-- ============================================================
...
local nm = nil
if pim.getPlayer then local ok, r = pcall(pim.getPlayer, pim); if ok and type(r) == "string" and r ~= "" then nm = r end end
if not nm and pim.getPlayerName then local ok, r = pcall(pim.getPlayerName, pim); if ok and type(r) == "string" and r ~= "" then nm = r end end
if not nm and pim.getUsername then local ok, r = pcall(pim.getUsername, pim); if ok and type(r) == "string" and r ~= "" then nm = r end end
if nm then return "present" end
...
-- ОПТИМИЗАЦИЯ: кэш присутствия 0.5 сек, чтобы не дёргать компонент на каждое действие
local presenceCache = { t = -1, v = nil }
local function invalidatePresence() presenceCache.t = -1; presenceCache.v = nil end
local function pimPresenceCached()
local now = computer.uptime()
if presenceCache.t >= 0 and (now - presenceCache.t) < 0.5 then return presenceCache.v end
local v = pimPresenceState()
presenceCache.t = now; presenceCache.v = v
return v
end
...
if changed then applyFilter(true); restoreSelectedItemByKey(selectedKey); markDirty("full"); writeDebugLog("ME обновлено") end
end
...
local function segLine(yy, segs)
local total = 0
for _, s in ipairs(segs) do total = total + unicode.len(s[1]) end
local xx = modalX + math.floor((modalW - total) / 2) + 1
for _, s in ipairs(segs) do writeText(xx, yy, s[1], s[2], Colors.bg_modal); xx = xx + unicode.len(s[1]) end
end
local Y = modalY + 3
...
writeText(Config.SCREEN_W - 30, Config.SCREEN_H, "Coded By Leytsfer v 6.10.7", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
local ART_PALETTE = {
0x131620, 0x000000, 0x010101, 0x030000, 0x020307, 0x844B4B, 0x5891A9, 0xFFF489,
0xC0A66D, 0x3C5070, 0x18406C, 0x000005, 0xB9CAC5, 0x011836, 0xFFFFFF, 0xE9F1EC,
}
local ART_FG = {
...
"1111111111111111111111111111111111111111111121111111111122111111111111111111111111111111111111111111111",
...
drawCenteredText(AUTHOR_Y, "Trade Shop by Leytsfer — v 6.10.7", Colors.text_dark, BG)
Buffer.flush()
...
if State.pimActive then return end
...
if State.pimActive then
State.deferredAcReports = State.deferredAcReports or {}
table.insert(State.deferredAcReports, payload)
else
...
local totalEma = (item.priceEma or 0)
