...
-- ===== БЕЗОПАСНОЕ САМООБНОВЛЕНИЕ (код через API, без URL на экране) =====
local SELFUPDATE_PATH = "/home/web.lua"
local SELFUPDATE_BAK = "/home/web.lua.bak"
local SelfUpdate = {}

local function selfUpdateSanity(code)
  if type(code) ~= "string" or #code < 2000 then return false, "файл слишком мал" end
  if not code:find("TRADE MARKET CLIENT", 1, true) then return false, "нет маркера" end
  local fn, err = load(code, "=selfupdate_check", "t", {})
  if not fn then return false, "синтаксис: " .. tostring(err) end
  return true
end
