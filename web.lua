-- ============================================================
-- TRADE MARKET CLIENT v6.10.4 (HELP & RESET FIX)
-- ============================================================
...
local function setIsAvailable(set)
    if not set or not set.items then return false end
    for _, it in ipairs(set.items) do
        if meCountOf(it.internalName, it.damage) < (tonumber(it.qty) or 0) then return false end
    end
    return true
end
...
local function drawModal()
    ...
    if kind == "help" then
        ...
        local titles = {
            ...
            "? ПОМОЩЬ - БАЛАНС И ВАЛЮТЫ",
            "? ПОМОЩЬ - ПОКУПКИ И ПОПОЛНЕНИЕ",
            "? ПОМОЩЬ - АВТОКРАФТ",
            "? ПОМОЩЬ - НАГРАДЫ И ВОЗМОЖНОСТИ"
        }
        local title = titles[State.helpPage] or "? ПОМОЩЬ"
        ...
        if State.helpPage == 1 then
            lines = {
                { text = "НАЧАЛО РАБОТЫ", color = Colors.accent_cyan, center = true },
                { text = "", color = Colors.text_main },
                { text = "1. Встаньте на PIM и дождитесь авторизации, загрузки аккаунта и каталога.", color = Colors.text_main },
                { text = "   Пока вы стоите на PIM, магазин закрепляет текущую сессию именно за вами.", color = Colors.text_gray },
                { text = "", color = Colors.text_main },
                { text = "— КАТАЛОГ —", color = Colors.accent_cyan, center = true },
                { text = "• [ Покупки ] - обычный каталог товаров магазина.", color = Colors.text_main },
                { text = "• Выбирайте товар мышкой или стрелками ↑ / ↓.", color = Colors.text_main },
                { text = "• Для поиска просто начните печатать название - отдельный клик по поиску не обязателен.", color = Colors.text_main },
                { text = "• Фильтр каталога: ВСЕ ТОВАРЫ / В НАЛИЧИИ / НЕ В НАЛИЧИИ.", color = Colors.text_main },
                { text = "", color = Colors.text_main },
                { text = "— ЧТО ОЗНАЧАЮТ КОЛОНКИ —", color = Colors.accent_cyan, center = true },
                { text = "В МЭ   - сколько выбранного товара сейчас находится в основной МЭ-системе.", color = Colors.success_green },
                { text = "COINA  - цена за 1 предмет в балансе COINA.", color = Colors.accent_cyan },
                { text = "EMA    - цена за 1 предмет в балансе EMA.", color = Colors.tomato },
                { text = "", color = Colors.text_main },
                { text = "— КОЛИЧЕСТВО —", color = Colors.accent_cyan, center = true },
                { text = "1. Выберите товар и нажмите на поле КОЛИЧЕСТВО.", color = Colors.text_main },
                { text = "2. Введите число. Магазин сразу пересчитает итоговую стоимость.", color = Colors.text_main },
                { text = "3. [ Стереть ] очищает введённое количество.", color = Colors.text_main },
                { text = "", color = Colors.text_main },
                { text = "— ВАЖНО ПРО PIM —", color = Colors.accent_cyan, center = true },
                { text = "• Экран и клавиатура принимают управление только от владельца текущей PIM-сессии.", color = Colors.tomato },
                { text = "• Если сойти с PIM, сессия закрывается и магазин возвращается в режим ожидания.", color = Colors.text_main },
                { text = "• Не сходите с PIM во время покупки, пополнения, выдачи набора или автокрафта.", color = Colors.text_main },
                { text = "", color = Colors.text_main },
                { text = "ESC закрывает открытое окно помощи и возвращает вас в магазин.", color = Colors.text_gray }
            }
        elseif State.helpPage == 2 then
            lines = {
                { text = "БАЛАНС, COINA И EMA", color = Colors.accent_cyan, center = true },
                { text = "", color = Colors.text_main },
                { text = "В VIP SHOP используются ДВА независимых баланса: COINA и EMA.", color = Colors.text_main },
                { text = "Они не заменяют друг друга и автоматически между собой не конвертируются.", color = Colors.text_gray },
                { text = "", color = Colors.text_main },
                { text = "— COINA —", color = Colors.accent_cyan, center = true },
                { text = "COINA списывается или начисляется там, где у товара указана цена COINA больше 0.", color = Colors.text_main },
                { text = "Текущий COINA-баланс всегда показан справа в разделе АККАУНТ.", color = Colors.accent_cyan },
                { text = "", color = Colors.text_main },
                { text = "— EMA —", color = Colors.accent_cyan, center = true },
                { text = "EMA - отдельный баланс. Он используется там, где у товара указана цена EMA больше 0.", color = Colors.text_main },
                { text = "Текущий EMA-баланс также отображается справа в разделе АККАУНТ.", color = Colors.tomato },
                { text = "", color = Colors.text_main },
                { text = "— КАК СЧИТАЕТСЯ ПОКУПКА —", color = Colors.accent_cyan, center = true },
                { text = "• Товар может стоить только COINA, только EMA или одновременно обе валюты.", color = Colors.text_main },
                { text = "• Если цена одной валюты равна 0 - эта валюта для данной операции не требуется.", color = Colors.text_main },
                { text = "• Если нужны обе валюты, на балансе должно хватать и COINA, и EMA.", color = Colors.text_main },
                { text = "• Итог = цена за 1 шт. * выбранное количество.", color = Colors.text_main },
                { text = "", color = Colors.text_main },
                { text = "— КАК ПОПОЛНЯЕТСЯ БАЛАНС —", color = Colors.accent_cyan, center = true },
                { text = "Основной способ - раздел [ Пополнить ]: вы передаёте разрешённые предметы магазину.", color = Colors.text_main },
                { text = "Для каждого предмета отдельно настроено, сколько COINA и/или EMA начисляется за 1 шт.", color = Colors.text_main },
                { text = "VIP STYLE открывает цветовые темы и персональный цвет ника.", color = Colors.text_main },
                { text = "", color = Colors.text_main },
                { text = "— АККАУНТ —", color = Colors.accent_cyan, center = true },
                { text = "Справа показываются: имя, COINA, EMA, дата регистрации и число транзакций.", color = Colors.text_main },
                { text = "После подтверждённой операции баланс обновляется прямо в интерфейсе магазина.", color = Colors.success_green }
            }
        elseif State.helpPage == 3 then
            lines = {
                { text = "ПОКУПКИ И ПОПОЛНЕНИЕ", color = Colors.accent_cyan, center = true },
                { text = "", color = Colors.text_main },
                { text = "— ПОКУПКА ТОВАРА —", color = Colors.accent_cyan, center = true },
                { text = "1. Откройте [ Покупки ], выберите товар и проверьте его наличие в МЭ.", color = Colors.text_main },
                { text = "2. Введите количество. Справа появится итог в COINA и EMA.", color = Colors.text_main },
                { text = "3. Нажмите [ Купить ]. Магазин проверит сессию, баланс, МЭ и место в инвентаре.", color = Colors.text_main },
                { text = "4. VPS защищённо подтверждает оплату и транзакцию, после чего МЭ выдаёт товар на PIM.", color = Colors.text_main },
                { text = "5. После успеха откроется чек: товар, количество, цена, баланс ДО/ПОСЛЕ и ID транзакции.", color = Colors.text_main },
                { text = "6. Если выдано меньше заказанного, стоимость невыданной части возвращается.", color = Colors.success_green },
                { text = "   Если ничего не выдано - выполняется полный возврат подтверждённой оплаты.", color = Colors.success_green },
                { text = "", color = Colors.text_main },
                { text = "— ПОПОЛНЕНИЕ / ПРОДАЖА ПРЕДМЕТОВ —", color = Colors.accent_cyan, center = true },
                { text = "1. Положите нужный предмет в свой инвентарь и нажмите [ Пополнить ].", color = Colors.text_main },
                { text = "2. Выберите этот предмет в каталоге - магазин сам покажет, сколько штук найдено у вас.", color = Colors.text_main },
                { text = "3. Введите количество. Если оставить поле пустым, магазин предложит всё найденное количество.", color = Colors.text_main },
                { text = "4. Нажмите [ Продать ] и внимательно проверьте окно расчёта COINA / EMA.", color = Colors.text_main },
                { text = "5. После подтверждения предметы переводятся из вашего инвентаря в МЭ магазина.", color = Colors.text_main },
                { text = "6. Сервер начисляет рассчитанные COINA и/или EMA и показывает чек пополнения.", color = Colors.success_green },
                { text = "", color = Colors.text_main },
                { text = "— ЕСЛИ СЕРВЕР ВРЕМЕННО НЕ ОТВЕТИЛ —", color = Colors.accent_cyan, center = true },
                { text = "Если предметы при пополнении уже приняты, операция сохраняется на HDD магазина.", color = Colors.text_main },
                { text = "Начисление будет повторено позже. Повторно отдавать эти же предметы НЕ НУЖНО.", color = Colors.tomato },
                { text = "", color = Colors.text_main },
                { text = "Перед подтверждением всегда проверяйте название, количество и итоговую сумму.", color = Colors.text_gray }
            }
        elseif State.helpPage == 4 then
            lines = {
                { text = "АВТОКРАФТ", color = Colors.accent_cyan, center = true },
                { text = "", color = Colors.text_main },
                { text = "Автокрафт позволяет купить больше предметов, чем прямо сейчас лежит в МЭ,", color = Colors.text_main },
                { text = "НО только если для выбранного товара в МЭ существует подходящий шаблон крафта.", color = Colors.accent_cyan },
                { text = "", color = Colors.text_main },
                { text = "— КАК ПОНЯТЬ, ЧТО АВТОКРАФТ ДОСТУПЕН —", color = Colors.accent_cyan, center = true },
                { text = "В информации о товаре должна быть строка: Автокрафт: ДОСТУПЕН.", color = Colors.accent_cyan },
                { text = "Введите количество больше текущего остатка - [ Купить ] сменится на [ Автокрафт ].", color = Colors.text_main },
                { text = "", color = Colors.text_main },
                { text = "— ЧТО ПРОИСХОДИТ ПОСЛЕ НАЖАТИЯ —", color = Colors.accent_cyan, center = true },
                { text = "1. Магазин повторно проверяет актуальный остаток и ищет свежий шаблон в МЭ.", color = Colors.text_main },
                { text = "2. Окно покажет: сколько есть, сколько не хватает и сколько нужно операций крафта.", color = Colors.text_main },
                { text = "3. Также будет показано, сколько создастся и сколько лишних предметов останется в МЭ.", color = Colors.text_main },
                { text = "4. Нажмите [ ЗАПУСТИТЬ АВТОКРАФТ ] и дождитесь завершения задания МЭ.", color = Colors.text_main },
                { text = "5. После появления нужного количества магазин проводит обычную защищённую покупку и выдачу.", color = Colors.success_green },
                { text = "", color = Colors.text_main },
                { text = "— ОПЛАТА И ЗАЩИТА —", color = Colors.accent_cyan, center = true },
                { text = "• Баланс проверяется ещё до запуска крафта.", color = Colors.text_main },
                { text = "• Само задание МЭ сначала должно успешно создать недостающие предметы.", color = Colors.text_main },
                { text = "• Затем VPS подтверждает оплату, а магазин выдаёт готовый товар.", color = Colors.text_main },
                { text = "• При неполной выдаче оплачивается только реально выданное; остальное возвращается.", color = Colors.success_green },
                { text = "", color = Colors.text_main },
                { text = "— ЕСЛИ УЙТИ С PIM —", color = Colors.accent_cyan, center = true },
                { text = "Сессия закрывается и товар не должен перейти другому игроку.", color = Colors.text_main },
                { text = "Если оплата уже успела подтвердиться, магазин пытается вернуть стоимость невыданного товара.", color = Colors.success_green },
                { text = "Уже созданные автокрафтом предметы при этом могут остаться в МЭ магазина.", color = Colors.text_gray },
                { text = "", color = Colors.text_main },
                { text = "Не сходите с PIM и заранее освободите место в инвентаре.", color = Colors.tomato, center = true }
            }
        elseif State.helpPage == 5 then
            lines = {
                { text = "VIP STYLE, КВЕСТЫ И ВАЖНАЯ ИНФОРМАЦИЯ", color = Colors.accent_cyan, center = true },
                { text = "", color = Colors.text_main },
                { text = "— VIP STYLE —", color = Colors.accent_cyan, center = true },
                { text = "• VIP STYLE приобретается один раз за 100 EMA и остаётся в аккаунте.", color = Colors.text_main },
                { text = "• После покупки справа появляется кнопка [ НАСТРОЙКИ ].", color = Colors.text_main },
                { text = "• Доступны 6 цветовых тем интерфейса и отдельный цвет ника.", color = Colors.text_main },
                { text = "• Выбранное оформление сохраняется на VPS и загружается при следующем входе.", color = Colors.success_green },
                { text = "", color = Colors.text_main },
                { text = "— КВЕСТЫ И НАБОРЫ —", color = Colors.accent_cyan, center = true },
                { text = "• Откройте [ Квесты и Наборы ], выберите испытание или набор и откройте его состав.", color = Colors.text_main },
                { text = "• В составе видно: сколько предметов есть в МЭ, сколько требуется и сколько уже выдано.", color = Colors.text_main },
                { text = "• Покупка/выдача доступна только когда все оставшиеся необходимые предметы есть в МЭ.", color = Colors.text_main },
                { text = "• Если инвентарь заполнен, незавершённая выдача сохраняется и может продолжиться позже.", color = Colors.text_main },
                { text = "• При следующем входе владельца на PIM магазин снова попробует продолжить сохранённую выдачу.", color = Colors.success_green },
                { text = "", color = Colors.text_main },
                { text = "— ОШИБКИ И СОХРАНЁННЫЕ ОПЕРАЦИИ —", color = Colors.accent_cyan, center = true },
                { text = "• Не нажимайте покупку или пополнение повторно, если окно сообщает, что операция сохранена.", color = Colors.tomato },
                { text = "• Покупки и пополнения используют уникальные ID и локальные журналы HDD против двойных операций.", color = Colors.text_main },
                { text = "• При сетевой ошибке следуйте тексту POPUP - магазин сам сообщает, списаны ли деньги/приняты ли вещи.", color = Colors.text_main },
                { text = "", color = Colors.text_main },
                { text = "— БЕЗОПАСНОСТЬ —", color = Colors.accent_cyan, center = true },
                { text = "• Не сходите с PIM до завершения активной операции.", color = Colors.text_main },
                { text = "• Не пытайтесь обходить блокировки, повторять операции для накрутки или вмешиваться в чужую сессию.", color = Colors.text_main },
                { text = "• При блокировке аккаунта покупки, пополнение, меню и автокрафт становятся недоступны.", color = Colors.tomato },
                { text = "", color = Colors.text_main },
                { text = "Если POPUP сообщает о редкой ошибке возврата или просит обратиться к администрации,", color = Colors.text_gray },
                { text = "сохраните номер/ID операции и не повторяйте действие до проверки.", color = Colors.text_gray }
            }
        end
    ...
    -- Standard Modals (Error, Info, Report, etc.)
        ...
        if State.setView == "list" then
            ...
            for row = 1, State.visibleRows do
                ...
                if s then
                    ...
                    elseif setIsAvailable(s) then st, stCol = "ГОТОВ", Colors.success_green
            ...
            for row = 1, State.visibleRows do
                ...
                if it then
                    ...
                    local got = setDispensedFor(s, it)
...
local function drawSetBox(panelX, panelY, panelW, topH, set)
    ...
    elseif setIsAvailable(set) then stText, stColor = "ГОТОВ К ВЫДАЧЕ", Colors.success_green
...
local function drawSetContentsBox(panelX, panelY, panelW, topH, set)
    ...
    if it then
        ...
        writeText(panelX + 2, baseY + lineSpacing * 2, "В НАБОРЕ: " .. (it.qty or 0), Colors.text_bright, Colors.bg_main)
        local got = setDispensedFor(set, it)
        local need = it.qty or 0
    ...
    local totalY = btnY + 2
    if bought then
        local dispTotal = setTotalDispensed(set)
        local reqTotal = setTotalQty(set)
...
function drawWelcomeScreen()
    ...
    drawCenteredText(30, "Trade Shop by Leytsfer — v 6.10.4", Colors.text_main, Colors.bg_main)
    local logStartY = 32
...
local function performSetPurchase()
    ...
    if not setIsAvailable(set) then openErrorModal("Недоступно", "В МЭ нет всех предметов набора"); return end
...
local function performSetDispense()
    ...
    if not TransactionModule.lock(txid) then openErrorModal("Подожди", "Транзакция уже выполняется или слишком частые запросы."); return end
    local me = component.me_interface
    local okItems, meItems = pcall(function() return me.getItemsInNetwork() end)
    local meIdx = {}
    if okItems and type(meItems) == "table" then
        for _, m in ipairs(meItems) do local k = m.name .. ":" .. (m.damage or 0); meIdx[k] = (meIdx[k] or 0) + (m.size or 0) end
    end
    clearLog()
    ...
    for _, it in ipairs(set.items or {}) do
        ...
        local need = (it.qty or 0) - (pr.dispensed[key] or 0)
        if need > 0 then
    ...
    for _, it in ipairs(set.items or {}) do
        local key = it.internalName .. ":" .. (it.damage or 0)
        if (pr.dispensed[key] or 0) < (it.qty or 0) then completed = false; break end
    end
    if completed then
        ...
        elseif e == "key_down" then
            ...
            if State.pimActive and State.currentPlayer and PimModule.isOwner(playerName) and PimModule.ensureValid(State.currentPlayer) then
                ...
                elseif State.modalState.active and State.modalState.kind == "help" then
                    ...
                    elseif ch == 203 and State.helpPage > 1 then State.helpPage = State.helpPage - 1; forceRender() -- Left Arrow
                    elseif ch == 205 and State.helpPage < 5 then State.helpPage = State.helpPage + 1; forceRender() -- Right Arrow
                    end
