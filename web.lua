...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.12", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.12", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
forceRender()
-- финальная проверка ПОСЛЕ открытия: ждём полное окно, ловим player_off
local gone = false
...
-- НЕ прерываемся по таймауту: дожидаемся конца окна
end
...
elseif e2 == "player_on" or e2 == "pim" or e2 == "pim_player_enter" then -- дубль входа
...
-- игрок ждёт крафт, ввод игнорируется
...
-- событие ухода авторитетно: любой уход при открытом магазине закрывает его
