...
currentShopMode = "buy", lastCatalogUpdate = 0, lastStockRefresh = 0,
...
-- fallback: ASCII + UTF-8 кириллица вручную
...
forceRender()
-- финальная проверка ПОСЛЕ открытия: игрок мог уйти во время загрузки интерфейса
local gone = false
local deadline = computer.uptime() + 0.3
while computer.uptime() < deadline and not gone do
...
if a1 == playerName or a1 == nil then gone = true end
else
...
if not gone and pimSupportsRead() then
local actual = PimModule.getPlayer()
if not actual or actual == "" or actual ~= playerName then gone = true end
end
if gone then
abortOpen(playerName)
return
...
if pimSupportsRead() then
local actual = PimModule.getPlayer()
if not actual or actual == "" or actual ~= State.currentPlayer then
handlePlayerLeave()
else
updateBuyCatalogFromME()
end
