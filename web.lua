-- ============================================================
-- TRADE MARKET CLIENT v6.9.21 (SETS & QUESTS)
-- ============================================================
...
local State = {
  ...
  setView = "list", selectedSet = nil, selectedSetItem = nil, setDirty = false
}
local Data = {
  ...
  setsCatalog = {}, setsDisplay = {}, setProgress = {},
...
local pimIntrospect
local applySetsFilter, performSetPurchase, performSetDispense, setIsAvailable, setProgressOf, setDispensedTotal, setRequiredTotal
-- ===== ВРЕМЯ / ЛОГ =====
...
function CatalogModule.loadSetsProgress()
  ...
  if data and type(data) == "table" then Data.setProgress = data; return true end
  Data.setProgress = {}; return false
end
function CatalogModule.saveSetsProgress() return FileModule.saveLuaMap(Config.FILES.setsProgress, Data.setProgress) end
-- ===== РЕПОРТЫ =====
local ReportModule = {}
...
-- ===== НАБОРЫ: ХЕЛПЕРЫ =====
function setProgressOf(setId)
  if not setId then return nil end
  return Data.setProgress[setId]
end
function setDispensedTotal(set)
  local pr = setProgressOf(set.id)
  if not pr or not pr.dispensed then return 0 end
  local total = 0
  for _, v in pairs(pr.dispensed) do total = total + v end
  return total
end
function setRequiredTotal(set)
  local total = 0
  for _, it in ipairs(set.items or {}) do total = total + (tonumber(it.qty) or 0) end
  return total
end
function setIsAvailable(set)
  ...
  for _, it in ipairs(set.items or {}) do
    ...
    if (idx[k] or 0) < (tonumber(it.qty) or 0) then return false end
  end
  return true
end
local function setMeCount(set, it)
  ...
  for _, it2 in ipairs(items) do
    if (it2.name .. ":" .. (it2.damage or 0)) == k then return it2.size or 0 end
  end
  return 0
end
function applySetsFilter()
  Data.setsDisplay = {}
  local searchLower = lowerStr(State.searchInput or "")
  for _, set in ipairs(Data.setsCatalog) do
...
local function applySetsDelta(changes, newVersion)
  ...
  for _, ch in ipairs(changes or {}) do
    ...
    if sid then
      if ch.action == "REMOVE" or ch.action == "delete" then
        local i = index[sid]
        if i then table.remove(cur, i); index = {}; for j, ss in ipairs(cur) do index[tonumber(ss.id)] = j end; delCnt = delCnt + 1 end
        ...
        local i = index[sid]
        if i then cur[i] = norm; updCnt = updCnt + 1 else table.insert(cur, norm); index[sid] = #cur; addCnt = addCnt + 1 end
  ...
  applySetsFilter()
  writeDebugLog("✓ Наборы дельта: +" .. addCnt .. " / изм. " .. updCnt .. " / удал. " .. delCnt)
end
local function loadSetPages()
  local allSets = {}
  ...
  State.welcomeLog[logIndex] = { message = "✅ Наборы: " .. #allSets .. " шт.", color = Colors.success_green }; drawWelcomeScreen()
  return allSets
end
local function syncSetProgressFromServer()
  if not State.currentPlayer then return end
  local res = HttpModule.request("oc_get_set_progress", { terminalId = Config.TERMINAL_ID, player = State.currentPlayer })
  if res and res.status == "ok" and type(res.progress) == "table" then
    for _, p in ipairs(res.progress) do
      local sid = tonumber(p.set_id)
      if sid then
        local localPr = Data.setProgress[sid]
        local serverDone = 0
        for _, v in pairs(p.dispensed or {}) do serverDone = serverDone + v end
        local localDone = 0
        if localPr and localPr.dispensed then for _, v in pairs(localPr.dispensed) do localDone = localDone + v end end
        if (not localPr) or (serverDone > localDone) or ((p.completed == 1 or p.completed == true) and not localPr.completed) then
          Data.setProgress[sid] = { dispensed = p.dispensed or {}, completed = (p.completed == 1 or p.completed == true) }
        end
      end
    end
...
local function syncCatalogsIfNeeded(source)
  ...
  if needSets then
    ...
    if sd and sd.status == "ok" then
      ...
      addWelcomeLog("📥 Дельта наборов недоступна, грузим целиком...", Colors.warning)
      local sets = loadSetPages()
      if sets then Data.setsCatalog = sets; Data.versions.sets = serverSets; CatalogModule.saveSets(sets); applySetsFilter() end
    end
  end
  if needUsers then
...
local function loadFullInit()
  ...
  local sets = loadSetPages()
  if sets then Data.setsCatalog = sets else State.catalogLoadFailed = true; return end
  ...
  CatalogModule.saveSets(Data.setsCatalog)
  ...
  table.insert(State.welcomeLog, { message = "✅ Готово! Buy: " .. buyCount .. ", Sell: " .. sellCount .. ", Sets: " .. setsCount .. ", Users: " .. userCount, color = Colors.success_green }); drawWelcomeScreen()
end
local function flushSessionToServer()
  PendingModule.flush(); PlayerModule.flush()
end
-- ===== МОДАЛКИ =====
    ...
    if State.setView == "list" then
      ...
      for row = 1, State.visibleRows do
        ...
        if set then
          ...
          local symbolColor = isSelected and Colors.cyan or Colors.dark_gray
          ...
          if pr and not pr.completed then statusText, statusColor = "ЖДЁТ", Colors.warning
          elseif setIsAvailable(set) then statusText, statusColor = "ГОТОВ", Colors.success_green
          else statusText, statusColor = "НЕТ", Colors.text_dark end
      ...
      writeText(listX + listW - 22, colY, "ПОЛУЧЕНО", Colors.accent_cyan, Colors.bg_panel)
      local items = set and set.items or {}
      local searchLower = lowerStr(State.searchInput or "")
      local shown = {}
      for _, it in ipairs(items) do
        local nm = lowerStr(it.displayName or it.internalName or "")
        if matchesSearch(nm, searchLower) then table.insert(shown, it) end
      end
      State.visibleRows = listH - 4
      local maxScroll = math.max(0, #shown - State.visibleRows)
      State.listScroll = math.max(1, math.min(State.listScroll, maxScroll + 1))
      for row = 1, State.visibleRows do
        local idx = State.listScroll + row - 1
        local it = shown[idx]
        local y = listY + 3 + row
        if it then
          local isSelected = (it == State.selectedSetItem)
          local bg = isSelected and Colors.bg_selected or Colors.bg_main
          Buffer.fill(listX + 1, y, listW - 2, 1, " ", Colors.text_bright, bg)
          local symbol = isSelected and ">" or "*"
          writeText(listX + 2, y, symbol, isSelected and Colors.cyan or Colors.dark_gray, bg)
          local name = it.displayName or it.internalName or ""
          if unicode.len(name) > 38 then name = unicode.sub(name, 1, 37) .. "…" end
          writeText(listX + 4, y, name, isSelected and Colors.white or Colors.text_bright, bg)
          local meCnt = setMeCount(set, it)
          ...
          local gotColor = got >= need and Colors.success_green or (got > 0 and Colors.warning or Colors.text_dark)
          writeText(listX + listW - 22, y, got .. "/" .. need, gotColor, bg)
        else Buffer.fill(listX + 1, y, listW - 2, 1, " ", Colors.text_bright, Colors.bg_main) end
      end
    end
    return
...
local function drawSetBox(panelX, panelY, panelW, topH, set)
  ...
  local pr = setProgressOf(set.id)
  local statusText, statusColor
  if pr and not pr.completed then statusText, statusColor = "ЖДЁТ ВЫДАЧИ", Colors.warning
  elseif setIsAvailable(set) then statusText, statusColor = "ГОТОВ К ВЫДАЧЕ", Colors.success_green
  else statusText, statusColor = "НЕДОСТУПЕН", Colors.text_dark end
  ...
  local bw = 20
  local bx = panelX + math.floor((panelW - bw) / 2)
  drawButton(bx, panelY + topH - 3, bw, 1, "[ Перейти к набору ]", Colors.accent_cyan, Colors.white, "set_open")
end
local function drawSetContentsBox(panelX, panelY, panelW, topH, set)
  ...
  local dispTotal = setDispensedTotal(set)
  local reqTotal = setRequiredTotal(set)
  local dispColor = dispTotal >= reqTotal and Colors.success_green or (dispTotal > 0 and Colors.warning or Colors.text_dark)
  writeText(panelX + 4 + inputW + 2, inputY, "ВЫДАНО: " .. dispTotal .. "/" .. reqTotal, dispColor, Colors.bg_main)
  local btnY = inputY + 2
  local pr = setProgressOf(set.id)
...
function performSetPurchase()
  ...
  local set = State.selectedSet
  if not set then openErrorModal("Ошибка покупки", "Набор не выбран"); return end
  local pr = setProgressOf(set.id)
  if pr and not pr.completed then openErrorModal("Ошибка покупки", "Набор уже куплен — заберите содержимое"); return end
  if not setIsAvailable(set) then openErrorModal("Ошибка покупки", "Набор недоступен: не все предметы есть в МЭ"); return end
  ...
  State.currentSession.balance = player.balance; State.currentSession.emaBalance = player.emaBalance; State.currentSession.transactions = player.transactions
  Data.setProgress[set.id] = { dispensed = {}, completed = false }
  CatalogModule.saveSetsProgress()
...
function performSetDispense()
  ...
  local set = State.selectedSet
  if not set then openErrorModal("Ошибка выдачи", "Набор не выбран"); return end
  local pr = setProgressOf(set.id)
  if not pr or pr.completed then openErrorModal("Ошибка выдачи", "Набор не куплен или уже выдан полностью"); return end
  if not component.isAvailable("me_interface") then openErrorModal("Ошибка выдачи", "ME интерфейс недоступен"); return end
  local me = component.me_interface
  local txid = computer.address():sub(1, 8) .. "_" .. computer.uptime()
  if not TransactionModule.lock(txid) then openErrorModal("Подожди", "Транзакция уже выполняется или слишком частые запросы."); return end
  clearLog()
  ...
  for _, it in ipairs(set.items or {}) do
    ...
    if need > 0 then
      ...
      while need > 0 do
        ...
        local toTake = math.min(need, 64, freeSlots * 64)
        local okExport, result = pcall(function() return me.exportItem(fingerprint, PULL_DIRECTION, toTake) end)
        local gotNow = 0
        if okExport then
          if type(result) == "number" then gotNow = result
          elseif type(result) == "boolean" and result then gotNow = toTake
          elseif type(result) == "table" then gotNow = result.count or result.amount or result.size or toTake end
        end
        if gotNow > 0 then got = got + gotNow; need = need - gotNow else meMissing = true; break end
      end
      if got > 0 then
        pr.dispensed[key] = (pr.dispensed[key] or 0) + got
        addLogMessage("✔ " .. (it.displayName or it.internalName) .. ": +" .. got .. " шт.", Colors.success_green)
      end
    end
  end
  ...
  CatalogModule.saveSetsProgress()
  PendingModule.add("set_progress", { player = State.currentPlayer, setId = set.id, dispensed = pr.dispensed, completed = completed })
  TransactionModule.unlock()
  local dispTotal = setDispensedTotal(set)
  local reqTotal = setRequiredTotal(set)
  if completed then
    addLogMessage("✅ Набор полностью выдан!", Colors.success_green)
  elseif inventoryFull then
    addLogMessage("⚠ Инвентарь заполнен. Выдано " .. dispTotal .. "/" .. reqTotal .. ".", Colors.warning)
    addLogMessage("Освободите место и нажмите «Выдать набор» снова.", Colors.warning)
  elseif meMissing then
    addLogMessage("⚠ Части предметов нет в МЭ. Выдано " .. dispTotal .. "/" .. reqTotal .. ".", Colors.warning)
  end
  State.needStockRefresh = true
  markDirty("full")
end
    ...
    if isButtonClicked(btn, x, y) then
      ...
      elseif btn.id == "set_open" then
        if State.selectedSet then
          State.setView = "contents"
          State.selectedIndex = 1
          State.listScroll = 1
          State.selectedSetItem = (State.selectedSet.items and State.selectedSet.items[1]) or nil
          clearLog()
          addLogMessage("Набор: " .. (State.selectedSet.name or ""), Colors.accent_cyan)
          addLogMessage("Выберите предмет и нажмите «Выдать набор».", Colors.text_gray)
          markDirty("full")
        end
  ...
  if x >= listX and x < listX + listW and y >= listY + 4 and y < listY + 4 + State.visibleRows then
    ...
    if State.currentShopMode == "sets" then
      if State.setView == "list" then
        local set = Data.setsDisplay[idx]
        if set then
          State.selectedIndex = idx; State.selectedSet = set; State.selectedSetItem = nil
          markDirty("full")
        end
      else
        local set = State.selectedSet
...
function switchToMode(mode)
  ...
  if mode == "sets" then
    State.setView = "list"
    State.selectedSet = nil; State.selectedSetItem = nil
    applySetsFilter()
  else
    State.setView = "list"
    State.selectedSet = nil; State.selectedSetItem = nil
  ...
  while true do
      ...
      if State.pimActive and State.currentPlayer and PimModule.isOwner(playerName) and PimModule.ensureValid(State.currentPlayer) then
        ...
        elseif State.searchInputActive then
          ...
          elseif ch == 8 then
            ...
            if State.currentShopMode == "sets" then applySetsFilter() else applyFilter(); State.selectedIndex = #Data.filteredItems > 0 and 1 or 0; State.selectedItem = Data.filteredItems[State.selectedIndex]; updateSelectorDisplay(State.selectedItem) end
            markDirty("full")
          elseif ch >= 32 then
    ...
    elseif e == "scroll" then
      ...
      if State.pimActive and State.currentPlayer and PimModule.isOwner(playerName) and PimModule.ensureValid(State.currentPlayer) and not State.categoryDropdownOpen and not isCraftingBusy() then
        ...
        local listW = math.floor(Config.SCREEN_W * UI.list.wRatio)
        if x >= listX and x < listX + listW and y >= listY + 4 and y < listY + 4 + State.visibleRows then
          local listLen = #Data.filteredItems
          if State.currentShopMode == "sets" then
            if State.setView == "list" then listLen = #Data.setsDisplay
            else
              listLen = 0
              local set = State.selectedSet
              if set then
                ...
                for _, it in ipairs(set.items or {}) do
                  local nm = lowerStr(it.displayName or it.internalName or "")
                  if matchesSearch(nm, searchLower) then listLen = listLen + 1 end
                end
              end
            end
          end
