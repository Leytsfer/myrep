...
State.pimUnknownCount = 0
elseif pst == "absent" then
State.presenceWorks = true
writeDebugLog("⚠️ Watchdog: PIM пуст — закрываем магазин")
handlePlayerLeave()
else
-- PIM не отвечает (unknown): 4 проверки подряд (~4 сек) — считаем игрока ушедшим
State.pimUnknownCount = (State.pimUnknownCount or 0) + 1
if State.pimUnknownCount >= 4 then
writeDebugLog("⚠️ Watchdog: PIM не отвечает 4 проверки подряд — закрываем магазин")
handlePlayerLeave()
end
end
end
