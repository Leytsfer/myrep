-- ============================================================
-- TRADE MARKET CLIENT v6.9.8 (NON-BLOCKING PROBE + FIX CANCEL)
-- ============================================================
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.8", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.8", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
-- Мгновенная отмена задачи крафта (рабочая форма AE2 1.7.10: job.cancel() БЕЗ self)
...
-- Тяжёлая проверка: выполняется ТОЛЬКО из очереди в главном цикле (не блокирует клик)
...
local okReq, job = pcall(function() return pattern.request(1) end)
if okReq and job then
-- мгновенная отмена в ту же долю секунды — ресурсы не успевают тратиться
pcall(function() job.cancel() end)
local ic = callMethod(job, "isCanceled")
if ic ~= true then pcall(function() job:cancel() end) end
canCraft = true
else
canCraft = false
end
State.acCraftCache[k] = { name = (st and (st.name or st.label or st.id)) or full, damage = item.damage or 0, size = (st and tonumber(st.size)) or 1, raw = pattern, canCraft = canCraft }
State.autocraftCraftablesDirty = true
writeDebugLog("🔎 Probe " .. full .. " => canCraft=" .. tostring(canCraft))
end
-- Лёгкая постановка в очередь (вызывается при клике, не блокирует)
...
-- фоновая очередь probe: по одной проверке за тик, НЕ блокирует клики
