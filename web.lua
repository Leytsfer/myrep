-- ============================================================
-- TRADE MARKET CLIENT v6.9.20 (CONFIRM WINDOW 4s) + SETS
-- ============================================================
...
local Config = {
  ...
  PRESENCE_HOLD_DELAY = 2,
  PRESENCE_CONFIRM_DELAY = 1.5,
...
local Data = {
  ...
  categoryTags = {
    ...
    ["DraconicEvolution"] = { "#DE" }, ["EIO"] = { "#EIO" }, ["Forestry"] = { "#FR" },
...
local Colors = {
  ...
  success = 0x00FFAA, success_green = 0x00FF00, error = 0xFF4D7A, error_red = 0xFF5555,
  inactive = 0x555566, warning = 0xFFAA00, tomato = 0xFF6347, white = 0xFFFFFF,
  green_bright = 0x3BFF18, orange = 0xFF8800, purple = 0xAA55FF, line = 0x00BFFF,
  blue = 0x0066FF, cyan = 0x00BFFF, dark_gray = 0x666666,
...
local switchToMode, handleTouch, handlePlayerEnter, handlePlayerLeave
local performBuy, performSell, safeExit
local applyBuyCatalog, applySellCatalog, updateBuyCatalogFromME
local validateBuyRequest, validateSellRequest
local HttpModule, PimModule, PlayerModule, CatalogModule
local TransactionModule, FileModule, ReportModule, PendingModule
local acAvailable, acCanCraft, acOpenConfirm, acStart, acCancel, acStep, acReport, acPlayerLeft, acStatusOf
local pimIntrospect
-- ===== ВРЕМЯ / ЛОГ =====
...
function CatalogModule.saveSets(sets) Data.setsCatalog = sets or {}; return FileModule.saveLuaList(Config.FILES.setsCatalog, Data.setsCatalog) end
...
function ReportModule.add(playerName, text)
  local report = { time = getRealTimeString(), name = playerName or "Аноним", text = text or "", viewed = false }
  table.insert(Data.reports, 1, report); ReportModule.save()
  return report
end
-- ===== JSON =====
...
local function from_json(str)
  ...
  if str:match('"status"%s*:%s*"ok"') and str:match('"items"%s*:%s*%[') then
...
function HttpModule.request(action, data)
  ...
  if ok and response and type(response) == "string" and response ~= "" then
    ...
    if response:find('"status"%s*:%s*"ok"') then
      writeDebugLog("⚠️ Ответ получен, но не распарсен целиком — считаю ok: " .. action)
      return { status = "ok", parsed = false }
    end
    writeDebugLog("⚠️ Не удалось распарсить ответ: " .. tostring(response):sub(1, 300))
  elseif not ok then
    writeDebugLog("⚠️ HTTP ошибка: " .. tostring(response))
  end
  return nil
end
-- ===== PENDING =====
...
local function pimSupportsRead()
  local pimAddr = PimModule.getAddr()
  if not pimAddr then return false end
  local pim = component.proxy(pimAddr)
  return (pim.getPlayer or pim.getPlayerName or pim.getUsername) ~= nil
end
pimIntrospect = function()
  local addr = PimModule.getAddr()
  if not addr then writeDebugLog("PIM: компонент не найден"); return end
  local ok, pim = pcall(component.proxy, addr)
  if not ok or not pim then writeDebugLog("PIM: не удалось получить proxy"); return end
  local names = {}
  for k, v in pairs(pim) do
    if type(v) == "function" or type(v) == "table" then table.insert(names, k) end
  end
  table.sort(names)
  writeDebugLog("PIM methods: " .. table.concat(names, ", "))
end
...
local function safeSelectorSetSlot(slot, stack)
  ...
  if not ok then writeDebugLog("⚠️ Selector: " .. tostring(res)) end
  return ok, res
end
...
local function craftCap()
  ...
  if item and State.currentSession then
    local coin, ema = item.priceCoin or 0, item.priceEma or 0
...
local function buyQtyCap(item)
  ...
  if State.currentShopMode == "buy" and acAvailable(item) then
    return math.max(stock, craftCap())
  end
  return stock
end
...
local function validateTerminalState()
  ...
  if State.serverState.terminalPaused then return false, "Терминал приостановлен" end
...
function validateBuyRequest()
  ...
  if (State.currentSession.balance or 0) < (State.selectedItem.priceCoin or 0) * State.purchaseQuantity then return false, "Недостаточно Coina" end
  if (State.currentSession.emaBalance or 0) < (State.selectedItem.priceEma or 0) * State.purchaseQuantity then return false, "Недостаточно EMA" end
...
local function applySetsFilter()
  ...
  for _, s in ipairs(Data.setsCatalog) do
    if (s.enabled == 1 or s.enabled == true) and matchesSearch(lowerStr(s.name or ""), searchLower) then
      table.insert(Data.setsDisplay, s)
    end
  end
  ...
  if State.setView == "list" then
    if #Data.setsDisplay > 0 then
      State.selectedIndex = 1; State.selectedSet = Data.setsDisplay[1]
      State.selectedSetItem = (State.selectedSet.items and State.selectedSet.items[1]) or nil
    else
      State.selectedIndex = 0; State.selectedSet = nil; State.selectedSetItem = nil
    end
  end
end
local function applyFilter()
  if State.currentShopMode == "sets" then applySetsFilter(); return end
  Data.filteredItems = {}
  local searchLower = lowerStr(State.searchInput or "")
...
local function setAvailable(set)
  if not set then return false end
  local idx = setsMeIndex()
  for _, it in ipairs(set.items or {}) do
    if (idx[it.internalName .. "|" .. (it.damage or 0)] or 0) < (it.qty or 0) then return false end
  end
  return true
end
local function setTotalQty(set)
  local t = 0
  for _, it in ipairs(set and set.items or {}) do t = t + (it.qty or 0) end
  return t
end
local function setDispensedTotal(set)
  local pr = Data.setsProgress[set and set.id]
  if not pr then return 0 end
  local t = 0
  for _, v in pairs(pr.dispensed or {}) do t = t + v end
  return t
end
local function isSetCompleted(set)
  if not set then return false end
  local pr = Data.setsProgress[set.id]
  if not pr then return false end
  for _, it in ipairs(set.items or {}) do
    if ((pr.dispensed or {})[it.internalName .. "|" .. (it.damage or 0)] or 0) < (it.qty or 0) then return false end
  end
  return true
end
...
local function loadCatalogPages(catalogType)
  ...
  while page <= totalPages do
    ...
    for _, item in ipairs(items) do table.insert(allItems, item) end
...
local function loadSetsPages()
  ...
  State.welcomeLog[logIndex] = { message = "✅ Наборы: " .. #allSets, color = Colors.success_green }; drawWelcomeScreen()
  return allSets, (tonumber(data and data.version) or 0)
end
-- ===== СИНХРОНИЗАЦИЯ =====
...
local function applySetsDelta(changes, newVersion)
  local cur = Data.setsCatalog
  local index = {}
  for i, s in ipairs(cur) do index[tonumber(s.id)] = i end
  local addCnt, updCnt, delCnt = 0, 0, 0
  for _, ch in ipairs(changes or {}) do
    local act = string.upper(tostring(ch.action or ""))
    local s = ch.set or ch.data or nil
    if s and s.id then
      ...
      if act == "REMOVE" then
        ...
        if i then
          ...
          delCnt = delCnt + 1
          index = {}
          for i2, s2 in ipairs(cur) do index[tonumber(s2.id)] = i2 end
        end
      else
        -- ADD или UPDATE (регистр не важен)
        local norm = {
          id = sid,
          name = s.name or "",
          price_coin = tonumber(s.price_coin) or 0,
          price_ema = tonumber(s.price_ema) or 0,
          enabled = tonumber(s.enabled) or 0,
          rev = tonumber(s.rev) or 1,
          items = s.items or s.items_list or {}
        }
        local i = index[sid]
        if i then cur[i] = norm; updCnt = updCnt + 1
        else table.insert(cur, norm); index[sid] = #cur; addCnt = addCnt + 1 end
      end
    end
  end
  if newVersion then Data.versions.sets = tonumber(newVersion) or Data.versions.sets end
  CatalogModule.saveSets(cur)
  CatalogModule.saveVersions()
  if applySetsFilter then applySetsFilter() end
  writeDebugLog("Наборы дельта: +"..addCnt.." изм."..updCnt.." удал."..delCnt.." ver="..tostring(Data.versions.sets))
  if State.pimActive and State.currentScreen == "shop" then markDirty("full") end
end

local function loadSetsFull()
  local all = {}
  local page, totalPages = 1, 1
  local ver = nil
  while page <= totalPages do
    local data = HttpModule.request("oc_get_sets", { terminalId = Config.TERMINAL_ID, page = page })
    if not data or data.status ~= "ok" then
      writeDebugLog("Наборы: ошибка полной загрузки стр."..page)
      return nil
    end
    for _, s in ipairs(data.sets or {}) do
      table.insert(all, {
  ...
  Data.setsCatalog = all
  Data.versions.sets = tonumber(ver) or Data.versions.sets
  CatalogModule.saveSets(all)
  CatalogModule.saveVersions()
  if applySetsFilter then applySetsFilter() end
  writeDebugLog("Наборы полная загрузка: "..#all.." наборов, ver="..tostring(Data.versions.sets))
  if State.pimActive and State.currentScreen == "shop" then markDirty("full") end
  return all
end
...
local function syncCatalogsIfNeeded(source)
  ...
  if not v or v.status ~= "ok" then addWelcomeLog("⚠️ Сервер недоступен, работаем на локальных данных", Colors.warning); return syncEnd("error", source) end
  ...
  addWelcomeLog("⚠️ Версии расходятся! Обновляем данные...", Colors.warning)
  ...
  if needSets then
    ...
    if sd and sd.status == "ok" then
      applySetsDelta(sd.changes or {}, serverSets)
    else
      local allSets, sv = loadSetsPages()
      if allSets then Data.setsCatalog = allSets; Data.versions.sets = sv or serverSets; CatalogModule.saveSets(allSets); CatalogModule.saveVersions(); if State.currentShopMode == "sets" then applySetsFilter() end end
    end
  end
  if needUsers then
    ...
    else addWelcomeLog("⚠️ Не удалось обновить данные игроков", Colors.warning) end
  end
...
local function loadFullInit()
  ...
  if not usersData or usersData.status ~= "ok" then usersData = { users = {}, buyVersion = 0, sellVersion = 0, usersVersion = 0, maintenance = false } end
  ...
  if usersData.users and type(usersData.users) == "table" then
    for _, user in ipairs(usersData.users) do
      if user.name then Data.players[user.name] = { balance = tonumber(user.balanceCoin) or 0, emaBalance = tonumber(user.balanceEma) or 0, transactions = tonumber(user.transactions) or 0, regDate = user.regDate or getRealTimeString(), banned = (tonumber(user.banned) or 0) == 1, banReason = user.banReason or "", transactionsList = {} } end
    end
  end
  usersData = nil
  PlayerModule.save()
  local allSets, vSets = loadSetsPages()
  if allSets then Data.setsCatalog = allSets; Data.versions.sets = vSets or 0; CatalogModule.saveSets(allSets) end
  Data.versions = { buy = vBuy, sell = vSell, users = vUsers, sets = Data.versions.sets or 0 }
  CatalogModule.saveVersions()
  applyBuyCatalog(Data.buyCatalog); applySellCatalog(Data.sellCatalog)
  ...
  table.insert(State.welcomeLog, { message = "✅ Готово! Buy: " .. buyCount .. ", Sell: " .. sellCount .. ", Users: " .. userCount, color = Colors.success_green }); drawWelcomeScreen()
end
local function flushSessionToServer() PendingModule.flush(); PlayerModule.flush() end
-- ===== МОДАЛКИ =====
...
local function drawModal()
  ...
  if kind == "reward" then
    ...
    if ema > 0 then table.insert(segs, { text = formatPrice(ema) .. " EMA", color = Colors.orange }) end
  ...
  elseif kind == "report_form" then
    ...
    if data.type == "general" then
      ...
      for i = startI, #wl do
        writeText(modalX + 3, curY + (i - startI), wl[i], Colors.text_bright, Colors.bg_input)
      end
      curY = curY + 6
    else
      ...
      for i = startI, #wl do
        writeText(modalX + 3, commentY + (i - startI), wl[i], Colors.text_bright, Colors.bg_input)
      end
...
local function drawCategoryDropdown()
...
local function drawHeader()
  ...
  local tradeMarketX = math.floor((Config.SCREEN_W + searchX + searchW) / 2) - math.floor(unicode.len("TRADE MARKET") / 2)
  writeText(tradeMarketX, 2, "TRADE MARKET", Colors.cyan, Colors.bg_header)
...
-- ===== НАБОРЫ: отрисовка списка =====
local function drawSetsList(listX, listY, listW, listH)
  if State.setView == "list" then
    ...
    for row = 1, State.visibleRows do
      ...
      if set then
        ...
        local avail = setAvailable(set)
        writeText(listX + listW - 32, y, avail and "ГОТОВ" or "НЕТ", avail and Colors.success_green or Colors.text_dark, bg)
        writeText(listX + listW - 22, y, formatPrice(set.price_coin), (set.price_coin or 0) > 0 and Colors.accent_cyan or Colors.text_dark, bg)
        writeText(listX + listW - 12, y, formatPrice(set.price_ema), (set.price_ema or 0) > 0 and Colors.tomato or Colors.text_dark, bg)
    ...
    local dashEnd = listX + listW - 2
    if dashEnd > dashStart then for x = dashStart, dashEnd do Buffer.set(x, listY + 1, "-", Colors.line, Colors.bg_main) end end
    if set then writeText(listX + listW - 2 - unicode.len("[" .. (set.name or "") .. "]"), listY + 1, "[" .. (set.name or "") .. "]", Colors.accent_cyan, Colors.bg_main) end
    local colY = listY + 2
    Buffer.fill(listX + 1, colY, listW - 2, 1, " ", Colors.text_bright, Colors.bg_panel)
    ...
    local items = (set and set.items) or {}
    State.visibleRows = listH - 4
    local maxScroll = math.max(0, #items - State.visibleRows)
    State.listScroll = math.max(1, math.min(State.listScroll, maxScroll + 1))
    local idxMe = setsMeIndex()
    for row = 1, State.visibleRows do
      local idx = State.listScroll + row - 1
      local it = items[idx]
      local y = listY + 3 + row
      if it then
        ...
        if unicode.len(name) > 38 then name = unicode.sub(name, 1, 37) .. "…" end
        writeText(listX + 4, y, name, isSelected and Colors.white or Colors.text_bright, bg)
        local meCnt = idxMe[it.internalName .. "|" .. (it.damage or 0)] or 0
        writeText(listX + listW - 32, y, formatQtyCompact(meCnt), meCnt > 0 and Colors.success_green or Colors.text_dark, bg)
        local pr = Data.setsProgress[set.id]
        local got = (pr and pr.dispensed and pr.dispensed[it.internalName .. "|" .. (it.damage or 0)]) or 0
        writeText(listX + listW - 16, y, got .. "/" .. (it.qty or 0), (got >= (it.qty or 0)) and Colors.success_green or Colors.warning, bg)
      else Buffer.fill(listX + 1, y, listW - 2, 1, " ", Colors.text_bright, Colors.bg_main) end
    end
  end
end
...
local function drawAutocraftBox(panelX, panelY, panelW, topH, job)
  ...
  table.insert(segsList, { { "ИТОГО: ", Colors.text_bright }, { formatPrice(tc) .. " COINS", Colors.accent_cyan }, { " | ", Colors.text_bright }, { formatPrice(te) .. " EMA", Colors.orange } })
  local maxW = 0
...
local function drawItemBox(panelX, panelY, panelW, topH, item)
  ...
  local coinPrice, emaPrice = item.priceCoin or 0, item.priceEma or 0
  ...
  if State.currentShopMode == "buy" then
    ...
    local btnW2 = useCraft and 18 or 7
    local label = useCraft and "[ ЗАКАЗАТЬ КРАФТ ]" or "[КУП]"
    ...
    fillBox(buyBtnX, inputY, 8, 1, canSell and Colors.success_green or Colors.text_dark)
    writeText(buyBtnX, inputY, "[ПРОД]", Colors.white, canSell and Colors.success_green or Colors.text_dark)
    table.insert(State.buttons, { id = "sell_item", x = buyBtnX, y = inputY, w = 8, h = 1 })
  end
...
local function drawSetBox(panelX, panelY, panelW, topH, set)
  ...
  local avail = setAvailable(set)
  writeText(panelX + 2, baseY + lineSpacing, "СТАТУС: " .. (available and "" or "") .. (avail and "Готов к выдаче" or "Недоступен"), avail and Colors.success_green or Colors.error_red, Colors.bg_main)
  writeText(panelX + 2, baseY + lineSpacing * 2, "ПРЕДМЕТОВ: " .. setTotalQty(set) .. " шт", Colors.text_bright, Colors.bg_main)
  local priceY = baseY + lineSpacing * 3
...
local function drawSetContentsBox(panelX, panelY, panelW, topH, set)
  ...
  if it then
    ...
    writeText(panelX + 2, baseY + lineSpacing * 2, "В НАБОРЕ: " .. (it.qty or 0) .. " шт", Colors.text_bright, Colors.bg_main)
    local pr = Data.setsProgress[set.id]
    local got = (pr and pr.dispensed and pr.dispensed[it.internalName .. "|" .. (it.damage or 0)]) or 0
    writeText(panelX + 2, baseY + lineSpacing * 3, "ПОЛУЧЕНО: " .. got .. "/" .. (it.qty or 0), (got >= (it.qty or 0)) and Colors.success_green or Colors.warning, Colors.bg_main)
  else
  ...
  local totalDisp = setDispensedTotal(set)
  local totalQty = setTotalQty(set)
  writeText(panelX + 18, inputY, "ВЫДАНО: " .. totalDisp .. "/" .. totalQty, (totalDisp >= totalQty) and Colors.success_green or Colors.warning, Colors.bg_main)
  local pr = Data.setsProgress[set.id]
  local purchased = pr and not pr.completed
  local btnW1, btnW2 = 16, 10
  local bx1 = panelX + 2
  local bx2 = bx1 + btnW1 + 2
  if purchased then
    drawButton(bx1, inputY + 2, btnW1, 1, "[ Выдать набор ]", Colors.blue, Colors.white, "set_dispense")
  else
    drawButton(bx1, inputY + 2, btnW1, 1, "[ Купить набор ]", Colors.success_green, Colors.white, "set_buy")
  end
  drawButton(bx2, inputY + 2, btnW2, 1, "[ Назад ]", Colors.bg_button, Colors.white, "set_back")
  local totalY = inputY + 4
  local coinPrice, emaPrice = set.price_coin or 0, set.price_ema or 0
  writeText(panelX + 2, totalY, "ИТОГО: ", Colors.text_bright, Colors.bg_main)
  local totalX = panelX + 2 + unicode.len("ИТОГО: ")
  local coinStr, emaStr = formatPrice(coinPrice), formatPrice(emaPrice)
  if coinPrice > 0 and emaPrice > 0 then
    writeText(totalX, totalY, coinStr .. " COINS", Colors.accent_cyan, Colors.bg_main); totalX = totalX + unicode.len(coinStr .. " COINS")
    writeText(totalX, totalY, " & ", Colors.text_bright, Colors.bg_main); totalX = totalX + 3
    writeText(totalX, totalY, emaStr .. " EM", Colors.tomato, Colors.bg_main)
  elseif coinPrice > 0 then writeText(totalX, totalY, coinStr .. " COINS", Colors.accent_cyan, Colors.bg_main)
  elseif emaPrice > 0 then writeText(totalX, totalY, emaStr .. " EM", Colors.tomato, Colors.bg_main)
  else writeText(totalX, totalY, "Бесплатно", Colors.text_bright, Colors.bg_main) end
end
local function drawAccBox(panelX, panelW, accY, accH)
  ...
  if State.currentSession then
    ...
    writeText(panelX + 2, accY + 7, "Транзакции: ", Colors.text_main, Colors.bg_panel)
...
local function drawItemList()
  ...
  if State.currentShopMode == "sets" then
    drawSetsList(listX, listY, listW, listH)
    return
  end
...
local function drawRightPanel()
  ...
  if State.currentShopMode == "sets" then
    if State.setView == "list" then
      if State.selectedSet then drawSetBox(panelX, panelY, panelW, topH, State.selectedSet)
      else drawBox(panelX, panelY, panelW, topH, Colors.line, Colors.bg_main); writeText(panelX + 2, panelY + 4, "Набор не выбран", Colors.text_dark, Colors.bg_main) end
    else
      if State.selectedSet then drawSetContentsBox(panelX, panelY, panelW, topH, State.selectedSet)
      else drawBox(panelX, panelY, panelW, topH, Colors.line, Colors.bg_main); writeText(panelX + 2, panelY + 4, "Набор не выбран", Colors.text_dark, Colors.bg_main) end
    end
...
local function drawBottomBar()
  ...
  local sellColor = State.currentShopMode == "sell" and Colors.orange or Colors.bg_button
  ...
  writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.20", Colors.text_dark, Colors.bg_panel)
  local reportBtnW = 18
...
function drawWelcomeScreen()
  ...
  drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.20", Colors.text_main, Colors.bg_main)
  local logStartY = 32
  ...
  if #State.welcomeLog > 0 then
    ...
    for i = startIndex, #State.welcomeLog do
      local y = logStartY + (i - startIndex)
...
local function drawMainScreen()
  ...
  drawRightPanel()
  drawBottomBar()
  if State.modalState.active then drawModal() end
...
local function acMeCount(internalName, damage)
  ...
  local total = 0
  for _, it in ipairs(items) do
    if it.name == internalName and (it.damage or 0) == (damage or 0) then total = total + (it.size or 0) end
  end
  return total
end
...
local function acDispense(job)
  ...
  if netCoin > 0 or netEma > 0 then
    ...
    if netEma > 0 then table.insert(segs, { formatPrice(netEma) .. " EMA", Colors.orange }) end
    acLogJob(job, segs)
  else
...
acOpenConfirm = function()
  ...
  local inStock = acMeCount(item.internalName, item.damage or 0)
  local ordered = State.purchaseQuantity
  local needCoin = (item.priceCoin or 0) * ordered
  local needEma = (item.priceEma or 0) * ordered
...
acStart = function()
  ...
  local inStock = acMeCount(job.internalName, job.damage)
  job.inStock = inStock
...
acStep = function()
  ...
  if now - (job.lastStockTick or 0) >= 2 then
    ...
    local stock = acMeCount(job.internalName, job.damage)
...
local function performBuy()
  ...
  if not ok then
    if err == "Недостаточно Coina" or err == "Недостаточно EMA" then
      ...
      local needC = (item.priceCoin or 0) * qty
      local needE = (item.priceEma or 0) * qty
      local missC = math.max(0, needC - (State.currentSession.balance or 0))
      local missE = math.max(0, needE - (State.currentSession.emaBalance or 0))
  ...
  local function finishBuy(success, extracted, wasPartial)
    TransactionModule.unlock()
    if success then
      local totalCoin = (item.priceCoin or 0) * extracted
      local totalEma = (item.priceEma or 0) * extracted
      ...
      if totalEma > 0 then table.insert(segs, { formatPrice(totalEma) .. " EM", Colors.orange }) end
...
-- ============================================================
-- ===== НАБОРЫ: СОСТОЯНИЕ, ФАЙЛЫ, ХЕЛПЕРЫ =====
-- ============================================================
Config.FILES.setsCatalog = Config.FILES.setsCatalog or "/home/setsCatalog.lua"
Config.FILES.setsProgress = Config.FILES.setsProgress or "/home/setsProgress.lua"
Data.setsCatalog = Data.setsCatalog or {}
Data.setsProgress = Data.setsProgress or {}
Data.versions.sets = Data.versions.sets or 0
State.setView = State.setView or "list"
State.selectedSet = State.selectedSet or nil
State.selectedSetItem = State.selectedSetItem or nil

function FileModule.saveSets(list)
  Data.setsCatalog = list or {}
  return FileModule.saveLuaList(Config.FILES.setsCatalog, Data.setsCatalog)
end
function FileModule.loadSets()
  local data = FileModule.loadLuaFile(Config.FILES.setsCatalog)
  if data and type(data) == "table" then Data.setsCatalog = data; return true end
  Data.setsCatalog = {}; return false
end
function FileModule.saveSetsProgress()
  return FileModule.saveLuaMap(Config.FILES.setsProgress, Data.setsProgress)
end
function FileModule.loadSetsProgress()
  local data = FileModule.loadLuaFile(Config.FILES.setsProgress)
  if data and type(data) == "table" then Data.setsProgress = data; return true end
  Data.setsProgress = {}; return false
end

-- Хватает ли предметов в МЭ на весь набор
function setsAvailable(set)
  if not set then return false end
  ...
  for _, si in ipairs(set.items or {}) do
    local k = si.internalName .. ":" .. (si.damage or 0)
    if (idx[k] or 0) < (si.qty or 0) then return false end
  end
  return true
end

-- Сколько уже выдано конкретного предмета набора
function setGetDisp(set, si)
  if not set or not si then return 0 end
  local pr = Data.setsProgress[set.id]
  if not pr or not pr.dispensed then return 0 end
  return pr.dispensed[si.internalName .. ":" .. (si.damage or 0)] or 0
end

-- Сколько всего выдано из набора (в штуках)
function setDispensedTotal(set)
...
-- Сколько всего штук в наборе
function setQtyTotal(set)
  if not set then return 0 end
  local t = 0
  for _, si in ipairs(set.items or {}) do t = t + (si.qty or 0) end
  return t
end

-- Куплен ли набор и не закрыт ли он
function setIsPurchased(set)
  if not set then return false end
  local pr = Data.setsProgress[set.id]
  return (pr ~= nil) and (not pr.completed)
end

-- Список наборов с учётом поиска
function setsFilteredList()
  local searchLower = lowerStr(State.searchInput or "")
  local out = {}
  for _, s in ipairs(Data.setsCatalog or {}) do
    if matchesSearch(lowerStr(s.name or ""), searchLower) then table.insert(out, s) end
  end
  table.sort(out, function(a, b) return lowerStr(a.name or "") < lowerStr(b.name or "") end)
  return out
end

-- ============================================================
-- ===== НАБОРЫ: ПОКУПКА И ВЫДАЧА =====
-- ============================================================
local function performSetPurchase()
  TransactionModule.checkTimeout()
  local ok, err = validateTerminalState()
  ...
  if setIsPurchased(set) then openErrorModal("Ошибка", "Набор уже куплен — заберите содержимое"); return end
  if not setsAvailable(set) then openErrorModal("Недоступно", "В МЭ недостаточно предметов для этого набора"); return end
  ...
  if (State.currentSession.balance or 0) < coin or (State.currentSession.emaBalance or 0) < ema then
    local missC = math.max(0, coin - (State.currentSession.balance or 0))
    local missE = math.max(0, ema - (State.currentSession.emaBalance or 0))
    safeOpenModal("insufficient", { itemName = set.name, qty = 1, missC = missC, missE = missE })
    return
  end
  local txid = computer.address():sub(1, 8) .. "_" .. computer.uptime()
  if not TransactionModule.lock(txid) then openErrorModal("Подожди", "Транзакция уже выполняется или слишком частые запросы."); return end
  setLogMessage("Выполняется покупка набора...", Colors.accent_main)
  ...
  PendingModule.add("transaction", { player = State.currentPlayer, type = "buy", item = "SET:" .. set.id, displayName = set.name, qty = 1, totalCoin = coin, totalEma = ema, timestamp = os.time() })
  Data.setsProgress[set.id] = { dispensed = {}, completed = false }
  FileModule.saveSetsProgress()
  PendingModule.add("set_progress", { player = State.currentPlayer, setId = set.id, dispensed = {}, completed = false })
  State.currentSession.balance = player.balance; State.currentSession.emaBalance = player.emaBalance; State.currentSession.transactions = player.transactions
  TransactionModule.unlock()
  clearLog()
  addLogMessage("✅ Куплен набор: " .. set.name, Colors.success_green)
  local segs = { { "Списано: ", Colors.text_bright } }
  ...
  if ema > 0 then table.insert(segs, { formatPrice(ema) .. " EM", Colors.orange }) end
...
local function performSetDispense()
  ...
  local txid = computer.address():sub(1, 8) .. "_" .. computer.uptime()
  if not TransactionModule.lock(txid) then openErrorModal("Подожди", "Транзакция уже выполняется или слишком частые запросы."); return end
  setLogMessage("Выдача набора: " .. set.name, Colors.accent_main)
  local inventoryFull = false
  for _, si in ipairs(set.items or {}) do
    if inventoryFull then break end
    local key = si.internalName .. ":" .. (si.damage or 0)
    local have = pr.dispensed[key] or 0
    local remaining = (si.qty or 0) - have
    if remaining > 0 then
      local id = si.internalName
      if not id:find(":", 1, true) then id = "minecraft:" .. id end
      local fingerprint = { id = id, dmg = si.damage or 0 }
      local gotTotal = 0
      while remaining > 0 do
        ...
        local toTake = math.min(remaining, 64, freeSlots * 64)
        local okExport, result = pcall(function() return me.exportItem(fingerprint, PULL_DIRECTION, toTake) end)
        local got = 0
        if okExport then
          if type(result) == "number" then got = result
        ...
        if got > 0 then
          gotTotal = gotTotal + got; remaining = remaining - got
        else
          local free2 = 36 - PimModule.countOccupiedSlots()
          if free2 <= 0 then inventoryFull = true
          else addLogMessage("⚠ Нет в МЭ: " .. (si.displayName or si.internalName), Colors.warning) end
          break
        end
      end
      if gotTotal > 0 then
        pr.dispensed[key] = have + gotTotal
        addLogMessage("✔ Выдано: " .. (si.displayName or si.internalName) .. " x" .. gotTotal, Colors.success_green)
      end
    end
  end
  local done = true
  for _, si in ipairs(set.items or {}) do
    if (pr.dispensed[si.internalName .. ":" .. (si.damage or 0)] or 0) < (si.qty or 0) then done = false; break end
  end
  if done then
    pr.completed = true
    addLogMessage("✅ Набор полностью выдан: " .. set.name, Colors.success_green)
  elseif inventoryFull then
    addLogMessage("⚠ Инвентарь заполнен. Вернитесь позже, чтобы забрать остаток.", Colors.warning)
  end
  FileModule.saveSetsProgress()
  PendingModule.add("set_progress", { player = State.currentPlayer, setId = set.id, dispensed = pr.dispensed, completed = pr.completed })
  TransactionModule.unlock()
  markDirty("full")
end

-- ============================================================
-- ===== НАБОРЫ: СИНХРОНИЗАЦИЯ =====
-- ============================================================
local function loadSetsPages()
  local all = {}
  local page, totalPages = 1, 1
  ...
  while page <= totalPages do
    ...
    os.sleep(0.1)
    local data = HttpModule.request("oc_get_sets", { page = page, terminalId = Config.TERMINAL_ID })
    if not data then State.welcomeLog[logIndex] = { message = "❌ Наборы: нет ответа", color = Colors.error_red }; drawWelcomeScreen(); return nil end
    if data.status ~= "ok" then State.welcomeLog[logIndex] = { message = "❌ Наборы: " .. tostring(data.message), color = Colors.error_red }; drawWelcomeScreen(); return nil end
    for _, s in ipairs(data.sets or {}) do table.insert(all, s) end
    totalPages = data.totalPages or 1
    page = page + 1
  end
  State.welcomeLog[logIndex] = { message = "✅ Наборы: " .. #all .. " шт.", color = Colors.success_green }; drawWelcomeScreen()
  return all, (tonumber(data and data.version) or 0)
end

local function applySetsDelta(changes, newVersion)
  local cur = Data.setsCatalog
  local index = {}
  for i, s in ipairs(cur) do index[s.id] = i end
  local addCnt, updCnt, delCnt = 0, 0, 0
  for _, ch in ipairs(changes or {}) do
    local s = ch.set or {}
    if s.id then
      if ch.action == "REMOVE" then
        local i = index[s.id]
        if i then
          table.remove(cur, i)
          index = {}
          for i2, s2 in ipairs(cur) do index[s2.id] = i2 end
          delCnt = delCnt + 1
        end
      else
        local i = index[s.id]
        if i then cur[i] = s; updCnt = updCnt + 1 else table.insert(cur, s); addCnt = addCnt + 1 end
      end
    end
  end
  Data.versions.sets = newVersion
  FileModule.saveSets(cur)
  CatalogModule.saveVersions()
  if State.currentShopMode == "sets" then
    local list = setsFilteredList()
    if State.selectedSet then
      local found = nil
      for _, s in ipairs(list) do if s.id == State.selectedSet.id then found = s; break end end
      State.selectedSet = found
      if not found then State.setView = "list"; State.selectedSetItem = nil
      elseif State.selectedSetItem then
        local f2 = nil
        for _, si in ipairs(State.selectedSet.items or {}) do
          if si.internalName == State.selectedSetItem.internalName and (si.damage or 0) == (State.selectedSetItem.damage or 0) then f2 = si; break end
        end
        State.selectedSetItem = f2
      end
    end
    markDirty("full")
  end
...
local function syncSetsIfNeeded(source)
  local v = HttpModule.request("oc_get_versions", { terminalId = Config.TERMINAL_ID })
  if not v or v.status ~= "ok" then
    writeDebugLog("Наборы: сервер версий недоступен")
    return
  end
  local serverSets = tonumber(v.setsVersion) or 0
  -- Если локальный каталог пуст — всегда полная загрузка, независимо от сохранённой версии
  if #Data.setsCatalog == 0 then
    loadSetsFull()
    return
  end
  if serverSets == (Data.versions.sets or 0) then return end
  local delta = HttpModule.request("oc_get_sets_delta", { terminalId = Config.TERMINAL_ID, fromVersion = Data.versions.sets or 0 })
  if delta and delta.status == "ok" then
    applySetsDelta(delta.changes or {}, delta.version or serverSets)
  else
    -- дельта недоступна или need_full — грузим целиком
    loadSetsFull()
  end
end

local function loadSetsProgressFromServer(playerName)
  local data = HttpModule.request("oc_get_set_progress", { terminalId = Config.TERMINAL_ID, player = playerName })
  if not data or data.status ~= "ok" then return end
  for _, p in ipairs(data.progress or {}) do
    local disp = p.dispensed
    if type(disp) == "string" then
      local ok2, parsed = pcall(from_json, disp)
      if ok2 and type(parsed) == "table" then disp = parsed else disp = {} end
    end
    local localPr = Data.setsProgress[p.set_id]
    local localDone = localPr and setDispensedTotalByMap(localPr.dispensed or {}) or 0
    local srvDone = setDispensedTotalByMap(disp or {})
    -- сервер авторитетен по completed; по dispensed берём максимум (защита от гонки)
    local merged = disp or {}
    if localPr and (not localPr.completed) and (not (p.completed == 1 or p.completed == true)) then
      for k, v in pairs(localPr.dispensed or {}) do
        if (merged[k] or 0) < v then merged[k] = v end
      end
    end
    Data.setsProgress[p.set_id] = { dispensed = merged, completed = (p.completed == 1 or p.completed == true) }
  end
  FileModule.saveSetsProgress()
end
...
-- ===== ВХОД / ВЫХОД =====
-- ============================================================
local function abortOpen(playerName)
...
local function openShopForPlayer(playerName)
  ...
  if player.balance == 0 and player.transactions == 0 then
    ...
    if userData and userData.status == "ok" and userData.user then
      player.balance = tonumber(userData.user.balanceCoin) or 0; player.emaBalance = tonumber(userData.user.balanceEma) or 0
      player.transactions = tonumber(userData.user.transactions) or 0; player.regDate = userData.user.regDate or getRealTimeString()
      PlayerModule.save()
    end
  end
  ...
  if State.currentSession.banned == true or State.currentSession.banned == 1 then
    openErrorModal("Доступ запрещён", tostring(State.currentSession.banReason or "Вы заблокированы"))
    return
  end
  if pimPresenceState() == "absent" then abortOpen(playerName); return end
  -- подтягиваем прогресс наборов с сервера
  loadSetsProgressFromServer(playerName)
  State.currentScreen = "shop"
  if State.currentShopMode == "sets" then
    local list = setsFilteredList()
    if #list > 0 then
      State.selectedIndex = 1; State.selectedSet = list[1]
      if State.setView == "contents" and State.selectedSet.items and #State.selectedSet.items > 0 then State.selectedSetItem = State.selectedSet.items[1]
      else State.setView = "list"; State.selectedSetItem = nil end
    else
      State.selectedIndex = 0; State.selectedSet = nil; State.selectedSetItem = nil
    end
...
local function handleTouch(x, y, playerName)
  ...
  if State.searchInputActive then State.searchInputActive = false; markDirty("header") end
  for _, btn in ipairs(State.buttons) do
    if isButtonClicked(btn, x, y) then
      if btn.id == "clear_search" then
        ...
        if State.currentShopMode == "sets" then
          local list = setsFilteredList()
          if #list > 0 then State.selectedIndex = 1; State.selectedSet = list[1] else State.selectedIndex = 0; State.selectedSet = nil; State.selectedSetItem = nil end
        else
          applyFilter()
      ...
      elseif btn.id == "set_open" then
        if State.selectedSet then
          State.setView = "contents"; State.selectedIndex = 1; State.listScroll = 1
          if State.selectedSet.items and #State.selectedSet.items > 0 then State.selectedSetItem = State.selectedSet.items[1] else State.selectedSetItem = nil end
          markDirty("full")
        end
      elseif btn.id == "set_back" then
        State.setView = "list"; State.selectedSetItem = nil
        State.selectedIndex = 1; State.listScroll = 1
        local list = setsFilteredList()
        State.selectedSet = list[1]
        clearLog()
        markDirty("full")
      ...
      elseif btn.id == "max_qty" then
        ...
        elseif State.currentShopMode == "sell" and State.sellPlayerQty > 0 then State.sellQuantity = State.sellPlayerQty; State.sellInputActive = false end
        markDirty("right")
  ...
  if x >= listX and x < listX + listW and y >= listY + 4 and y < listY + 4 + State.visibleRows then
    ...
    if State.currentShopMode == "sets" then
      local list = (State.setView == "contents") and (State.selectedSet and State.selectedSet.items or {}) or setsFilteredList()
      local entry = list[idx]
      if entry then
        State.selectedIndex = idx
        if State.setView == "contents" then State.selectedSetItem = entry
        else State.selectedSet = entry; State.selectedSetItem = nil end
  ...
  if x >= inputX and x < inputX + inputW + 2 and y == inputY then
    if State.currentShopMode == "buy" then State.purchaseInputActive = true; State.sellInputActive = false else State.sellInputActive = true; State.purchaseInputActive = false end
...
function switchToMode(mode)
  ...
  if mode == "sets" then
    ...
    local list = setsFilteredList()
    if #list > 0 then State.selectedIndex = 1; State.selectedSet = list[1] else State.selectedIndex = 0; State.selectedSet = nil end
    State.listScroll = 1
    updateSelectorDisplay(nil)
  else
    State.selectedSet = nil; State.selectedSetItem = nil; State.setView = "list"
    if mode == "buy" then
    ...
    if #Data.filteredItems > 0 then
      State.selectedIndex = 1; State.selectedItem = Data.filteredItems[1]
      if State.currentShopMode == "sell" then refreshSellPlayerQty() else State.sellPlayerQty = 0 end
      updateSelectorDisplay(State.selectedItem)
...
local function main()
  ...
  PlayerModule.load(); CatalogModule.loadBuy(); CatalogModule.loadSell(); ReportModule.load(); PendingModule.load(); CatalogModule.loadVersion()
  FileModule.loadSets(); FileModule.loadSetsProgress()
  drawWelcomeScreen()
  pimIntrospect()
  if #Data.buyCatalog == 0 and #Data.sellCatalog == 0 then
    loadFullInit()
    if State.catalogsLoaded then
      local all, ver = loadSetsPages()
      if all then Data.versions.sets = ver or 0; FileModule.saveSets(all); CatalogModule.saveVersions() end
      drawWelcomeScreen()
    end
  else
    applyBuyCatalog(Data.buyCatalog); applySellCatalog(Data.sellCatalog)
  ...
  while true do
    ...
    if State.needStockRefresh and State.pimActive and State.currentScreen == "shop" then
      ...
      if State.currentShopMode == "sets" then markDirty("full") else updateBuyCatalogFromME() end
    end
    if not State.pimActive and not State.syncInProgress and computer.uptime() - State.lastCatalogCheck > Config.CATALOG_CHECK_INTERVAL then
      State.lastCatalogCheck = computer.uptime(); syncCatalogsIfNeeded("timer"); syncSetsIfNeeded("timer")
    end
    if not State.pimActive and not State.syncInProgress and computer.uptime() - State.lastCatalogCheck > Config.CATALOG_CHECK_INTERVAL then
      State.lastCatalogCheck = computer.uptime()
      syncCatalogsIfNeeded("timer")
      syncSetsIfNeeded("timer")
    end
    ...
    elseif e == "key_down" then
      ...
      if State.pimActive and State.currentPlayer and PimModule.isOwner(playerName) and PimModule.ensureValid(State.currentPlayer) then
        if isCraftingBusy() then
          -- игрок ждёт крафт
        else if State.autocraftJob and ch == 27 then
        ...
        elseif State.searchInputActive then
          if ch == 13 or ch == 27 then
            if ch == 27 then State.searchInput = "" end
            State.searchInputActive = false
            if State.currentShopMode == "sets" then
              local list = setsFilteredList()
              State.selectedIndex = #list > 0 and 1 or 0
              State.selectedSet = list[State.selectedIndex]
              if State.setView == "contents" then State.selectedSetItem = nil end
            else
              applyFilter()
        ...
        elseif State.purchaseInputActive or State.sellInputActive then
          if ch == 13 then State.purchaseInputActive = false; State.sellInputActive = false; markDirty("right")
          elseif ch == 8 then
            local qty = State.currentShopMode == "buy" and State.purchaseQuantity or State.sellQuantity
    ...
    elseif e == "scroll" then
      ...
      local x, y = ev[3], ev[4]
      local playerName = ev[6] or "Неизвестный"
      if State.pimActive and State.currentPlayer and PimModule.isOwner(playerName) and PimModule.ensureValid(State.currentPlayer) and not State.categoryDropdownOpen and not isCraftingBusy() then
        local listX, listY = UI.list.x, UI.list.y
        local listW = math.floor(Config.SCREEN_W * UI.list.wRatio)
        if x >= listX and x < listX + listW and y >= listY + 4 and y < listY + 4 + State.visibleRows then
          local listLen
          if State.currentShopMode == "sets" then
            ...
            listLen = #Data.filteredItems
          end
          local maxScroll = math.max(0, listLen - State.visibleRows)
...
gpu.setBackground(Colors.bg_main)
event.listen("terminate", function() State.isShuttingDown = true; PlayerModule.save(); PendingModule.save(); FileModule.saveSetsProgress() end)
event.listen("computer_shutdown", function() State.isShuttingDown = true; PlayerModule.save(); PendingModule.save(); FileModule.saveSetsProgress() end)
while true do
