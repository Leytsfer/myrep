-- ============================================================
-- TRADE MARKET CLIENT v6.10.0 (SETS & QUESTS)
-- ============================================================
...
setView = "list", selectedSet = nil, selectedSetItem = nil, setScroll = 1
}
local Data = {
...
-- ===== НАБОРЫ: ФИЛЬТРЫ/ХЕЛПЕРЫ =====
local function applySetsFilter()
Data.setsDisplay = {}
local searchLower = lowerStr(State.searchInput or "")
for _, s in ipairs(Data.setsCatalog) do
if s.enabled ~= 0 and s.enabled ~= false then
local nameLower = lowerStr(s.name or "")
if matchesSearch(nameLower, searchLower) then table.insert(Data.setsDisplay, s) end
end
end
...
local function meIndexAll()
local idx = {}
if not component.isAvailable("me_interface") then return idx end
local me = component.me_interface
local ok, items = pcall(function() return me.getItemsInNetwork() end)
if not ok or type(items) ~= "table" then return idx end
for _, it in ipairs(items) do idx[it.name .. ":" .. (it.damage or 0)] = (idx[it.name .. ":" .. (it.damage or 0)] or 0) + (it.size or 0) end
return idx
end
local function setAvailable(set)
if not set then return false end
local idx = meIndexAll()
for _, it in ipairs(set.items or {}) do
if (idx[it.internalName .. ":" .. (it.damage or 0)] or 0) < (it.qty or 0) then return false end
end
return true
end
local function setKey(it) return (it.internalName or "") .. ":" .. (it.damage or 0) end
local function setProgressOf(set)
...
return pr.dispensed[setKey(it)] or 0
end
...
local t = 0
for _, it in ipairs(set.items or {}) do t = t + (it.qty or 0) end
return t
end
...
for _, it in ipairs(set.items or {}) do
if (pr.dispensed or {})[setKey(it)] or 0 < (it.qty or 0) then return false end
end
return true
end
...
local idx = index[sid]
if idx then table.remove(currentData, idx); index = {}; for i2, s2 in ipairs(currentData) do index[tonumber(s2.id)] = i2 end; delCnt = delCnt + 1 end
else
local idx = index[sid]
if idx then currentData[idx] = s; updCnt = updCnt + 1 else table.insert(currentData, s); index[sid] = #currentData; addCnt = addCnt + 1 end
end
end
end
...
addWelcomeLog("Дельта наборов недоступна, грузим наборы целиком...", Colors.warning)
...
PlayerModule.save()
local sets = loadSetsPages()
if sets then Data.setsCatalog = sets; vSets = vSets else addWelcomeLog("Наборы: ошибка загрузки, будет повтор", Colors.warning) end
...
table.insert(State.welcomeLog, { message = "Готово! Buy: " .. buyCount .. ", Sell: " .. sellCount .. ", Users: " .. userCount .. ", Sets: " .. #Data.setsCatalog, color = Colors.success_green }); drawWelcomeScreen()
end
...
local avail = setAvailable(s)
writeText(listX + listW - 32, y, avail and "ДА" or "НЕТ", avail and Colors.success_green or Colors.text_dark, bg)
...
local idx = meIndexAll()
for row = 1, State.visibleRows do
local i2 = State.listScroll + row - 1
local it = items[i2]
local y = listY + 3 + row
if it then
local isSelected = (i2 == State.selectedIndex)
local bg = isSelected and Colors.bg_selected or Colors.bg_main
Buffer.fill(listX + 1, y, listW - 2, 1, " ", Colors.text_bright, bg)
local symbol = isSelected and ">" or "*"
writeText(listX + 2, y, symbol, isSelected and Colors.cyan or Colors.purple, bg)
...
writeText(listX + 4, y, name, isSelected and Colors.white or Colors.text_bright, bg)
local meCnt = idx[it.internalName .. ":" .. (it.damage or 0)] or 0
...
local function drawSetBox(panelX, panelY, panelW, topH, s)
drawBox(panelX, panelY, panelW, topH, Colors.line, Colors.bg_main)
...
local lineSpacing = 2
local baseY = panelY + 3
writeText(panelX + 2, baseY, "НАБОР: " .. (s.name or ""), Colors.text_bright, Colors.bg_main)
local avail = setAvailable(s)
writeText(panelX + 2, baseY + lineSpacing, "СТАТУС: " .. (avail and "Готов к выдаче" or "Недоступен"), avail and Colors.success_green or Colors.error_red, Colors.bg_main)
local coinPrice, emaPrice = s.price_coin or 0, s.price_ema or 0
...
local function drawSetContentsBox(panelX, panelY, panelW, topH, s)
drawBox(panelX, panelY, panelW, topH, Colors.line, Colors.bg_main)
local title = "СОДЕРЖИМОЕ НАБОРА"
writeText(panelX + 2, panelY + 1, title, Colors.cyan, Colors.bg_main)
local ds = panelX + 2 + unicode.len(title) + 1
...
local totalDisp = setTotalDispensed(s)
local totalQty = setTotalQty(s)
...
local btnY = inputY + 2
local purchased = setProgressOf(s) and not setCompleted(s)
local bw1, bw2 = 16, 10
local bx1 = panelX + 2
local bx2 = bx1 + bw1 + 2
if purchased then
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.10.0", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.10.0", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
-- ===== НАБОРЫ: ПОКУПКА/ВЫДАЧА =====
local function performSetPurchase()
TransactionModule.checkTimeout()
local ok, err = validateTerminalState()
if not ok then openErrorModal("Ошибка покупки набора", err); return end
local s = State.selectedSet
if not s then openErrorModal("Ошибка", "Набор не выбран"); return end
if setProgressOf(s) and not setCompleted(s) then openErrorModal("Ошибка", "Набор уже куплен, заберите содержимое"); return end
if not setAvailable(s) then openErrorModal("Недоступно", "В МЭ нет всех предметов набора"); return end
local coin, ema = s.price_coin or 0, s.price_ema or 0
...
safeOpenModal("insufficient", { itemName = s.name, qty = 1, missC = missC, missE = missE })
return
end
local txid = computer.address():sub(1, 8) .. "_" .. computer.uptime()
if not TransactionModule.lock(txid) then openErrorModal("Подожди", "Транзакция уже выполняется или слишком частые запросы."); return end
...
PlayerModule.addTransaction(State.currentPlayer, { time = getRealTimeString(), type = "buy", item = s.name, qty = 1, coin = coin, ema = ema })
PendingModule.add("transaction", { player = State.currentPlayer, type = "buy", item = "SET:" .. s.id, displayName = s.name, qty = 1, totalCoin = coin, totalEma = ema, timestamp = os.time() })
State.currentSession.balance = player.balance; State.currentSession.emaBalance = player.emaBalance; State.currentSession.transactions = player.transactions
Data.setsProgress[s.id] = { dispensed = {}, completed = false }
CatalogModule.saveSetsProgress()
PendingModule.add("set_progress", { player = State.currentPlayer, setId = s.id, dispensed = {}, completed = false })
TransactionModule.unlock()
clearLog()
addLogMessage("✅ Куплен набор: " .. s.name, Colors.success_green)
local segs = { { "Списано: ", Colors.text_bright } }
...
TransactionModule.checkTimeout()
local ok, err = validateTerminalState()
if not ok then openErrorModal("Ошибка выдачи набора", err); return end
local s = State.selectedSet
if not s then openErrorModal("Ошибка", "Набор не выбран"); return end
local pr = setProgressOf(s)
...
local txid = computer.address():sub(1, 8) .. "_" .. computer.uptime()
if not TransactionModule.lock(txid) then openErrorModal("Подожди", "Транзакция уже выполняется или слишком частые запросы."); return end
setLogMessage("Выдача набора: " .. s.name, Colors.accent_main)
local me = component.me_interface
...
local remaining = (it.qty or 0) - have
if remaining > 0 then
...
if got > 0 then gotTotal = gotTotal + got; remaining = remaining - got else break end
end
if gotTotal > 0 then
pr.dispensed[key] = have + gotTotal
addLogMessage("✔ Выдано: " .. (it.displayName or it.internalName) .. " x" .. gotTotal, Colors.success_green)
acDecrementSnapshot(it.internalName, it.damage or 0, gotTotal)
end
if remaining > 0 and not inventoryFull then
addLogMessage("⚠ Нет в МЭ: " .. (it.displayName or it.internalName), Colors.warning)
end
end
end
local done = setCompleted(s)
pr.completed = done
CatalogModule.saveSetsProgress()
PendingModule.add("set_progress", { player = State.currentPlayer, setId = s.id, dispensed = pr.dispensed, completed = done })
TransactionModule.unlock()
local totalDisp = setTotalDispensed(s)
local totalQty = setTotalQty(s)
if done then
addLogMessage("✅ Набор полностью выдан: " .. s.name, Colors.success_green)
else
if inventoryFull then
addLogMessage("⚠ Инвентарь заполнен. Выдано " .. totalDisp .. "/" .. totalQty .. ". Вернитесь позже.", Colors.warning)
else
addLogMessage("⚠ Выдано частично: " .. totalDisp .. "/" .. totalQty, Colors.warning)
end
end
markDirty("full")
end
...
if State.currentShopMode == "sets" then
applySetsFilter()
if State.setView == "contents" and State.selectedSet then State.selectedIndex = 1; State.selectedSetItem = (State.selectedSet.items or {})[1] end
else
applyFilter()
...
addLogMessage("Выберите предмет и нажмите Выдать набор.", Colors.text_dark)
markDirty("full")
end
...
if State.setView == "list" then
local s = Data.setsDisplay[idx]
if s then State.selectedIndex = idx; State.selectedSet = s; State.selectedSetItem = nil; markDirty("full") end
else
local items = (State.selectedSet and State.selectedSet.items) or {}
local it = items[idx]
...
elseif mode == "sets" then
State.setView = "list"; State.listScroll = 1
applySetsFilter()
if #Data.setsDisplay > 0 then State.selectedIndex = 1; State.selectedSet = Data.setsDisplay[1] else State.selectedIndex = 0; State.selectedSet = nil end
State.selectedSetItem = nil
updateSelectorDisplay(nil)
...
State.presenceWorks = true
writeDebugLog("Watchdog: PIM пуст — закрываем магазин")
handlePlayerLeave()
end
end
if e == "touch" then
