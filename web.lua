...
FILES = { players = "/home/players.db", buyCatalog = "/home/buyCatalog.lua", sellCatalog = "/home/sellCatalog.lua", reports = "/home/reports.json", pendingChanges = "/home/pending_changes.lua", catalogVersion = "/home/catalogVersion.dat" }
}
...
deferredEvents = {},
lastPimCheck = 0, pimAbsentCount = 0,
...
currentShopMode = "buy", lastCatalogUpdate = 0,
...
-- компактный формат количества: 1k / 50k / 100k / 1kk / 5kk / 1kkk
...
-- формат с пробелами: 100 000 / 1 700 500 / 10 000 000 000
...
if type(data) == "table" then Data.versions = { buy = tonumber(data.buy) or 0, sell = tonumber(data.sell) or 0, users = tonumber(data.users) or 0 }
else Data.versions = { buy = tonumber(data) or 0, sell = 0, users = 0 } end
return Data.versions
end
function CatalogModule.saveVersions()
FileModule.saveLuaFile(Config.FILES.catalogVersion, Data.versions)
writeDebugLog("💾 Версии: buy=" .. Data.versions.buy .. " sell=" .. Data.versions.sell .. " users=" .. Data.versions.users)
end
...
local actual = PimModule.getPlayer()
if actual and actual ~= "" and (actual ~= State.currentPlayer or actual ~= State.pimOwner) then return false end
return true
end
...
Data.filteredItems = {}
local searchLower = string.lower(State.searchInput or "")
...
if (searchLower == "" or string.find(nameLower, searchLower, 1, true)) and itemInCategory(item, categoryTagList) then table.insert(Data.filteredItems, item) end
end
...
if not needBuy and not needSell and not needUsers then addWelcomeLog("✅ Версии актуальны, обновление не требуется", Colors.success_green); return syncEnd("ok", source) end
...
usersData = nil
PlayerModule.save()
Data.versions = { buy = vBuy, sell = vSell, users = vUsers }
CatalogModule.saveVersions()
...
local function setLogMessage(text, color) State.logMessages = {}; if text and text ~= "" then table.insert(State.logMessages, {text = text, color = color or Colors.text_dark}) end; markDirty("right") end
local function addLogMessage(text, color) if text and text ~= "" then table.insert(State.logMessages, {text = text, color = color or Colors.text_dark}) end; markDirty("right") end
...
elseif kind == "my_reports" then modalW, modalH = 90, 26 end
...
elseif kind == "report_select" then borderColor, title = Colors.tomato, "НАШЕЛ ОШИБКУ?"
...
-- кнопки с вертикальными отступами (не налезают друг на друга)
...
local maxLines = logH - 3
local total = #State.logMessages
local startIdx = math.max(1, total - maxLines + 1)
for i = startIdx, total do
local entry = State.logMessages[i]
local y = logY + 3 + (i - startIdx)
if y > logY + logH - 2 then break end
if entry.segs then
local w = 0
for _, s in ipairs(entry.segs) do w = w + unicode.len(s[1]) end
local xx = panelX + math.floor((panelW - w) / 2)
for _, s in ipairs(entry.segs) do writeText(xx, y, s[1], s[2], Colors.bg_main); xx = xx + unicode.len(s[1]) end
else
local msg = tostring(entry.text or "")
writeText(panelX + math.floor((panelW - unicode.len(msg)) / 2), y, msg, entry.color or Colors.text_bright, Colors.bg_main)
end
end
end
...
local acText = acSt == "yes" and "ДОСТУПЕН" or (acSt == "no_resources" and "НЕ ХВАТАЕТ РЕСУРСОВ" or (acSt == "no" and "НЕТ ШАБЛОНА" or "ПРОВЕРКА..."))
local acColor = acSt == "yes" and Colors.success_green or (acSt == "no_resources" and Colors.warning or (acSt == "no" and Colors.text_dark or Colors.cyan))
...
local useCraft = overStock and acAvailable(item) and acCanCraft(item)
...
drawButton(questX, y + 1, btnW, btnH, "[ Наборы | Квесты ]", Colors.purple, Colors.white, "page_quests")
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.7", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.7", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
-- надёжная отмена крафта: несколько попыток с проверкой статуса
local function cancelCraftReq(req)
if not req then return false end
for i = 1, 6 do
pcall(function() req:cancel() end)
pcall(function() req.cancel(req) end)
os.sleep(0.1)
...
writeDebugLog("⚠️ Не удалось подтвердить отмену крафта")
return false
end
...
if not ok1 or type(list) ~= "table" or #list == 0 then State.acCraftCache[k] = false; return end
local chosen = nil
for _, c in ipairs(list) do
local st = acStackOf(c)
if st then
local nm = st.name or st.label or st.id
local d = tonumber(st.damage or st.dmg or 0) or 0
if nm and acKey(nm, d) == k then chosen = { name = nm, damage = d, size = (tonumber(st.size) or 1), raw = c }; break end
end
end
if not chosen and #list > 0 then
local c = list[1]
local st = acStackOf(c)
local nm = (st and (st.name or st.label or st.id)) or full
local d = tonumber(st and (st.damage or st.dmg) or 0) or 0
chosen = { name = nm, damage = d, size = (tonumber(st and st.size) or 1), raw = c }
end
if not chosen then State.acCraftCache[k] = false; return end
local req = nil
local shapes = {
function() return chosen.raw.request(1) end,
function() return me.requestCrafting(chosen.raw, 1) end,
function() return chosen.raw:request(1) end,
}
...
if not req then
State.acCraftCache[k] = { name = chosen.name, damage = chosen.damage, size = chosen.size, raw = chosen.raw, canCraft = false }
return
end
-- даём AE2 время зарегистрировать задачу, затем проверяем и надёжно отменяем
os.sleep(0.5)
local canceled = false
local isCanceled = callMethod(req, "isCanceled")
if isCanceled == true then canceled = true end
local status = callMethod(req, "getStatus")
if not canceled and type(status) == "table" then
if status.isCanceled or status.canceled or status.state == "canceled" then canceled = true
elseif (type(status.missing) == "table" and #status.missing > 0) then canceled = true
elseif (type(status.missingItems) == "table" and #status.missingItems > 0) then canceled = true end
end
-- принудительно отменяем тестовый крафт
cancelCraftReq(req)
State.acCraftCache[k] = { name = chosen.name, damage = chosen.damage, size = chosen.size, raw = chosen.raw, canCraft = not canceled }
...
acCanCraft = function(item)
if not item then return false end
local v = State.acCraftCache[acKey(item.internalName, item.damage or 0)]
return type(v) == "table" and v.canCraft == true
end
acStatusOf = function(item)
if not item then return "unknown" end
local v = State.acCraftCache[acKey(item.internalName, item.damage or 0)]
if type(v) == "table" then
if v.canCraft == true then return "yes" elseif v.canCraft == false then return "no_resources" else return "unknown" end
...
local v = State.acCraftCache[acKey(item.internalName, item.damage or 0)]
if type(v) == "table" and v.canCraft == true then return v end
return nil
end
...
local function acLogJob(job, segs)
local plain = ""
for _, s in ipairs(segs) do plain = plain .. s[1] end
local entry = { segs = segs, text = plain }
...
if type(v) == "table" and v.canCraft == true then table.insert(list, { name = v.name, damage = v.damage, display = v.name }) end
end
...
job.paid = true
acLogJob(job, { { "Оплата подтверждена: ", Colors.text_bright }, { formatPrice(fullCoin) .. " COINS", Colors.accent_cyan }, { " / ", Colors.text_bright }, { formatPrice(fullEma) .. " EMA", Colors.orange } })
local me = component.me_interface
...
acLogJob(job, { { "Крафт завершён: ", Colors.text_bright }, { "успешно", Colors.success_green } })
job.phase = "done"
else
acLogJob(job, { { "✔ Выдано: ", Colors.warning }, { extracted .. "шт из " .. job.ordered .. "шт", Colors.accent_cyan } })
acLogJob(job, { { "Крафт завершён: ", Colors.text_bright }, { "частично", Colors.warning } })
job.phase = "partial"
end
...
if not entry then
openErrorModal("Автокрафт", "Для этого предмета нет шаблона крафта в ME или не хватает ресурсов.")
return
end
...
if (State.currentSession.balance or 0) < needCoin or (State.currentSession.emaBalance or 0) < needEma then
openErrorModal("Недостаточно средств", "Для заказа крафта нужен баланс на все " .. ordered .. " шт.")
return
end
...
job.lineStock = acLogJob(job, { { "Сейчас в МЭ: ", Colors.text_bright }, { inStock .. "шт", Colors.accent_cyan } })
job.lineTime = acLogJob(job, { { "Прошло времени: ", Colors.text_bright }, { "0 сек", Colors.accent_cyan } })
if job.toCraft <= 0 then
job.phase = "crafting"
job.startUptime = computer.uptime(); job.elapsed = 0
acDispense(job)
return
end
local entry = acEntryFor({ internalName = job.internalName, damage = job.damage })
if not entry then
acLogJob(job, { { "Крафт завершён: ", Colors.text_bright }, { "ошибка (нет шаблона)", Colors.error_red } })
...
if not req then
acLogJob(job, { { "Крафт завершён: ", Colors.text_bright }, { "ошибка (ME не приняла)", Colors.error_red } })
acFinishAndClose(job, false)
return
end
job.req = req
...
local st = craftStatus(req)
if st == "canceled" or st == "missing" or st == "error" then preOk = false; break end
os.sleep(0.25)
end
if not preOk then
cancelCraftReq(req)
acLogJob(job, { { "Крафт завершён: ", Colors.text_bright }, { "ошибка (нет ресурсов)", Colors.error_red } })
acFinishAndClose(job, false)
return
end
forceRender()
...
if now - (job.lastStockTick or 0) >= 1 then
...
local stock = acMeCount(job.internalName, job.damage)
if stock >= job.ordered then acDispense(job); return end
elseif st == "canceled" or st == "missing" or st == "error" then
cancelCraftReq(job.req)
acLogJob(job, { { "Крафт завершён: ", Colors.text_bright }, { "ошибка (отменён ME)", Colors.error_red } })
acFinishAndClose(job, false)
return
end
end
...
cancelCraftReq(job.req)
acLogJob(job, { { "Крафт завершён: ", Colors.text_bright }, { "ошибка (таймаут)", Colors.error_red } })
acFinishAndClose(job, false)
return
end
...
cancelCraftReq(job.req)
acLogJob(job, { { "Крафт завершён: ", Colors.text_bright }, { "отменён игроком", Colors.warning } })
acFinishAndClose(job, false)
end
end
...
if not ok then
if err == "Недостаточно Coina" or err == "Недостаточно EMA" then safeOpenModal("insufficient", { lines = { "Недостаточно средств для покупки.", "Пополни баланс и повтори попытку." } })
...
if totalCoin > 0 then addLogMessage("💰 Списано: " .. formatPrice(totalCoin) .. " COINS", Colors.accent_cyan) end
if totalEma > 0 then addLogMessage("💎 Списано: " .. formatPrice(totalEma) .. " EMA", Colors.tomato) end
...
if totalCoin > 0 then addLogMessage("💰 Получено: " .. formatPrice(totalCoin) .. " COINS", Colors.accent_cyan) end
if totalEma > 0 then addLogMessage("💎 Получено: " .. formatPrice(totalEma) .. " EMA", Colors.tomato) end
refreshSellPlayerQty(); scheduleClearLog(5)
else
...
-- игрок уже ушёл — откатываем состояние входа, магазин не открываем
...
-- проверка ДО открытия интерфейса: если PIM читается и игрока уже нет — не открываем
if pimSupportsRead() then
local actual = PimModule.getPlayer()
if not actual or actual == "" or actual ~= playerName then
...
if State.lastCatalogUpdate == 0 then updateBuyCatalogFromME() end
...
while true do
local pulled = { pcall(event.pull, 0) }
if not pulled[1] or pulled[2] == nil then break end
table.remove(pulled, 1)
local e2, a1 = pulled[1], pulled[2]
if e2 == "player_off" or e2 == "pim_player_leave" then if a1 == playerName then stillHere = false end
elseif e2 == "player_on" or e2 == "pim" or e2 == "pim_player_enter" then if a1 == playerName then stillHere = true end
else table.insert(State.deferredEvents, pulled) end
end
if stillHere and pimSupportsRead() then
local actual = PimModule.getPlayer()
if actual ~= playerName then stillHere = false; writeDebugLog("⚠️ Аппаратная проверка: игрока нет на PIM") end
end
-- добор "в пути": коротко дочитываем очередь событий, чтобы поймать поздний player_off
if stillHere then
local deadline = computer.uptime() + 0.25
while stillHere and computer.uptime() < deadline do
local pulled = { pcall(event.pull, 0.1) }
if pulled[1] and pulled[2] then
table.remove(pulled, 1)
local e2, a1 = pulled[1], pulled[2]
if e2 == "player_off" or e2 == "pim_player_leave" then
if a1 == playerName then stillHere = false end
elseif e2 == "player_on" or e2 == "pim" or e2 == "pim_player_enter" then
if a1 == playerName then stillHere = true end
else
table.insert(State.deferredEvents, pulled)
end
else
break
end
...
writeDebugLog("⚠️ Игрок ушёл во время синхронизации — магазин не открывается")
if not State.pimActive then State.welcomeLog = {}; drawWelcomeScreen() end
return
end
...
State.searchInput = ""; State.searchInputActive = false; applyFilter()
...
elseif btn.id == "page_quests" then openInfoModal("Наборы и квесты", { "Раздел в разработке.", "Здесь будут доступны готовые наборы предметов и квесты." }) end
return
end
end
local listX, listY = UI.list.x, UI.list.y
...
-- watchdog: быстрая проверка наличия игрока на PIM (каждую 1 сек)
...
if pimSupportsRead() then
local actual = PimModule.getPlayer()
if actual == State.currentPlayer then State.pimAbsentCount = 0
else
State.pimAbsentCount = State.pimAbsentCount + 1
if State.pimAbsentCount >= 2 then writeDebugLog("⚠️ Watchdog: игрок не на PIM — закрываем магазин"); State.pimAbsentCount = 0; handlePlayerLeave() end
end
end
end
if e == "touch" then
...
if State.autocraftJob and ch == 27 then
...
State.searchInputActive = false; applyFilter()
State.selectedIndex = #Data.filteredItems > 0 and 1 or 0
State.selectedItem = Data.filteredItems[State.selectedIndex]
if State.currentShopMode == "sell" then refreshSellPlayerQty() end
updateSelectorDisplay(State.selectedItem); markDirty("full")
elseif ch == 8 then
State.searchInput = unicode.sub(State.searchInput or "", 1, -2); applyFilter()
State.selectedIndex = #Data.filteredItems > 0 and 1 or 0
State.selectedItem = Data.filteredItems[State.selectedIndex]; updateSelectorDisplay(State.selectedItem); markDirty("full")
elseif ch >= 32 then
State.searchInput = (State.searchInput or "") .. unicode.char(ch); applyFilter()
State.selectedIndex = #Data.filteredItems > 0 and 1 or 0
State.selectedItem = Data.filteredItems[State.selectedIndex]; updateSelectorDisplay(State.selectedItem); markDirty("full")
end
...
local x, y = ev[3], ev[4]
local playerName = ev[6] or "Неизвестный"
if State.pimActive and State.currentPlayer and PimModule.isOwner(playerName) and PimModule.ensureValid(State.currentPlayer) and not State.categoryDropdownOpen then
local listX, listY = UI.list.x, UI.list.y
local listW = math.floor(Config.SCREEN_W * UI.list.wRatio)
if x >= listX and x < listX + listW and y >= listY + 4 and y < listY + 4 + State.visibleRows then
local maxScroll = math.max(0, #Data.filteredItems - State.visibleRows)
...
if not playerName or playerName == "" then local actual = PimModule.getPlayer(); if actual and actual ~= "" then playerName = actual end end
...
else writeDebugLog("⚠️ Не удалось определить игрока на PIM") end
end
elseif e == "player_off" or e == "pim_player_leave" then
local playerName = ev[2]
writeDebugLog("👋 player_off: " .. tostring(playerName))
local ignoreLeave = false
if State.currentPlayer and playerName == State.currentPlayer then
local actual = PimModule.getPlayer()
if actual and actual == State.currentPlayer then ignoreLeave = true end
end
if not ignoreLeave then handlePlayerLeave() end
end
end
end
...
gpu.setBackground(Colors.bg_main)
event.listen("terminate", function() State.isShuttingDown = true; PlayerModule.save(); PendingModule.save() end)
event.listen("computer_shutdown", function() State.isShuttingDown = true; PlayerModule.save(); PendingModule.save() end)
while true do
