...
-- поиск: без регистра, каждое слово запроса должно встречаться в названии
...
-- ===== ОТРИСОВКА =====
local function clearAllText() State.logMessages = {}; State.welcomeLog = {}; State.syncLogs = {}; State.logPending = {}; State.logNextAt = 0; Buffer.clear() end
...
writeText(Config.SCREEN_W - 20, Config.SCREEN_H, "v 6.9.9", Colors.text_dark, Colors.bg_panel)
local reportBtnW = 18
...
drawCenteredText(30, "Trade Shop by Leytsfer — v 6.9.9", Colors.text_main, Colors.bg_main)
local logStartY = 32
...
-- Проверка ТОЛЬКО наличия шаблона (без теста ресурсов)
...
-- очередь лога с задержкой 0.3с на строку
local function acLogJob(job, segs, live)
table.insert(State.logPending, { job = job, segs = segs, live = live })
end
local function pumpLogQueue()
local now = computer.uptime()
if #State.logPending == 0 then State.logNextAt = 0; return end
if State.logNextAt == 0 then State.logNextAt = now end
local pumped = false
while #State.logPending > 0 and now >= State.logNextAt do
local q = table.remove(State.logPending, 1)
local plain = ""
for _, s in ipairs(q.segs) do plain = plain .. s[1] end
local entry = { segs = q.segs, text = plain }
table.insert(State.logMessages, entry)
if #State.logMessages > 40 then table.remove(State.logMessages, 1) end
if q.job then
if q.live == "stock" then q.job.lineStock = entry
elseif q.live == "time" then q.job.lineTime = entry end
table.insert(q.job.pendingLogs, { text = plain, time = os.time(), player = State.currentPlayer or "", event = q.job.phase or "info" })
if #q.job.pendingLogs > 20 then table.remove(q.job.pendingLogs, 1) end
end
State.logNextAt = now + 0.3
pumped = true
end
if pumped then markDirty("right") end
end
local function acFlushLogQueue()
while #State.logPending > 0 do
local q = table.remove(State.logPending, 1)
local plain = ""
for _, s in ipairs(q.segs) do plain = plain .. s[1] end
local entry = { segs = q.segs, text = plain }
...
job.req = req
job.phase = "crafting"
job.startUptime = computer.uptime(); job.elapsed = 0; job.lastHeartbeat = 0; job.lastStockTick = 0; job.lastReportU = 0
...
if now - (job.lastHeartbeat or 0) >= 15 then
job.lastHeartbeat = now
acLogJob(job, { { "✔ Крафт идёт: ", Colors.success_green }, { math.floor(job.elapsed) .. " сек", Colors.accent_cyan } })
end
...
State.autocraftJob = nil
State.logPending = {}; State.logNextAt = 0
State.modalState.active = false; State.modalState.kind = nil; State.modalState.data = nil; State.modalData = nil
end
...
elseif btn.id == "autocraft_close" then State.autocraftJob = nil; State.logPending = {}; State.logNextAt = 0; forceRender()
...
State.autocraftJob = nil; State.logPending = {}; State.logNextAt = 0
applyFilter()
if #Data.filteredItems > 0 then
State.selectedIndex = 1; State.selectedItem = Data.filteredItems[1]
...
if State.autocraftJob.phase == "confirm" then State.autocraftJob = nil; State.logPending = {}; State.logNextAt = 0; forceRender()
