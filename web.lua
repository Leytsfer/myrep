...
local function applySetsDelta(changes, newVersion)
  local index = {}
  for i, s in ipairs(Data.setsCatalog) do index[s.id] = i end
  local addCnt, updCnt, delCnt = 0, 0, 0
  for _, ch in ipairs(changes or {}) do
    local s = ch.set or {}
    if s.id then
      if ch.action == "REMOVE" then
        if index[s.id] then table.remove(Data.setsCatalog, index[s.id]); index = {}; for i, ss in ipairs(Data.setsCatalog) do index[ss.id] = i end; delCnt = delCnt + 1 end
      else
        if index[s.id] then Data.setsCatalog[index[s.id]] = s; updCnt = updCnt + 1 else table.insert(Data.setsCatalog, s); addCnt = addCnt + 1 end
      end
    end
  end
  Data.versions.sets = newVersion
  CatalogModule.saveSets(Data.setsCatalog); CatalogModule.saveVersions()
...
local function syncSetsIfNeeded(source)
  local v = HttpModule.request("oc_get_versions", { terminalId = Config.TERMINAL_ID })
  if not v or v.status ~= "ok" then return end
  local serverSets = tonumber(v.setsVersion) or 0
  ...
  if delta and delta.status == "ok" then
    applySetsDelta(delta.changes or {}, serverSets)
  elseif source == "player" then
    State.pendingFullReload = true
  else
    local all, ver = loadSetsPages()
    if all then Data.versions.sets = ver or serverSets; FileModule.saveSets(all); CatalogModule.saveVersions() end
  end
end
