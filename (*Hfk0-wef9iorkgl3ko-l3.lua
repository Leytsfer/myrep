...
local CLIENT_VERSION = "6.10.6"   -- текущая версия ЭТОГО файла; при релизе меняй и её, и шапку
...
local function performSelfUpdate()
    ...
    -- Прямой запрос без JSON-обёртки
    ...
    if not okReq or not code or type(code) ~= "string" or #code < 500 then
        writeDebugLog("⛔ Самообновление: сервер не отдал код ("
            .. tostring(code and #code or 0) .. " байт)")
        selfUpdateBusy = false
        return false
    end

    -- Если сервер вернул текст ошибки
    ...
    -- Извлекаем версию из шапки для лога
    ...
    os.sleep(0.5)          -- даём логу записаться на диск
...
-- Безопасное самообновление: только когда терминал простаивает
