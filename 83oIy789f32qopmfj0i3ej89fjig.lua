...
local function performSelfUpdate()
  ...
  local okReq, r = pcall(function()
    return HttpModule.request("oc_get_update", { terminalId = Config.TERMINAL_ID })
  end)
  if not okReq or not r or r.status ~= "ok" or type(r.code) ~= "string" or #r.code < 500 then
    writeDebugLog("⛔ Самообновление: сервер не отдал код")
    selfUpdateBusy = false
    return false
  end
  local okCompile = pcall(function() assert(load(r.code, "=selfupdate_check", "t", {})) end)
  ...
  if not f then selfUpdateBusy = false; return false end
  ...
  writeDebugLog("✅ Самообновление установлено: v" .. tostring(r.version) .. ", перезагрузка")
