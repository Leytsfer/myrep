...
local function performSelfUpdate()
  if State.pimActive or State.syncInProgress or State.TRANSACTION_LOCK then return false end
  local r = HttpModule.request("oc_get_update", { terminalId = Config.TERMINAL_ID })
  if not r or r.status ~= "ok" or type(r.code) ~= "string" then return false end
  if (r.version or "0") == VERSION then return false end
  if #r.code < 2000 or not r.code:find("TRADE MARKET CLIENT", 1, true) then return false end
  local okCompile = pcall(function() assert(load(r.code, "=update_sanity", "t", {})) end)
  if not okCompile then writeDebugLog("⛔ Обновление отклонено: код не компилируется"); return false end
  local tmp = "/home/web.lua.new"
  local f = io.open(tmp, "w")
  if not f then return false end
  f:write(r.code); f:close()
  if fs.exists("/home/web.lua.bak") then fs.remove("/home/web.lua.bak") end
  if fs.exists("/home/web.lua") then fs.rename("/home/web.lua", "/home/web.lua.bak") end
  fs.rename(tmp, "/home/web.lua")
  writeDebugLog("✅ Применено обновление v" .. tostring(r.version) .. ", перезагрузка")
...
function SelfUpdate.checkAndApply()
  if State.pimActive or State.syncInProgress or State.TRANSACTION_LOCK then
    return false, "busy"
  end
  local r = HttpModule.request("oc_get_update", { terminalId = Config.TERMINAL_ID, version = VERSION })
  if not r or r.status ~= "ok" or type(r.code) ~= "string" then
    return false, "нет ответа сервера"
  end
  local newVer = tostring(r.version or "0")
  if newVer == VERSION then return false, "версия совпадает" end
  local ok, err = selfUpdateSanity(r.code)
  if not ok then
    writeDebugLog("⛔ Самообновление отклонено: " .. err)
    return false, err
  end
  local okWrite = pcall(function()
    local tmp = SELFUPDATE_PATH .. ".tmp"
    local f = assert(io.open(tmp, "w"))
    f:write(r.code)
    f:close()
    if fs.exists(SELFUPDATE_BAK) then fs.remove(SELFUPDATE_BAK) end
    if fs.exists(SELFUPDATE_PATH) then fs.rename(SELFUPDATE_PATH, SELFUPDATE_BAK) end
    assert(fs.rename(tmp, SELFUPDATE_PATH))
  end)
  if not okWrite then
    writeDebugLog("⛔ Самообновление: не удалось записать файл")
    return false, "write failed"
  end
  writeDebugLog("✅ Самообновление: v" .. VERSION .. " -> v" .. newVer .. ". Перезагрузка")
  State.isShuttingDown = true
  pcall(function() PlayerModule.save(); PendingModule.save(); CatalogModule.saveSetsProgress() end)
  computer.shutdown(true)
  return true
end
...
-- Главный признак: имя игрока на PIM (все методы И поле, как в PimModule.getPlayer)
local nm = PimModule.getPlayer()
if nm and nm ~= "" then return "present" end
-- Фолбэк: инвентарь (только если имя прочитать не удалось)
...
if type(s) == "table" and type(s[1]) == "string" then total = total + unicode.len(s[1]) end
end
...
if r.clientVersion and r.clientVersion ~= VERSION
   and not State.pimActive and not State.syncInProgress and not State.TRANSACTION_LOCK then
  performSelfUpdate()
  return
end
local changed = false
...
-- Финальная страховка: если игрок ушёл, пока магазин открывался (HTTP внутри openShopForPlayer) — сразу закрываем
...
-- Первое действие после открытия: если игрока уже нет на PIM (queued-событие) — закрываем магазин
...
-- ФИКС: при переходе между вкладками очищаем строку поиска
...
-- "unknown" намеренно не трогает счётчики: сбои компонента не должны закрывать магазин
end
...
if confirmedAbsent() then handlePlayerLeave(); return end
if State.bannedNow then
